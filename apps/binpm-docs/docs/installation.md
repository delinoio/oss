...
binpm does not edit shell profile files from install, add, doctor, or plain env output. Persistent profile changes are opt-in: use `binpm env --global --shell <shell>` for a non-mutating global PATH command, or preview the exact profile update with `binpm env setup --shell <shell> --dry-run` before applying it with `binpm env setup --shell <shell>`. Setup appends only the global bin PATH line and prints rollback guidance. The printed project-local command is for the current project or shell session only; `binpm env --local --shell <shell>` prints only that session command.
