...
`binpm install <source>` installs globally, even inside a repository with `binpm.toml`. Use `--as <cmd>` when the global command name should differ from the repository name, and `--bin <upstream-binary>` when an archive needs explicit binary selection. `binpm install <source> --local` is not supported; use `binpm add <cmd> <source>` to declare and install a project-local tool. `binpm install` without a package spec syncs the local `binpm.toml` manifest.
