...
```bash
pnpm exec vp run binpm-docs#dev
pnpm exec vp run binpm-docs#test
pnpm exec vp run binpm-docs#build
pnpm exec vp run binpm-docs#preview
```

`pnpm exec vp run binpm-docs#dev` runs Rspress on fixed port `46304` and exits on conflicts instead of automatically selecting another port.
`pnpm exec vp run binpm-docs#preview` serves the production build on fixed port `46261`.
