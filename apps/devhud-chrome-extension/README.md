...
Use `pnpm exec vp run devhud-chrome-extension#typecheck`, `lint`, `test:unit`, `test:components`, `test:accessibility`, `build:frontend`, and `test:package` for the split deterministic CI contracts. The aggregate `test` command remains available. Production builds require the matching `DEVHUD_CHROME_EXTENSION_ID` and base64 SPKI `DEVHUD_CHROME_EXTENSION_PUBLIC_KEY` release configuration.
