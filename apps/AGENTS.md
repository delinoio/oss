...
- The application foundation and Rust workspace member are present. Keep the closed internal registry, provider-owned theme/navigation state, desktop/mobile empty-state shells, scoped runtime information, and package-local deterministic checks while adding product, mobile/widget, CI, packaging, release, publisher, or support work through synchronized contracts and validation. The frontend may persist only `devhud.settings.v1` and `devhud.widget-configuration.v1` through record-specific native commands; do not grant filesystem, default-store, or arbitrary-key access. Exclude those records from iOS device backups and disable Android cloud backup and device transfer.
