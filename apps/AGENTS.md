...
- The application, production desktop shell, and Rust workspace member are present. Keep the closed internal registry, provider-owned theme/navigation state, tray-resident desktop lifecycle, mobile empty-state shell, scoped native boundaries, and package-local deterministic checks while adding product, mobile/widget, CI, release, publisher, or support work through synchronized contracts and validation. The frontend may persist only `devhud.settings.v1` and `devhud.widget-configuration.v1` through record-specific native commands; do not grant filesystem, default-store, or arbitrary-key access.
...
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, and host-appropriate repeated lifecycle smoke tests. CI runs the frontend typecheck, lint, unit, accessibility, and build commands when DevHud inputs change. Add the full desktop matrix, mobile/widget, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
