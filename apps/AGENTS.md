...
- The foundation's package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, contract/pin checks, lockfile checks, Rust checks, a debug desktop build, host-appropriate smoke startup, mobile generation/builds, build-only native widget builds/tests, and fail-closed release artifact inspection. CI runs the frontend and portable mobile commands when DevHud inputs change and host-native Android/iOS widget jobs when widget inputs change. Add full desktop-matrix, packaging, updater, signature, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
