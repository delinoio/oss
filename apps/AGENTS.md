...
- Crash submission is opt-in and user-previewed; its build/code strings obey the 256-byte identifier bound and its typed redacted summary and stack obey the 4 KiB/32 KiB bounds. Administrator surfaces consume only metadata-only admin-safe upload views and must never display or log settings bodies, secrets, DOM, screenshots, public or signed asset locators, Deck results, agent output, or local paths.
