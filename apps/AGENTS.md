...
- The pinned upstream `tauri-runtime-cef` also fails to compile on macOS because the Linux/BSD-only `TerminationSignals` struct has an unconditional implementation that references the Linux/BSD-only `libc` dependency. Do not add a downstream dependency shim or patch upstream; this failed x64/ARM64 gate stops before any runtime or packaging claim.
