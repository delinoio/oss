...
- `apps/devhud` is DevHud's sole client/native implementation path. DevHud feature servers/protos belong only under the four paths in `docs/project-devhud.md`; never move client/native code into DeliDev/delibase. DeliDev integration is limited to the documented Logto identity, existing account/organization settings connection management, and delibase-backed feature billing.
