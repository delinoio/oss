...
- Keep checkout, card collection, invoices, receipts, cancellation, and recovery on credential-free HTTPS `polar.sh` pages; never issue browser `UsageService` mutations or calculate charges. Subscription-checkout retries must retain an account-and-organization-scoped pending key in route-surviving memory until hosted navigation succeeds, while billing mutations remain disabled offline.
