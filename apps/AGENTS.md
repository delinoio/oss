...
- The application, production desktop shell, native mobile hosts, and Rust workspace member are present. Keep the closed internal registry, provider-owned theme/navigation state, tray-resident desktop lifecycle, mobile empty-state shell, scoped runtime information, and package-local deterministic checks while adding product, native-widget, CI, release, publisher, or support work through synchronized contracts and validation. The frontend may persist only `devhud.settings.v1` and `devhud.widget-configuration.v1` through record-specific native commands; do not grant filesystem, default-store, or arbitrary-key access. Exclude those records from iOS device backups and disable Android cloud backup and device transfer.
...
- Preserve the active DevHud identifiers `dev.deli.devhud`, `devhud.settings.v1`, and `devhud.widget-configuration.v1`. Do not add an App Group entitlement or native widget identifier until a later contract explicitly authorizes native widgets.
- Production tools and user-visible widgets remain empty in `0.1.0`. Distributed mobile hosts must contain no WidgetKit extension or Android AppWidgetProvider source, target, embedding, or registration, and no CLI, backend, public API, plugin SDK, deep link, telemetry, account system, or DeliDev integration is authorized.
...
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, host-appropriate repeated lifecycle smoke tests, mobile generation/builds, and portable mobile contract/tests. CI runs the frontend and portable mobile commands when DevHud inputs change. Add the full desktop matrix, native platform, native-widget, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
