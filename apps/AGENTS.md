...
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, `check:security`, `test:security`, `check:diagnostics`, `test:diagnostics`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, host-appropriate repeated lifecycle smoke tests, non-gating performance instrumentation, mobile generation/builds, portable mobile contract/tests, build-only native widget builds/tests, and fail-closed release artifact inspection. CI runs the frontend, generated-bundle security, portable mobile, and performance commands when DevHud inputs change and host-native Android/iOS widget jobs when widget inputs change. Add the full desktop matrix, packaging, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
- Performance evidence must remain redacted and schema-validated. Its stable schema and runtime validator must agree on mobile target-kind identity and method bindings, aggregation must derive a target status only after combining all evidence for that identity, and performance-build output must not be buffered into published evidence. Require a clean Git worktree for build identity, and profile only a release-profile binary built with the isolated instrumentation feature and local namespace; normal releases must omit that feature.
