...
- RealQA uses only injected native macOS, Windows, and Ubuntu X11 still-image adapters. Preserve signed multi-display geometry and per-display scaling, re-read topology around capture, select displays by the current pointer, keep selection/toolbar dialogs keyboard accessible with Space mode switching and Escape cancellation, and never invoke a recording API. Mobile frontend builds, including the direct x64 iOS simulator path, exclude the desktop-only RealQA annotation font. Floating confirmation previews the first newly captured image, editor text is capped at 2,048 Unicode scalar values before native submission, and the editor disables removal of the final active image. Auto-saved draft payloads and manifests use authenticated encryption with a non-exported platform-secure key; a successful save renews the 30-day deadline, the default 10 GiB quota never evicts an unexpired draft, and logout is an authoritative purge boundary. Flatten only to metadata-minimized sRGB PNG, capped at 10 images and 50 MiB each with recoverable downscaling, and never log image bytes, draft keys, editor text, or secrets.
- Direct capture shortcuts must finish acquisition before restoring a hidden or minimized window for their visible confirmation. Recheck cancellation at the atomic draft create/append commit boundary, retain successful deletion when a follow-up list refresh fails, validate coordinate-entered annotations inline against the active image, and close the command palette before dispatching a capture shortcut.
- Keep capture-picker geometry compatible with the production no-inline-style CSP. Treat fully opaque black window frames as protected content only on macOS and Windows, without rejecting legitimate opaque-black display or X11 captures. Reclaim image payloads no longer referenced by current, undo, or redo state before quota-checked editor mutations.
- Keep annotation font metrics in static CSP-compatible styles, crop accumulated preview state before blur filtering to match native edge sampling, and reset editor activity during React Strict Mode effect setup.
