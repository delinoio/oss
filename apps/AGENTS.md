...
- Keep the signed-out base shell usable and bundled-only. Use Logto Authorization Code with PKCE only: one-shot random desktop loopback and exact verified Deck mobile callback `https://deli.dev/auth/devhud/callback`; the DeliDev static artifact owns that callback route and its exact-path Apple/Android association files. Vault only refresh token/device-session key, keep access/ID/forwarded tokens in memory, and allow one active account/device.
- Network allowlists are exact and feature-specific: signed GitHub Releases updater, Logto/DeliDev auth, `https://deck.deli.dev`, `https://realqa.deli.dev`, `https://assets.realqa.deli.dev`, the verified mobile callback, and exact-origin Chrome Native Messaging. Bundled webviews do not fetch those origins or receive `http://tauri.localhost` CORS access. Generated Deck/RealQA clients use the private native Connect transport's closed procedure/origin table, and RealQA upload uses its separate validated signed-PUT operation; neither accepts arbitrary URLs, methods, headers, redirects, or generic requests. Provider GitHub/delibase/R2 calls remain server-side. Never add wildcard origins, arbitrary remote resources/UI, client GitHub tokens, remote configuration, or telemetry.
...
- Deck's only offline PR-data exception is a minimal encrypted widget snapshot; ordinary Deck shows offline state. Logout/reset first unregister server push delivery; offline/ambiguous cleanup disables local/platform delivery and retains only a credential-free tombstone retried before later registration. RealQA offline drafts require prior login/device binding, remain encrypted/account-bound, lock on logout, and require online reauthentication to upload. Preserve the exact reset/disconnect/feature-deletion distinctions in the app and server contracts.
