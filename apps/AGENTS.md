...
- GitHub.com is the only v1 issue/pull-request provider. Its PAT profiles require explicit per-repository selection with no fallback, and repository-less Decks cannot retain a profile reference. PAT values stay in platform secure storage while only stable non-secret IDs/descriptors and pending-removal tombstones synchronize. Profile removal must commit its descriptor deletion and tombstone before deleting the device-local PAT, retain failed cleanup for retry, and clear the tombstone only after secure deletion succeeds. GitHub REST traffic goes directly to `api.github.com`, never through `devhud-api`, and validation/errors must preserve the documented permission/restriction/rate/network distinctions without logging credentials, headers, URL queries/fragments, bodies, or private response data.
