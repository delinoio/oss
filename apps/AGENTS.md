...
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, `check:diagnostics`, `test:diagnostics`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, host-appropriate repeated lifecycle smoke tests, mobile generation/builds, portable mobile contract/tests, build-only native widget builds/tests, and fail-closed release artifact inspection. CI runs the frontend and portable mobile commands when DevHud inputs change and host-native Android/iOS widget jobs when widget inputs change. Add the full desktop matrix, packaging, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
