...
- The application, production desktop shell, native mobile hosts, and private Rust mobile-widget bridge are Cargo workspace members. Keep the closed internal registry, provider-owned theme/navigation state, tray-resident desktop lifecycle, mobile empty-state shell, scoped runtime information, and package-local deterministic checks while adding product, widget distribution, CI, packaging, release, publisher, or support work through synchronized contracts and validation. The frontend may persist only `devhud.settings.v1` and `devhud.widget-configuration.v1` through record-specific native commands; widget operations may cross only the private standard Tauri Swift/Kotlin plugin boundary, never a public plugin or generic key/value API. Exclude iOS local/App Group state from device backups and disable Android cloud backup and device transfer.
...
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, host-appropriate repeated lifecycle smoke tests, mobile generation/builds, portable mobile contract/tests, build-only native widget builds/tests, and fail-closed release artifact inspection. CI runs the frontend and portable mobile commands when DevHud inputs change and host-native Android/iOS widget jobs when widget inputs change. Add the full desktop matrix, packaging, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
