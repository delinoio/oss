...
- Network allowlists are exact and feature-specific: signed GitHub Releases updater, Logto/DeliDev auth, `https://deck.deli.dev`, `https://realqa.deli.dev`, `https://assets.realqa.deli.dev`, the verified mobile callback, and exact-origin Chrome Native Messaging. Provider GitHub/delibase/R2 calls remain server-side. Never add wildcard origins, arbitrary remote resources/UI, client GitHub tokens, remote configuration, or telemetry.
- Keep capabilities split by exact window/platform/command. All webviews including DevTools retain bundled navigation/UI, popup/download denial, and no generic filesystem/process/environment/screen/store/network plugin authority. Feature windows receive only narrow commands/origin access from their contracts.
