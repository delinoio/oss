...
- Preserve application identifier `dev.deli.devhud`, build-only widget identifier `dev.deli.devhud.widget`, future App Group `group.dev.deli.devhud`, and storage keys `devhud.settings.v1` and `devhud.widget-configuration.v1`.
- Current production tools, widget configuration, and user-visible widgets remain empty. Deck may add authenticated desktop/mobile/tray/shortcut/notification/native-widget surfaces; RealQA may add authenticated desktop-only capture/editor/encrypted drafts and an exact-origin signed Chrome MV3 native host. Distribution/publication requires a later explicit change. No CLI, public API, public plugin/tracker SDK, arbitrary deep link, arbitrary remote tool/UI, runtime code download, or remote client/extension telemetry is authorized.
- Keep the signed-out base shell usable and bundled-only. Use Logto Authorization Code with PKCE only: one-shot random desktop loopback and exact verified Deck mobile callback `https://deli.dev/auth/devhud/callback`; the DeliDev static artifact owns that callback route and its exact-path Apple/Android association files. Vault only the refresh token/device-session key and an active or cleanup-pending Deck registration's single-purpose `UnregisterDevice` revocation grant, keep access/ID/forwarded tokens in memory, and allow one active account/device.
- Network allowlists are exact and feature-specific: signed GitHub Releases updater, Logto/DeliDev auth, `https://deck.deli.dev`, `https://realqa.deli.dev`, `https://assets.realqa.deli.dev`, the verified mobile callback, and exact-origin Chrome Native Messaging. Bundled webviews do not fetch those origins or receive `http://tauri.localhost` CORS access. Generated Deck/RealQA clients use the private native Connect transport's closed procedure/origin table; the sole signed-out RPC is native `DeckDeviceService.UnregisterDevice` cleanup using its OS-vault single-registration grant; and RealQA upload uses its separate validated signed-PUT operation. None accepts arbitrary URLs, methods, headers, redirects, or generic requests. Provider GitHub/delibase/R2 calls remain server-side. Never add wildcard origins, arbitrary remote resources/UI, client GitHub tokens, remote configuration, or telemetry.
- Keep capabilities split by exact window/platform/command. All webviews including DevTools retain bundled navigation/UI, popup/download denial, and no generic filesystem/process/environment/screen/store/network plugin authority. Authenticated feature windows receive only the native Connect procedures and exact RealQA upload operation granted by their contracts.
- Deck's only offline PR-data exception is a minimal encrypted widget snapshot; ordinary Deck shows offline state. Opaque notification events resolve only for the online authenticated matching registration after current authorization/detail opt-in checks; all other cases use generic text without provider refresh. Logout/reset first unregister server push delivery; offline/ambiguous cleanup disables local/platform delivery and retains only the registration ID, lease expiry, and single-purpose `UnregisterDevice` revocation grant in an OS-vault tombstone retried before later registration even after an account switch. RealQA offline drafts require prior login/device binding, remain encrypted/account-bound, lock on logout, and require online reauthentication to upload; after successful local-draft removal, authenticated paginated `ListSubmissions` exposes only caller-authorized retained submission/asset references needed for later deletion. Preserve the exact reset/disconnect/feature-deletion distinctions in the app and server contracts.
- Deck and RealQA use separate GitHub Apps, GitHub.com only, and disabled production-facing catalog records. Do not claim or perform DNS/deployment, production App/extension registration, R2 provisioning, catalog activation, widget/extension/store publication, or rollout in issue #755/#757 work.
...
- The planned Deck controls in `/account` and `/o/:orgSlug/settings` may consume `@delinoio/devhud-deck-connect` only for `DeckIntegrationService`. Keep the Deck-audience token and forwarded delibase-audience token memory-only and network-only, and do not add Deck view, pull-request, mutation, device, notification, widget, or top-level route UI to DeliDev.
