...
- Production tools, production widget configuration, and user-visible widgets remain empty in `0.1.0`. Compile and test the package-local WidgetKit and AppWidgetProvider foundations, but never add the iOS extension to the distributed app's target dependencies, embedded extensions, or provisioning payload, and never add the Android provider module or receiver metadata to the distributed app. No CLI, backend, public API, plugin SDK, deep link, telemetry, account system, remote tool, or DeliDev integration is authorized.
...
- Keep capabilities split into exact desktop HUD, desktop settings, and mobile-main manifests with explicit application-command permissions only. All webviews, including desktop preview DevTools, must retain bundled-only navigation/resources, popup/download denial, no generic filesystem/process/environment/network plugin authority, and no remote capability origin. Desktop CEF networking remains deny-by-default until the separately scoped GitHub Releases updater is implemented.
- Package-local tasks cover deterministic frontend build/rebuild, typecheck, lint, unit tests, `test:a11y`, `check:security`, `test:security`, `check:diagnostics`, `test:diagnostics`, contract/pin checks, lockfile checks, Rust checks, debug and sign-ready preview desktop builds, host-appropriate repeated lifecycle smoke tests, mobile generation/builds, portable mobile contract/tests, build-only native widget builds/tests, and fail-closed release artifact inspection. CI runs the frontend, generated-bundle security, and portable mobile commands when DevHud inputs change and host-native Android/iOS widget jobs when widget inputs change. Add the full desktop matrix, packaging, updater implementation, signature validation, release, SBOM, provenance, and measurement tasks with their corresponding implementations; do not represent missing work with passing placeholders.
...
- If `apps/devhud` changes after implementation begins, run its documented typecheck, lint, unit/accessibility, desktop/mobile, and release-validation tasks; frontend changes also require the repository frontend test baseline.
