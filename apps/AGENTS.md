...
- Crash submission is opt-in and user-previewed; its typed redacted summary and stack obey the 4 KiB/32 KiB bounds. Administrator surfaces consume only admin-safe views and must never display or log settings bodies, secrets, DOM, screenshots, Deck results, agent output, or local paths.
