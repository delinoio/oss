...
- Desktop shortcut configuration uses only the documented six closed action enums and structured modifier/key values. `right-primary` is physical right Command on macOS and physical right Control on Windows/X11; never collect, persist, bridge, or diagnose unrelated/raw keyboard events. Native failures must preserve the last valid settings binding, and localized macOS Accessibility/Input Monitoring plus Linux X11/XWayland guidance must remain available.
