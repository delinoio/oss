...
- Diagnostics is an implemented local-first boundary: reporting is off by default, guests can preview/export only, authenticated unblocked users must consent to each exact payload, blocked/deletion-pending users are denied, and offline troubleshooting remains available. Preserve the recursive exclusions, UUID v7 correlations, physically enforced bounded local retention, path-free export result, explicit browser and native runtime revision rules, scoped destructive cleanup, typed safe submission failures, and no-analytics/no-remote-flag rules in `docs/apps-devhud-foundation.md`.
