...
- The implemented administrator SPA uses the bootstrap `admin` public client and exact redirect, delegates Authorization Code S256 PKCE and state validation to Logto, verifies an explicit OIDC nonce, and treats client permission guards as presentation only because every Admin RPC enforces the exact role server-side.
