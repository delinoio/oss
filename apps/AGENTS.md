...
- Background-usage authorization management stays inside the existing `/account` and `/o/:orgSlug/settings` billing/usage sections with no new top-level route. Human clients may create/list/get/revoke visible grants through `BillingService`, keeping grant and summarized usage state memory-only/network-only; they must never call the M2M-only authorized usage RPCs. Organization Owners/Admins receive organization-wide visibility, Members only grants they created, and RealQA may show only its own grant/payer/current-daily-unit/failure/rebind/revoke state. Deck has no v1 background purpose.
