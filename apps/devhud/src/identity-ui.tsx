...
export function SynchronizedSettingsBoundary({ copy }: { readonly copy: Copy }) {
  ...
  return <>
    ...
    {identity.conflict && <SnapshotChoice key="conflict" choiceId="conflict" copy={copy} entries={identity.conflict.diff} title={copy.conflictTitle} summary={copy.conflictSummary} primary={copy.reapplyLocal} secondary={copy.adoptServer} onPrimary={() => invoke(async () => { await identity.reapplyConflictLocal(); setUrlMappingReset((current) => current + 1); })} onSecondary={() => { identity.adoptConflictServer(); setUrlMappingReset((current) => current + 1); }} />}
