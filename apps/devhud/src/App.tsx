import { useCallback, useEffect, useMemo, useRef, useState, type ComponentType, type ReactNode, type Ref, type RefObject } from "react";
...
import { AccountIdentity, FirstRunIdentity, SynchronizedAppearanceBoundary, SynchronizedSettingsBoundary, UrlMappingDraftProvider, type SettingsSectionContributions } from "./identity-ui";
...
export interface AppProps {
  ...
  readonly desktopSettings?: {
    readonly ShortcutBoundary: ComponentType<{ readonly bridge?: NativeBridgeV1 }>;
    readonly ShortcutPaletteTrigger: ComponentType<{ readonly copy: Copy; readonly isMac: boolean; readonly onOpen: () => void; readonly triggerRef: Ref<HTMLButtonElement>; readonly compact?: boolean }>;
    readonly sections: SettingsSectionContributions;
  };
}
...
export function App({ bridge = nativeBridge, initialRuntime, initialContentState = defaultContentState, nativeMessaging, desktopSettings }: AppProps) {
  ...
  const DesktopShortcutBoundary = desktopSettings?.ShortcutBoundary;
  const DesktopShortcutPaletteTrigger = desktopSettings?.ShortcutPaletteTrigger;
  ...
  const navigation = shellLayout !== ShellLayout.Mobile && <aside className={`shell-navigation shell-navigation-${shellLayout}`}>
    ...
    {mobile || !DesktopShortcutPaletteTrigger ? <Button className="palette-trigger" ref={paletteTrigger} variant="ghost" icon={<SearchIcon />} onClick={openPalette} aria-label={copy.openPalette}>{shellLayout === ShellLayout.Sidebar ? copy.openPalette : null}</Button> : <DesktopShortcutPaletteTrigger copy={copy} isMac={isMac} triggerRef={paletteTrigger} onOpen={openPalette} compact={shellLayout === ShellLayout.Rail} />}
  </aside>;
  ...
  return boundary(<>
    {runtime?.platform === RuntimePlatform.Desktop && DesktopShortcutBoundary && <DesktopShortcutBoundary bridge={bridge} />}
    <AppShell layout={shellLayout} skipLabel={copy.skipToContent} navigation={navigation} topBar={topBar} bottomBar={bottomBar} data-devhud-ready="true" data-runtime-platform={runtime?.platform ?? "desktop"} data-lifecycle={lifecycle}>
      ...
      {surface === SurfaceId.Settings && <SynchronizedSettingsBoundary copy={copy} bridge={bridge} language={language} mobile={mobile} onOpenExternal={openExternal} onModalConfirmationOpenChange={setScreenModalConfirmationOpen} onUpdaterApprovalOpenChange={handleUpdaterApprovalOpenChange} shortcutCapabilities={runtimeCapabilities} sectionContributions={runtime?.platform === RuntimePlatform.Desktop ? desktopSettings?.sections : undefined} NativeMessagingSettings={runtime?.platform === RuntimePlatform.Desktop ? nativeMessaging?.Settings : undefined} localAgentsAvailable={runtime?.platform === RuntimePlatform.Desktop && runtime.capabilities.localAgents === true} notification={supportsNotifications ? { permission: copy[notificationPermissionLabels[notificationPermission]], failed: notificationRequestFailed, onRequest: () => void requestNotifications() } : undefined} storeUpdates={mobile && runtime?.capabilities.storeUpdates ? { configured: storeConfigured, failed: storeOpenFailed, onOpen: () => void openStore() } : undefined} />}
