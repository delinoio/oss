...
export function App({ bridge = nativeBridge, initialRuntime, initialContentState = defaultContentState }: AppProps) {
  ...
  const boundary = (content: ReactNode) => runtime ? <DevHudServiceBoundary key={preferences.apiOrigin} apiOrigin={preferences.apiOrigin} active={onboarding || surface === SurfaceId.Account || surface === SurfaceId.Settings || authCallback !== null} online={online} callbackUrl={authCallback} platform={runtime.platform} bridge={bridge} onContinueLocally={finishOnboarding} onLoggedOut={() => setSurface(SurfaceId.Account)} initialAppearance={{ theme: preferences.theme, language: preferences.language }} identitySessionRef={identitySession}>{content}</DevHudServiceBoundary> : content;
