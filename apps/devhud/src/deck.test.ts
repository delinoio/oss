...
describe("Deck query and local transitions", () => {
  ...
  it("accepts legacy caches without pending notifications and rejects malformed pending notifications", () => {
    ...
    expect(readDeckCache({ getItem: () => JSON.stringify(cache) }, "origin", "deck", "repo:octo/widgets is:pr")?.failure).toBeNull();
    expect(readDeckCache({ getItem: () => JSON.stringify({ ...cache, failures: 1 }) }, "origin", "deck", "repo:octo/widgets is:pr")?.failure).toBe("unknown");
    expect(readDeckCache({ getItem: () => JSON.stringify({ ...cache, failures: 1, failure: "rate-limit" }) }, "origin", "deck", "repo:octo/widgets is:pr")?.failure).toBe("rate-limit");
    ...
    expect(readDeckCache({ getItem: () => JSON.stringify({ ...cache, failure: "invalid" }) }, "origin", "deck", "repo:octo/widgets is:pr")).toBeNull();
