...
export function Sheet({ open, title, backLabel, inert, initialFocusRef, returnFocusRef, restoreFocus = true, onClose, children }: { readonly open: boolean; readonly title: ReactNode; readonly backLabel: string; readonly inert?: boolean; readonly initialFocusRef?: RefObject<HTMLElement | null>; readonly returnFocusRef?: RefObject<HTMLElement | null>; readonly restoreFocus?: boolean; readonly onClose: () => void; readonly children: ReactNode }) {
  const titleId = useId();
  return <ModalSurface open={open} title={title} titleId={titleId} className="ui-sheet" inert={inert} initialFocusRef={initialFocusRef} returnFocusRef={returnFocusRef} restoreFocus={restoreFocus} onClose={onClose}><div className="sheet-content">{children}</div><Button variant="ghost" onClick={onClose}>{backLabel}</Button></ModalSurface>;
}
