...
function WidgetSnapshotSynchronization({ deck, enabled, state, synchronize }: { readonly deck: Deck; readonly enabled: boolean; readonly state: DeckRefreshState; readonly synchronize: (deck: Deck, snapshot?: WidgetDeckSnapshot) => Promise<void> }) {
  useEffect(() => {
    if (!enabled || state.cache === null || state.cache.query !== deck.query || state.cacheProfileRef !== deck.profileRef) return;
    void synchronize(deck, widgetSnapshot(deck, state.cache, state.failure)).catch(() => undefined);
  }, [deck, enabled, state.cache, state.cacheProfileRef, state.failure, synchronize]);
  return null;
}
...
export function DeckPollingBoundary({ bridge, active, online, language = "en", provider: suppliedProvider, children }: DeckPollingBoundaryProps) {
  ...
  const refresh = useCallback(async (deckId: string, manual = false) => {
    ...
    try {
      ...
      const hydratedCache = configurations.current.get(deckId)?.profileRef === currentDeck.profileRef ? caches.current.get(deckId) : null;
      ...
      if (search.incompleteResults) {
        ...
        setDeckState(deckId, (current) => ({ ...current, cache: next, cacheProfileRef: currentDeck.profileRef, failure: "incomplete-results" }));
        return;
      }
      ...
      if (isCurrentDeck()) {
        caches.current.set(deckId, next);
        setDeckState(deckId, (current) => ({ ...current, cache: next, cacheProfileRef: currentDeck.profileRef, failure: null }));
      }
    } catch (error) {
      ...
      caches.current.set(deckId, next);
      setDeckState(deckId, (current) => ({ ...current, cache: next, cacheProfileRef: currentDeck.profileRef, failure }));
    } finally {
  ...
  useEffect(() => {
    ...
    void identity.githubPatScopeId.then((scopeId) => {
      ...
      for (const deck of scheduledDecks) {
        ...
        setDeckState(deck.id, (current) => ({ ...current, cache, cacheProfileRef: deck.profileRef, failure: cache?.failure ?? (cache !== null && cache.failures > 0 ? "unknown" : null) }));
      }
    }).catch(() => {});
    return () => { cancelled = true; };
...
export function DeckSurface({ copy, selectedDeckId = null, onDismissMissingLink, onModalConfirmationOpenChange }: DeckSurfaceProps) {
  ...
  return <section className="deck" aria-label={copy.deckTitle}>
    <PageHeader title={copy.deckTitle} summary={copy.deckSummary} actions={<div className="deck-workspace-controls">
      ...
      {mobile && deck && <Button onClick={openSettings}>{copy.deckSettings}</Button>}
    </div>} />
    <div className="deck-workspace-layout">
      <div className="deck-workspace">
        {deck ? <><DeckRefreshStatus copy={copy} state={refreshState} canPoll={polling.canPoll} online={polling.online} />
          {!polling.online && refreshState.cache === null ? <OfflineState copy={copy} /> : refreshState.loading && refreshState.cache === null ? <LoadingState copy={copy} /> : refreshState.failure !== null && refreshState.cache === null ? <StatePanel eyebrow={copy.error} title={failureCopy(copy, refreshState.failure)} summary={copy.deckNoCachedResults} role="alert" tone="danger" headingLevel={3} /> : <DeckResults copy={copy} groupBy={deck.display.groupBy} results={results} draftsFiltered={draftsFiltered} successfulEmpty={refreshState.cache !== null && refreshState.cache.lastSuccessfulAt !== null && refreshState.failure === null && cachedResults.length === 0} />}
