...
import { localAgentExecutablePath, readAgentConsents, readExecutablePaths, writeAgentConsents, writeExecutablePaths, type AgentConsents, type ExecutablePaths } from "./local-agent-preferences.ts";
...
export function LocalAgentSettings({ copy, bridge }: { readonly copy: Copy; readonly bridge: NativeBridgeV1 }) {
  ...
  return <section className="native-setting" aria-label={copy.localAgentsTitle}>
    <h3 tabIndex={-1}>{copy.localAgentsTitle}</h3>
