...
`cef-pins.json` also binds each architecture-specific AppImage Sharun helper bundle, including its launcher, to its SHA-256 digest. The desktop wrapper verifies the selected asset before making its bytes available to AppImage packaging.
...
AAB artifact verification requires `--bundletool-jar` pointing to the exact checksum-pinned inspector declared in `mobile-platforms.json`; it validates the final merged base manifest's exact non-exported Deck widget receiver, update action, and provider metadata before release evidence is recorded.
