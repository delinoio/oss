...
Development binds only `127.0.0.1:46305` and fails if the port is occupied. Platform smoke requires a native production artifact and certifies only macOS 13+, Windows 10 22H2+, or Ubuntu 22.04+ X11 on the matching architecture. XWayland is best effort; native Wayland is unsupported.
