...
The unpublished RealQA Chrome bridge is the only extension exception. It captures only the active visible viewport, requests exact-origin optional access only for user-activated DOM selection, and falls back to the existing OS capture path on restricted pages. Its Rust `devhud-native-host` binary revalidates the exact caller origin, closed metadata schema, redaction, image/message limits, and local pairing before queueing input for the composer. Reset clears pairing and queued host input; Chrome-owned site permissions must be reviewed separately in `chrome://extensions`.
