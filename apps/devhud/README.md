...
The isolated workflow attempts this gate natively on macOS 14+ x64 and ARM64 runners. At the pinned revision both builds stop inside upstream `tauri-runtime-cef`: a Linux/BSD-gated signal type has an unconditional implementation that references the Linux/BSD-only `libc` dependency. Fixing this requires an upstream source change, so no runtime, DMG, updater, signing, or process evidence is claimed. The pinned revision also blocks fatal renderer-termination observation on Windows and Linux; see `docs/apps-devhud-foundation.md`.
