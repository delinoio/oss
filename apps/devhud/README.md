...
No production tool, visible widget, updater network implementation, public release automation, public API, CLI, or arbitrary deep link is included. See `docs/apps-devhud-foundation.md` for the complete contract.

The unpublished RealQA Chrome bridge is the only extension exception. It captures only the active visible viewport, binds user-activated DOM selection to the captured tab/window/exact URL, requests optional access only for that HTTP(S) origin, retains the active capture in the service worker while Chrome closes the popup for page selection, preserves the selected target metadata in the composer, and offers a user-confirmed pointer-free primary-display fallback with no default URL on restricted pages. Its Rust `devhud-native-host` binary revalidates the exact caller origin, closed metadata schema, redaction, image/message limits, and local pairing. After a prior RealQA device binding, it hands captures over private local IPC directly to the exact `realqa-composer` window's bounded in-memory queue; capture pixels and page metadata are never persisted in native-host state. Logout and reset clear queued captures and all accepted composer sources; reset also clears pairing. Chrome-owned site permissions must be reviewed separately in `chrome://extensions`.
