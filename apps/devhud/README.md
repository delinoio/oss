...
DevHud is a bundled-shell Tauri application with one shared English React frontend. Its signed-out shell and diagnostics remain local. RealQA is the first closed internal production tool and is registered only on macOS, Windows, and Ubuntu; iOS and Android retain the signed-out empty mobile shell and expose no RealQA entry. RealQA entry requires the native Logto account boundary, while a prior-bound offline account may capture, edit, and retain encrypted local drafts until online reauthentication enables its closed protobuf transport and signed image upload. Desktop builds retain the exact pinned upstream CEF runtime and sandbox. Mobile builds use only Tauri's standard WKWebView host on iOS and System WebView host on Android; CEF is not a mobile dependency or feature.
