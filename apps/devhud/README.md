...
Package-local WidgetKit and Android `AppWidgetProvider` sources compile and test independently and are also included in the production mobile hosts. The iOS app embeds the separately identified `dev.deli.devhud.widget` extension through App Group `group.dev.deli.devhud`; Android registers exactly one non-exported Deck widget receiver plus the system-launched per-instance configuration activity. Each instance selects one of at most 20 Deck configurations without silently falling back to a different view and renders WidgetKit small/medium/large on iPhone and through macOS's iPhone-widget availability, or responsive Android compact/wide/list layouts. New widgets default to counts only, while repository and PR titles require explicit per-widget privacy opt-in. The only mobile links are the auto-verified exact `https://deli.dev/auth/devhud/callback` authentication boundary and exact `https://deli.dev/devhud/deck/open` view/PR/refresh/opaque-event action boundary. Neither grants arbitrary deep linking, remote UI, telemetry, or PR mutation authority. RealQA remains desktop-only.
