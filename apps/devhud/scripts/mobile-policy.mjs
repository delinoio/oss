...
export function assertAndroidNativeBridge(androidNativeBridge) {
  ...
  assert(cleanupPendingDiagnosticsExport.includes("if (!hasPersistedDiagnosticsWriteGrant(destination)) return forgetDiagnosticsCleanup()") && cleanupPendingDiagnosticsExport.indexOf("hasPersistedDiagnosticsWriteGrant(destination)") < cleanupPendingDiagnosticsExport.indexOf("contentResolver.delete"), "Android diagnostics cleanup must clear stale markers without inspecting destinations whose grants were already released");
