...
export function assertAndroidNativeBridge(androidNativeBridgeInput) {
  ...
  assert((androidNativeBridge.match(/\.commit\(\)/gu) ?? []).length === 10, "Android secure-setting and diagnostics-cleanup writes must confirm persistence");
...
export function assertIosNativeBridge(iosNativeBridgeInput) {
  ...
  assert(reconcileWidgetCredentialReplacement.includes("readDataMigratingLegacy(setting)") && reconcileWidgetCredentialReplacement.includes("replaceWidgetCredentials(transaction, data:"), "iOS must reconcile interrupted widget replacements from the authoritative main PAT");
