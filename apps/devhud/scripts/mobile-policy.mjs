...
export function assertIosNativeBridge(iosNativeBridgeInput) {
  ...
  assert(replaceWidgetSnapshot.includes("mergeWidgetSnapshot(current: previousSnapshot, incoming: snapshot)") && replaceWidgetSnapshot.includes("widgetForegroundReloadDeadlineKey") && replaceWidgetSnapshot.indexOf("defaults.synchronize()") < replaceWidgetSnapshot.indexOf("reloadAllTimelines()"), "iOS foreground widget snapshots must merge monotonically and persist a stored-only reload marker before reloading WidgetKit");
