...
test("mobile policy requires lifecycle-owned Android persistence and native platform safeguards", () => {
  ...
  assert.throws(() => assertAndroidNativeBridge(androidNativeBridge.replace("diagnosticsExportPickerActive = true", "diagnosticsExportPickerActive = false")), /record the active picker/u);
  ...
  assert.throws(() => assertAndroidNativeBridge(androidNativeBridge.replace("if (!hasPersistedDiagnosticsWriteGrant(destination)) return forgetDiagnosticsCleanup()", "if (false) return forgetDiagnosticsCleanup()")), /without inspecting destinations/u);
