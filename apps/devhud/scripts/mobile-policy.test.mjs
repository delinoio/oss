...
test("widget targets preserve secure isolation, bilingual privacy warnings, and bounded previews", () => {
  ...
  assert.equal((androidStore.match(/synchronized\(widgetStoreMutationLock\)/gu) ?? []).length, 6);
  ...
  assert.match(androidStore, /fun credential\(deckId: String\)[\s\S]*state\.contains\(transactionPrefix \+ deckId\)[\s\S]*WidgetCredential\(revision\?\.let \{ decrypt\(it, deckId\) \}, revision\)/u);
  ...
  assert.match(androidStore, /replaceProfileToken[\s\S]*synchronized\(widgetStoreMutationLock\)[\s\S]*encrypt\(token, deckId\)[\s\S]*editor\.commit\(\)/u);
  ...
  assert.match(androidProvider, /failure\(configuration, previous, "missing-token"[\s\S]*replaceSnapshot\(snapshot, credential\.revision\)[\s\S]*if \(stored\) snapshot else store\.snapshot\(deckId\)/u);
  ...
  assert.match(iosNativeBridge, /replaceWidgetCredentials[\s\S]*let replacement = encodeWidgetCredential\(data\)[\s\S]*storeWidgetCredential\(replacement, deckId: item\.deckId\)/u);
  ...
  assert.match(iosWidget, /func credential\(_ deckId: String\)[\s\S]*guard defaults\?\.bool\(forKey: transactionPrefix \+ deckId\) != true else \{ return nil \}[\s\S]*StoredWidgetCredential[\s\S]*revision: data/u);
