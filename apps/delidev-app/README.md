...
The development server binds to loopback on fixed port `46301`, rejects explicit host and port CLI overrides, and exits if that port is already occupied instead of selecting another port. The production preview and Playwright server continue to use their existing configuration.
