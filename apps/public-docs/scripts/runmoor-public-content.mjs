...
// Preserve the public-content checks that protected these guides while they
// move into the shared public-docs publication. Route exceptions are explicit
// so valid Runmoor public paths cannot broaden the repository-path allowlist.
...
export function createPublicContentValidator(routeIds) {
  ...
  const routePattern = routes.filter((route) => route !== "/")
    ...
    .join("|");
  const htmlRoutes = new Set(routes.map((route) => {
    if (route === "/") return "/index.html";
    // Rspress emits the integrated Runmoor overview at a nested index path.
    if (route === "/runmoor") return "/runmoor/index.html";
    return `${route}.html`;
  }));
  const forbiddenPaths = [
    // Architecture labels in the public support matrix (for example
    // `amd64/arm64`) are prose, not repository or filesystem paths.
    new RegExp(`(?:^|[\\s("'\\x60>])/(?!(?:${routePattern})(?:\\.html)?(?:[?#"'\\x60<\\s]|$)|(?:amd64|arm64)(?=[.,;:!?)]|\\s|$))[A-Za-z0-9._~-]+(?:[/\\\\][^\\s"'\\x60<>]*)?`, "u"),
    /(?:^|[\s("'`>])(?:\.\.[\\/])+(?:[A-Za-z0-9._~-]+[\\/])+[^\s"'`<>]*/u,
