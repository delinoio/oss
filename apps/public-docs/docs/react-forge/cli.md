...
The unreleased [SFX extension](/react-forge/sfx) adds `Format.Wav` tasks with `.wav` output. The unreleased [Sprite extension](/react-forge/sprite) adds `Format.Sprite` tasks with `.sprite.zip` output. Both are absent from npm `0.1.1`; check [release status](/react-forge/releases) before using their imports.
