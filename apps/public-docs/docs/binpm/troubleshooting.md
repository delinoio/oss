...
When `~/.binpm/bin` is not on `PATH`, doctor prints setup guidance that points to `binpm env --global --shell <bash|zsh|fish|powershell|pwsh>`. binpm does not edit profile files from doctor or plain env output. To opt in, preview the exact profile file and global PATH line with `binpm env setup --shell <shell> --dry-run`, then run `binpm env setup --shell <shell>` to append only that global line. Use `--shell pwsh` for PowerShell 7 setup so binpm updates the profile that PowerShell 7 loads. The project-local command is for the current project or shell session only.
