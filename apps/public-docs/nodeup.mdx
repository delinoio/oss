...
- Toolchain lifecycle: `toolchain list|install|uninstall|link|unlink`
...
Register or remove an external linked runtime record:

```bash
nodeup toolchain link work-node /opt/node-v22
nodeup toolchain unlink work-node
```

Linked runtime removal does not delete the external runtime directory. Linked runtime validation requires a runnable `bin/node` on Unix or `bin/node.exe` for Windows platform behavior.
