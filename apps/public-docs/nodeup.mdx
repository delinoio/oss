...
Use the dedicated `NODEUP_BINPM_HOME` directory so binpm's local manifest and `nodeup` binary do not modify whichever project directory you started from. In bash or zsh, replace `<shell>` with `bash` or `zsh`. Apply the printed environment command before running `nodeup`.
