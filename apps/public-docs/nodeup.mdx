...
- `nodeup show color` reports effective human stdout, human stderr, and log color decisions for troubleshooting.
- `NODEUP_LOG_COLOR=always|auto|never` controls log ANSI styling, defaults to colored logs, and honors `NO_COLOR` when unset or set to `auto`.
- Invalid `NODEUP_COLOR` and `NODEUP_LOG_COLOR` values are ignored and reported by `nodeup show color`.
