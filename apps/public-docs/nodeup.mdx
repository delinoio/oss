...
Use the dedicated `NODEUP_BINPM_HOME` directory so binpm's local manifest and `nodeup` binary do not modify whichever project directory you started from. In POSIX shells, replace `<shell>` with `bash`, `zsh`, or `fish`.
