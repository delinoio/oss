...
export function docxBlocks(nodes: SerializedNode[], documentId: string): Model[] {
  return nodes.flatMap(node => {
    ...
    if (node.type === "docx:list") return node.children.map(item => {
      ...
      return { type: "paragraph", id: item.id, style: commonStyle(item.props.style), list: { instance_id: node.id, kind: p.kind ?? "bullet", level: p.level ?? 0 }, runs: runs(item.children) };
    });
