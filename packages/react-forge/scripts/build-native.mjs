...
const child = spawn("cargo", ["build", "--manifest-path", `${root}/Cargo.toml`, "-p", "react-forge-node", "--target-dir", target, "--target", host.target], { stdio: "inherit" });
...
try {
  ...
  await copyFile(`${target}/${host.target}/debug/${host.library}`, new URL(`react-forge.${host.id}.node`, output));
} finally {
