...
For the refined presentation, use `test:scenes --output <directory> --device METAL --samples 256 --hero-resolution 4096`. The other views remain 2048×2048; the studio hero is 4096×2731 and the individual heroes are 4096×4096. Softbox lighting, a seamless backdrop, AgX and a fixed seed are shared by both importers. The web viewer adds only a studio environment, lights and a shadow-catching floor; it does not replace the imported materials. Retain the initial evidence as a historical baseline and record quality revisions separately. Generated comparison sheets and rendered images are evidence, not substitutes for the actual GLB/FBX geometry.
