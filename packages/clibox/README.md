...
`with-rate-limit` requires `--name`, `--limit`, and `--period`; `--burst` defaults to one. A token is consumed immediately before execution and is not refunded. `--wait-timeout` limits admission, with `0` immediate; bucket configuration must match and a backward wall clock adds no tokens. `with-lock` waits by default; `skip` exits 0 and `fail` exits 75 without a workload. `--wait-timeout` is allowed only when waiting, and nested same-key locks contend normally.
