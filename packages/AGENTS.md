...
- Keep tests runnable with Node built-ins, and smoke-test npm/pnpm consumer tarball installs with scripts disabled, including native JSON readiness, environment execution and literal empty argv/exit propagation. Build/package/release tasks are package-owned; native integration and publication tasks are not Turbo-cacheable.
