...
- Installed consumer smoke tests must exercise the seven issue #917 commands as well as help/version across the existing eight target artifacts. Linux/macOS/Windows CI also runs the Rust CLI process tests. Keep command documentation consistent between Cargo and npm without exposing release internals.
