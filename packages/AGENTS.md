...
- Keep tests runnable with Node built-ins, and smoke-test npm/pnpm consumer tarball installs with scripts disabled. Installed smoke tests must exercise dotenv list/merge and YAML normalize including reference resolution, literal value preservation, idempotence, and silent file publication. Run clibox Rust process tests on Linux, macOS, and Windows in the existing CI job. Build/package/release tasks are package-owned; native integration and publication tasks are not Turbo-cacheable.
