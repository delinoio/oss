...
- Treat the external `0.0.1` npm packages only as name reservations. The first source release is `0.1.0` and must publish the complete verified seven-package candidate through the exact-tag OIDC workflow after all seven Trusted Publishers are configured.
