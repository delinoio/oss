...
- Keep distribution tests runnable with Node built-ins, and smoke-test npm/pnpm consumer tarball installs with scripts disabled, including native JSON readiness. CI additionally runs Rust unit/process tests on Linux, macOS, and Windows. Build/package/release tasks are package-owned; native integration and publication tasks are not Turbo-cacheable.
