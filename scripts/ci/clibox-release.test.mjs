...
test("clibox release covers all eight native targets and Alpine consumer execution", () => {
  ...
  assert.equal(steps.flatMap(({ run }) => run?.split("\n") ?? []).filter((line) => line.trim() === 'cargo test --locked -p clibox -p clibox-system -p clibox-transform -p clibox-wait --target "$CLIBOX_TARGET"').length, 1);
  ...
  assert.ok(build);
  assert.match(build.run, /cargo test --locked -p clibox -p clibox-system -p clibox-transform -p clibox-wait --target/u);
...
test("clibox input changes select its aggregated consumer checks and force external Cargo inputs", () => {
  ...
  assert.equal(ci.jobs[id].steps.filter(({ run }) => run === "cargo test --locked -p clibox -p clibox-system -p clibox-transform -p clibox-wait").length, 1);
  ...
  for (const event of [Event.Push, Event.PullRequest]) {
    for (const file of ["packages/clibox/src/launcher.cjs", "crates/clibox/src/main.rs", "crates/clibox-system/src/lib.rs", "crates/clibox-transform/src/lib.rs", "crates/clibox-wait/src/lib.rs", ".github/workflows/release-clibox.yml", "scripts/release/project.mjs"]) {
  ...
  for (const task of ["test", "test:package"]) {
    for (const crate of ["clibox", "clibox-system", "clibox-transform", "clibox-wait"]) {
...
test("native publication follows the independently guarded signed GNU release", () => {
  ...
  for (const file of ["crates/clibox/src/main.rs", "crates/clibox-system/src/lib.rs", "crates/clibox-transform/src/lib.rs", "crates/clibox-wait/src/lib.rs", "packages/clibox/scripts/github-release.mjs"]) {
