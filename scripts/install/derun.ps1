...
function Resolve-LatestTag {
  ...
  if (-not $match) {
    throw "[install.derun] failed to resolve latest tag"
  }
...
function Verify-Checksum {
  ...
  if (-not $expected) {
    throw "[install.derun] checksum entry not found for $AssetName"
  }
  ...
  if ($expectedHash -ne $actualHash) {
    throw "[install.derun] checksum mismatch for $AssetName"
  }
}
...
function Install-WithPackageManager {
  if (Get-Command winget -ErrorAction SilentlyContinue) {
    Write-Host "[install.derun] installing via winget"
    winget install --id DelinoIO.Derun --exact --accept-package-agreements --accept-source-agreements
    return $true
  }

  Write-Warning "[install.derun] winget is unavailable; falling back to direct installation"
  return $false
}
...
function Install-Direct {
  ...
  $assetName = "derun-windows-amd64.zip"

  $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("derun-install-" + [System.Guid]::NewGuid().ToString("N"))
  ...
  try {
    ...
    Copy-Item -Path (Join-Path $extractDir "derun.exe") -Destination (Join-Path $InstallDir "derun.exe") -Force

    Write-Host "[install.derun] installed derun.exe to $InstallDir"
  }
  finally {
