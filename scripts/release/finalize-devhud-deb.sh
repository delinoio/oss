...
pnpm exec clibox text replace '@HOST@' "$host" --input="$script_directory/linux/postinst.in" \
  | pnpm exec clibox text replace '@MANIFEST@' "$manifest" > "$work/package/DEBIAN/postinst"
pnpm exec clibox text replace '@HOST@' "$host" --input="$script_directory/linux/prerm.in" \
  | pnpm exec clibox text replace '@MANIFEST@' "$manifest" \
  | pnpm exec clibox text replace '@RUNTIME_ROOT@' '/run/user' > "$work/package/DEBIAN/prerm"
