...
import { identity, origin, pins, requireValue, architectures } from './model.mjs';
...
const allowedImages = ['ubuntu:22.04', 'ubuntu:24.04', 'ubuntu:26.04', 'debian:12', 'debian:13', 'fedora:43', 'fedora:44', 'registry.access.redhat.com/ubi9/ubi:latest', 'registry.access.redhat.com/ubi10/ubi:latest', 'rockylinux:9', 'rockylinux:10', 'almalinux:9', 'almalinux:10'];
...
try {
  ...
  for (const project of projects) await asyncRun(['run', '--rm', '--platform', `linux/${values.architecture}`, ...(values.fixture ? ['--network', network] : []), '-v', `${root}/scripts/release/linux-packages/install-test.sh:/install-test.sh:ro`, values.image, 'bash', '/install-test.sh', project.project, project.version, project.channel, values.architecture, values.fixture ? 'http://registry:8080' : origin, values.fixture ? 'fixture' : 'live']);
} finally {
