...
- `devhud-store-release.mjs`: polls App Store build processing read-only, idempotently reconciles store submissions, classifies pending/approved/public/withdrawn status, performs protected full publication operations, and discovers, cancels, or verifies the exact release before an automatic infrastructure rollback.
