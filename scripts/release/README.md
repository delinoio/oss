...
Use **Actions → Release Project → Run workflow** on `main`, choose one of `binpm`, `cargo-mono`, `nodeup`, `with-watch`, `derun`, `runmoor`, or `clibox`, and choose `patch`, `minor`, or `major`. The workflow commits the version, validates the exact release source, publishes the selected crate when applicable, and pushes the single release tag. All seven projects follow this sequence without waiting for main CI; main CI runs independently and its result does not gate publication. That tag starts the existing project distribution workflow (including npm for clibox) asynchronously; the coordinator does not wait for or report its completion. Main pushes no longer publish the Rust workspace. Configuration, concurrency, and recovery are defined in `docs/repository-workflow-contract.md`.
