...
Use **Actions → Release Project → Run workflow** on `main`, choose one of `binpm`, `cargo-mono`, `nodeup`, `with-watch`, `derun`, `runmoor`, and choose `patch`, `minor`, or `major`. The workflow commits the version, waits for its exact CI Result, publishes the selected crate when applicable, and waits for the existing signed binary/Homebrew release. Main pushes no longer publish the Rust workspace. Configuration, concurrency, and recovery are defined in `docs/repository-workflow-contract.md`.
...
- `project.mjs`: closed project/version adapters, exact version-only run journal, commit and single-tag pushes, bounded CI/release inspection. `RELEASE_PROJECT=binpm RELEASE_BUMP=patch node scripts/release/project.mjs plan` is a read-only local preview requiring no credentials. Other coordinator commands are manual-main workflow operations.
- Rerun the same coordinator run after an interrupted commit/registry/tag phase to preserve its version. For a failed CI or downstream run, first rerun that exact run from its Actions page, then retry the coordinator. Existing tags must point to the recorded commit. No recovery path force-pushes, republishes all crates, or automatically overwrites public assets.
