...
- `devhud-candidate-artifact.mjs`: discovers the oldest retained complete candidate for an exact version and source revision across recovery dispatches and earlier attempts of the current rerun so signed bytes and provenance identity are preserved.
- `validate-devhud-public-assets.mjs`: validates the exact downloaded GitHub Release inventory, signed checksums and bundles for every public binary and extracted evidence payload, release index, and remotely served updater manifests against the public package bytes.
...
- `devhud-store-release.mjs`: polls App Store build processing read-only, idempotently reconciles store submissions and publication, classifies unsubmitted/pending/approved/public/withdrawn status, and discovers, cancels, or verifies the exact release before an automatic infrastructure rollback.
