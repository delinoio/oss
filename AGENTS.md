...
- The coordinated DevHud release may retain only DevHud-owned public documentation in its release-bound candidate. Its Cloudflare Pages publication must rebuild the complete public-docs aggregate from current `main` and overlay only `/devhud` and its required shared static assets, so delayed or historical recovery cannot overwrite newer package-local documentation subpaths.
