...
- DevHud production capture-picker geometry must remain compatible with the strict no-inline-style CSP. macOS and Windows opaque-black protected window frames return `protected-content` before persistence, while X11 and display captures retain legitimate opaque-black output. Draft recovery reclaims payloads no longer referenced by current, undo, or redo state before quota-checked editor mutations, every revision change removes now-inaccessible flattened derivatives before quota accounting, and native line rasterization preserves requested even and odd stroke widths.
