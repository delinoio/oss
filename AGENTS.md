...
- The production registry contains exactly the closed internal RealQA tool on macOS, Windows, and Ubuntu; iOS and Android must filter it before rendering or dispatch, and widget registration/user-visible widget state remain empty. This client registration does not deploy services, configure DNS/R2, register GitHub Apps, publish images/widgets/extensions/stores, inject production identities, enable catalog entries, or roll out operations.
- Run root `pnpm generate:proto`/`pnpm check:proto`, per-package TypeScript typechecks, and generated Go test/vet for both proto roots now. DevHud implements focused `test:deck` and `test:deck:widgets` plus complete RealQA package/Turborepo CI entrypoints for proto, Go unit/integration/billing races, frontend, Rust, Linux capture, Chrome/Native Messaging, fixture artifacts, DeliDev settings, and inactive-production safety. RealQA CI must retain the existing aggregate jobs, run each new change-scoped job through `CI Result`, build only local non-root `linux/amd64`/`linux/arm64` image fixtures, and use ephemeral fixture signatures/attestations without registry, deployment, secret, production identity, mobile, App/extension registration, DNS/R2 provisioning, catalog activation, or store-publication authority. Missing planned commands must not be represented by passing placeholders.
