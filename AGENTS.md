...
- `servers/delibase` owns Go/PostgreSQL/sqlc persistence, organization/team/invitation policy, immutable organization/team-membership and provider identities, historical subscription and billing-period snapshots, append-only billing and reservation invariants, provider integrations, server configuration, and future GHCR artifacts. Retained refunds preserve their provider-order binding and monotonic provider/reversal state; retained paid cycles immutably snapshot their provider-order, subscription, billing-period, and original period-bound identities, increase reversals only to match retained refunds, and block deletion of their referenced subscriptions and billing periods.
- Delibase subscription checkout creation is serialized per organization across all caller idempotency keys. A successful hosted checkout blocks another checkout until provider expiry, while provider failure rolls back the local checkout reservation. Billing summaries prefer an active subscription, otherwise report an unexpired checkout as checkout-pending, and select inactive subscription history by provider-event recency.
