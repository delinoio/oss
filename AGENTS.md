...
- Architecture-specific AppImage Sharun executable launcher bytes are immutable desktop packaging inputs in `apps/devhud/cef-pins.json`; AppImage packaging must verify each launcher's committed SHA-256 digest before the bundler can consume it. The upstream helper archive remains separately validated by the packager.
