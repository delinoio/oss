...
- Run root `pnpm generate:proto`/`pnpm check:proto`, per-package TypeScript typechecks, and generated Go test/vet for both proto roots now. As remaining product and server implementations land, add the exact app commands in `docs/apps-devhud-foundation.md` (`test:deck`, `test:deck:widgets`, `test:realqa`, `test:realqa:native`, `test:realqa:extension`, `check:realqa:package`) and Go format/vet/test plus sqlc/PostgreSQL/provider/image checks for both servers. Missing planned commands must not be represented by passing placeholders.
