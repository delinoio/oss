...
- `release-devhud` is defined in `.github/workflows/release-devhud.yml` and is manual-only with exact `main` version input plus `dry-run` or protected `release` mode.
- DevHud public release contract: serialize every version through one project-wide release group; retain, discover by exact revision across recovery dispatches and earlier attempts of the current rerun, reuse, and revalidate the original complete private signed candidate through the protected review window; fail closed across every documented signing, store, GitHub, Logto, PostgreSQL, R2, asset, registry, docs, and exact-identity provider-neutral deployment boundary; wait on protected review gates; reconcile pending submissions, same-commit retries, and interrupted partial store publication without repeating completed mutations; block the first publication on the exact docs candidate; bind the deployed `/devhud` page to that candidate; remotely reverify the exact GitHub asset set and every extracted evidence payload against signed checksums and keyless bundles, all updater manifests against package bytes, and immutable OCI digests and keyless signatures; query every exact store state before cleanup, withdraw every held store submission after any pre-publication failure including a store-publication job failure, and reconcile attempted infrastructure promotion from live controller status before rollback only while no store is public; and serialize infrastructure, all stores, regular GitHub Release, updater, public docs, independent verification, and GA without beta or partial GA.
