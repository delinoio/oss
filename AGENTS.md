...
- `apps/devhud` contains only the non-product feasibility package, Cargo workspace member, and isolated native macOS CEF gate automation. Its exact Tauri pin includes the upstream macOS compilation correction and must not be replaced with a downstream shim, local patch, fork, or moving branch. Renderer termination remains unobservable through Tauri's public API on Windows/Linux without an upstream source change, so do not claim the complete cross-platform gate has passed or add product, mobile/widget, production packaging/updater, release, publisher, or support work until a separate architecture decision resolves that blocker.
