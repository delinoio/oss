...
- `docs/servers-devhud-deck-foundation.md`: Implemented inactive Deck
  ...
  client-owned refresh/live billing, snapshot, and notification-resolution
  provider slices.
- `docs/protos-devhud-deck-api-contract.md`: Implemented private
  `devhud.deck.v1` Connect contract.
...
- `apps/devhud` is the only full DevHud feature client/native path and contains the implemented unpublished exact-origin Chrome MV3/desktop Native Messaging capture bridge, account-bound encrypted local-draft/native-adapter foundation, and dependency-injected encrypted-draft online-submission state machine. The implemented private Deck and RealQA wire contracts belong at `protos/devhud-deck` and `protos/devhud-realqa`; the bounded Deck server foundation/provider slice and implemented inactive RealQA preset/tracker/auth/deletion/internal GitHub.com provider/online-submission foundation belong only at `servers/devhud-deck` and `servers/devhud-realqa`. RealQA repository issue schemas preserve additive issue type, Issue Form input/textarea prefill, textarea render-language, and dropdown multiplicity metadata, while selected repository owner/name snapshots follow signed rename webhooks by stable provider repository ID. DeliDev owns only the implemented `DeckIntegrationService` and `RealQATrackerService` connection/destination management UI in its existing settings sections plus the static verified-link callback/association artifacts described above; it must not consume RealQA preset/submission services or Deck views, pull-request data, mutations, devices, notifications, or widgets. Never move other DevHud feature implementation into DeliDev/delibase paths or add another DevHud ownership path without synchronized contracts.
...
- Deck is authorized only for authenticated desktop/mobile/tray/shortcut/notification/native-widget workflows plus connection management from the existing DeliDev settings sections. RealQA is authorized only for authenticated desktop capture/editor/offline-draft workflows and its exact-origin signed Chrome MV3 native host; no mobile RealQA. Deck and RealQA require separate minimal-permission GitHub Apps, support GitHub.com only, and keep production-facing catalog records disabled. Deck's disabled delibase meter is `(devhud, deck_github_pull_request_refresh)`, unit `provider_refresh`, precision zero, exactly 50 USD micros/unit, a minimum 86,400-second reservation TTL, and allowlisted only to Deck; one dispatched logical provider refresh is one unit. Non-dispatching `GetRefreshPreflight` returns the server-derived price plus an opaque short-lived token bound to the prospective `RefreshView` identity, origin, and client kind; creation of every new automatic/widget/manual/view-open/shortcut attempt requires the unexpired token and current authoritative catalog validation, while only manual UI displays the price warning and an authenticated exact identity-and-digest replay is looked up first and returns or resumes after token expiry or catalog change. RealQA's future disabled meters are `(devhud, realqa_image_transfer)`, precision-zero `encoded_mib` at 500 USD micros/unit, and `(devhud, realqa_image_storage)`, precision-zero `mib_day` at 2 USD micros/unit, both allowlisted only to RealQA. Transfer reserves aggregate ceiling-MiB units from all accepted declared upload lengths and commits the same calculation over the successfully verified subset, excluding server-sanitized output growth; storage authorization maximum and daily settlement use the exact checked formulas in the RealQA server contract, and zero numerators skip delibase billing mutations. Divergent mappings fail before submission/settlement. Deck and RealQA each validate a dedicated outbound exact delibase origin/audience/service UUID/client ID/secret set and least-privilege token acquisition before billed work. Every Deck reserve, release, and commit combines its M2M bearer with the current memory-only forwarded user bearer from the same active client request; no scheduler, detached continuation, billing worker, stored bearer, finalization grant, or background authorization may finish it. RealQA recurring calls use the same feature-scoped identity with authorized-usage scopes and no forwarded bearer.
- The sole DeliDev exception to the full-feature RealQA client boundary is `RealQATrackerService` connection and destination discovery in `/account` and `/o/:orgSlug/settings`; it does not authorize capture, presets, submissions, uploads, lifecycle deletion, or a top-level RealQA route.
...
- The production registry contains the closed internal Deck tool on desktop, iOS, and Android and the closed internal RealQA tool on macOS, Windows, and Ubuntu desktop. iOS and Android must filter RealQA before rendering or dispatch. Distributed Deck widgets select one server-backed configured view per instance across WidgetKit small/medium/large and Android compact/wide/list families, default to counts only, require explicit per-widget repository/title disclosure, participate in active-client polling from the returned device configuration, and retain cold-launch actions until frontend acknowledgement. These client registrations do not deploy services, configure DNS/R2, register GitHub Apps, publish images/extensions/stores, inject production identities, enable catalog entries, or roll out operations.
- Run root `pnpm generate:proto`/`pnpm check:proto`, per-package TypeScript typechecks, and generated Go test/vet for both proto roots now. DevHud implements focused `test:deck` coverage for its authenticated composable product surface, raw/builder query fidelity, mutation/confirmation/offline behavior, and client-owned refresh controller; `test:deck:widgets` covers the one-shot widget refresh boundary. It implements complete Deck package/Turborepo CI entrypoints for proto, Go unit/GitHub/no-scheduler/refresh races, frontend, desktop/mobile auth smoke, widget policy/native builds, deterministic no-SLO reports, local images, and inactive-production safety. It also implements complete RealQA package/Turborepo CI entrypoints for proto, Go unit/integration/billing races, frontend, Rust, Linux capture, Chrome/Native Messaging, fixture artifacts, DeliDev settings, and inactive-production safety. Both matrices must retain existing aggregate jobs, run every coordinated change-scoped job through `CI Result`, build only local non-root `linux/amd64`/`linux/arm64` image fixtures, and use ephemeral fixture signatures/attestations without registry, deployment, secret, production identity, mobile RealQA, App/extension registration, DNS/R2 provisioning, catalog activation, or store-publication authority. Run Go format/vet/test plus sqlc/PostgreSQL/provider/image checks for both servers. Missing planned commands must not be represented by passing placeholders.
...
- `devhud-widget-android`: compiles and tests the distributed Android Deck widget plus private native plugin and verifies the exact non-exported receiver in the release app artifact.
- `devhud-widget-ios`: compiles and tests the distributed WidgetKit Deck extension plus private native plugin on macOS and verifies extension embedding and narrow app/extension provisioning identities in the release app artifact.
...
- CI jobs perform self-gating except for the complete Deck and RealQA validation matrices, whose documented `deck-changes` and `realqa-changes` jobs provide shared path scopes to their coordinated jobs; there is no repository-wide `detect-changes` job.
