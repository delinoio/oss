...
- Root `pnpm install` must install Lefthook in linked worktrees when `core.hooksPath` already targets Git's shared common-directory hooks path, while preserving Lefthook's protective failure for unrelated custom hook paths.
