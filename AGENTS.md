...
- `react-forge` retains the six-host native matrix; its Linux x64 job prepares and inspects scene exports once. `react-forge-scenes` uses the identical change plan, consumes that run's prepared inputs and renders four product pairs in independent 120-minute Linux jobs at 2048px or larger and 96 samples. Require both formats, all four views, matching input/image hashes and all four jobs in `CI Result`; do not reduce visual coverage to fit the native matrix's 60-minute budget.
