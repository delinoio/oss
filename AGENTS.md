...
- Root `pnpm dev` is the DevHud team workflow. Root documentation development uses `pnpm dev:public-docs`, `pnpm dev:nodeup-docs`, `pnpm dev:binpm-docs`, `pnpm dev:runmoor-docs`, or `pnpm dev:runlens-docs`; every root and package-local development command must use the app-owned fixed ports documented in `apps/AGENTS.md` and fail on conflicts instead of automatically remapping a server.
