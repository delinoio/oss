...
- DevHud public release contract: reuse and revalidate the complete private signed candidate before external action; fail closed across every documented signing, store, GitHub, Logto, PostgreSQL, R2, asset, registry, docs, and provider-neutral deployment boundary; wait on protected review gates; block the first publication on the exact docs candidate; bind the deployed `/devhud` page to that candidate; remotely reverify immutable OCI digests and keyless signatures; withdraw every held store submission before automatic infrastructure rollback; and serialize infrastructure, all stores, regular GitHub Release, updater, public docs, independent verification, and GA without beta or partial GA.
