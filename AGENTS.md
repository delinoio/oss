...
- Issue #971 adds the private `clibox-fspy` workspace member for file-access workflows. Its Linux capture backend and partial workflow handlers are not a public command. Implement and validate the operation stream and all seven commands on all eight native targets against `docs/crates-clibox-fspy-contract.md` before exposing the family or counting it as shipped; legacy fspy `READ` access hints cannot prove content reads.
