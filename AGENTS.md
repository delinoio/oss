...
- Architecture-specific AppImage Sharun helper-bundle bytes, including the launcher, are immutable desktop packaging inputs in `apps/devhud/cef-pins.json`; AppImage packaging must verify the bundle's committed SHA-256 digest before the bundler can consume it.
