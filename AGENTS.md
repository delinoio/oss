...
- `binpm install <source> --as <cmd> --bin <upstream-binary>` must preserve explicit global command aliases and selected upstream binaries in global package records without changing source identity. Human install output must show the installed command alias and selected upstream binary as separate fields so repository names, local/global command aliases, and upstream binary names are not conflated.
