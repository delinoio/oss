...
- `apps/devhud` contains only the non-product feasibility package, Cargo workspace member, and isolated native macOS CEF gate. Its private x64/ARM64 DMG and updater validation is gate-only; do not publish or treat those artifacts as product packages. At the pinned revision, renderer termination is not observable through Tauri's public API on Windows/Linux without an upstream source change, so the complete gate remains blocked. Do not add product, mobile/widget, production packaging/updater, release, publisher, or support work until a separate architecture decision resolves the documented blocker.
