...
- Keep `clibox` as the root CLI composer with direct path dependencies on `clibox-config`, `clibox-fspy`, `clibox-system`, `clibox-transform`, and `clibox-wait`; companion crates must not depend on one another or the executable. Family command definitions, runtimes, errors, and unit tests belong to their owning crate. Only the executable version participates in product release synchronization; companion versions remain internal. All six crates must be selected by clibox CI/release tests and covered by npm task cache inputs and change detection.
- Issue #971 adds the private `clibox-fspy` workspace member and source CLI family for file-access workflows. Complete and validate the paired operation stream and all seven commands on all eight native targets against `docs/crates-clibox-fspy-contract.md` before counting the family as releasable. Published clibox 0.2.0 does not include these commands; release remains a separate manual action. Legacy fspy `READ` access hints cannot prove content reads.
- Missing subcommands for `clibox run`, `port`, `clipboard`, `wait`, `fspy`, `text`, `time`, `base64`, `hash`, `dotenv`, and `yaml` must show command-specific clap help on stderr with exit code 2. Preserve root no-argument and explicit help success on stdout, and redact all other parser failures.
...
- React Forge also owns `crates/forge-scene`, `crates/forge-glb` and `crates/forge-fbx` for generation-only GLB/FBX with bounded animation. Follow `docs/packages-react-forge-scene-contract.md`; runtime generation remains local and converter-free.
