...
- `apps/devhud` is the only full DevHud feature client/native path. The
  ...
  `protos/devhud-deck` and `protos/devhud-realqa`; Deck's bounded server
  foundation belongs only at `servers/devhud-deck`, and other planned feature
  server work belongs only at `servers/devhud-deck` and
  `servers/devhud-realqa`. DeliDev owns only the planned
  `DeckIntegrationService` connection-management UI in its existing settings
  sections and the static verified-link callback/association artifacts
