...
- DevHud updater networking remains a closed native capability with no token and no frontend CSP/navigation access. Preserve the pinned-root/signed-successor/explicit-rollback trust chain, installed-package selection, 30-second then 24-active-hour schedule, separate download/install/restart approvals, typed redacted diagnostics, rollback for replaceable app bundles, and truthful restart-only recovery after committed platform-package installs defined in `docs/apps-devhud-updater-contract.md`. Desktop release publication must run the updater trust-root readiness gate.
