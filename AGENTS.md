...
- Run root `pnpm generate:proto`/`pnpm check:proto`, per-package TypeScript typechecks, and generated Go test/vet for both proto roots now. DevHud implements focused `test:deck` coverage for its authenticated composable product surface, raw/builder query fidelity, mutation/confirmation/offline behavior, and client-owned refresh controller; `test:deck:widgets` covers the one-shot widget refresh boundary. It also implements complete RealQA package/Turborepo CI entrypoints for proto, Go unit/integration/billing races, frontend, Rust, Linux capture, Chrome/Native Messaging, fixture artifacts, DeliDev settings, and inactive-production safety. RealQA CI must retain the existing aggregate jobs, run each new change-scoped job through `CI Result`, build only local non-root `linux/amd64`/`linux/arm64` image fixtures, and use ephemeral fixture signatures/attestations without registry, deployment, secret, production identity, mobile, App/extension registration, DNS/R2 provisioning, catalog activation, or store-publication authority. Run Go format/vet/test plus sqlc/PostgreSQL/provider/image checks for both servers. Missing planned commands must not be represented by passing placeholders.
