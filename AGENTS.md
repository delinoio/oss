...
- Missing subcommands for `clibox env`, `run`, `port`, `clipboard`, `wait`, `text`, `time`, `base64`, `hash`, `dotenv`, and `yaml` must show command-specific clap help on stderr with exit code 2. Preserve root no-argument and explicit help success on stdout, and redact all other parser failures.
