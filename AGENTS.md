...
- DevHud local development is selected only as `team` or `oss`. `pnpm exec vp run dev` is the fail-closed authorized-team path; it pins the preflight issuer comparison through migration and service launch, and concurrent team starts in one checkout fail while that pin is active. `pnpm exec vp run dev:oss` is the contributor path and cannot depend on hosted Infisical. Both validate before migration and launch the same frontend/admin/API Vite Task boundary. OSS Compose projects and preserved dependency volumes are scoped to the checkout's generated identity material, concurrent OSS starts in one checkout fail before dependency ownership is shared, interrupted Logto database initialization is repaired on retry, and `pnpm exec vp run dev:oss:down` must not stop another checkout's dependencies.
...
- Follow `docs/repository-task-contract.md`. Vite Task from exact `vite-plus@0.3.3` owns repository and workspace task orchestration. Use `pnpm exec vp run <task>` or `<package>#<task>`; runner options precede the task name.
- Keep runnable task definitions in package-owned `vite.config.ts` files and only installation lifecycle scripts in `package.json`. Do not introduce duplicate script/task names or require a global Vite+ installation.
- Preserve pnpm, Lefthook, Rsbuild/Rspress, Vitest/Jest, Cargo, and Go. Cache only explicitly configured deterministic tasks, with bounded outputs and all external inputs. Development, embedded administrator output, native/mobile packaging, signing, smoke, and publication tasks remain uncached.
- Vite Task 0.3.3 does not filter uncached task environments. Root development must sanitize before launching the runner, and service wrappers own configuration injection. Validate the actual runner boundary with `pnpm exec vp run test:tasks` and the existing cross-platform environment suite.
- Update task consumers, CI path gates, contracts, and tests together. API/sweeper compilation must still generate the administrator bundle first, including inside Docker.
...
- `node-binpm-docs-test`: runs `pnpm install --frozen-lockfile` and `pnpm exec vp run binpm-docs#test`.
- `node-nodeup-docs-test`: runs `pnpm install --frozen-lockfile` and `pnpm exec vp run nodeup-docs#test`.
...
- Node workspace jobs use in-job `dorny/paths-filter` gates and the pinned local Vite Task runner through `pnpm exec vp run <package>#<task>`. Selected jobs execute every designated check without affected-package calculation; unselected jobs complete as successful no-ops.
