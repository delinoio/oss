...
- Run root `pnpm generate:proto`/`pnpm check:proto`, per-package TypeScript typechecks, and generated Go test/vet for both proto roots now. DevHud implements focused `test:deck` and `test:deck:widgets` commands for the dependency-injected refresh controller, and its Chrome capture bridge implements `test:realqa:native`, `test:realqa:extension`, and `check:realqa:package`; expand those commands as remaining product slices land and run Go format/vet/test plus sqlc/PostgreSQL/provider/image checks for both servers. Missing planned commands must not be represented by passing placeholders.
