...
- The CLI provides help/version and `wait tcp`, `wait http`, and `wait file` under issue #919, alongside the #916 utilities, the seven offline #917 transformations, and the #920 configuration commands. Waits are stateless, use immediate nonoverlapping polling and monotonic deadlines, support handled cancellation, and expose only redacted human/quiet/JSON results. Publish crates.io through Release Project, followed by `clibox@v<version>` and the npm workflow. The GNU binaries also feed the stable native repository described above.
