...
- DevHud public release contract: serialize every version through one project-wide release group; reuse and revalidate the complete private signed candidate before external action; fail closed across every documented signing, store, GitHub, Logto, PostgreSQL, R2, asset, registry, docs, and exact-identity provider-neutral deployment boundary; wait on protected review gates; reconcile same-commit retries from already-public stores without repeating store mutations; block the first publication on the exact docs candidate; bind the deployed `/devhud` page to that candidate; remotely reverify immutable OCI digests and keyless signatures; withdraw every held store submission after any pre-publication failure and invoke infrastructure rollback only when promotion completed; and serialize infrastructure, all stores, regular GitHub Release, updater, public docs, independent verification, and GA without beta or partial GA.
