...
- Run root `pnpm generate:proto`/`pnpm check:proto`, the Deck package TypeScript typecheck, and Deck generated Go test/vet now. As remaining implementations land, add the exact app commands in `docs/apps-devhud-foundation.md` (`test:deck`, `test:deck:widgets`, `test:realqa`, `test:realqa:native`, `test:realqa:extension`, `check:realqa:package`), RealQA proto checks, and Go format/vet/test plus sqlc/PostgreSQL/provider/image checks for both servers. Missing planned commands must not be represented by passing placeholders.
