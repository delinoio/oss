...
- Issue #971 adds the private `clibox-fspy` workspace member for file-access workflows. Its current record-validation foundation is not a public command. Implement the operation stream and all seven commands against `docs/crates-clibox-fspy-contract.md` before exposing the family or counting it as shipped; legacy fspy `READ` access hints cannot prove content reads.
