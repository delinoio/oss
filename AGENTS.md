...
- RealQA composer source pixels and nondestructive operations remain local to the draft/session boundary. Its native image-safety path validates every closed edit operation, crop and sequence before deterministic flattening, re-encodes approved PNG/WebP without source metadata, and accounts for the replacement output under the same per-image/session limits. Only the typed approved result may form a future upload payload; raw source bytes never enter upload payloads, server state, logs, diagnostics, or clipboard side effects.
