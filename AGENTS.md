...
- Delibase subscription checkout creation is serialized per organization across all caller idempotency keys. A successful hosted checkout blocks another checkout until provider expiry, while provider failure rolls back the local checkout reservation.
