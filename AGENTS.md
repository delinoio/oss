...
- Ambiguous recurring-storage reserves must replay their stable key after rollover before a definitive period rejection starts grace; an accepted hold that reaches terminal authorization/access failure during commit must be released. Owner deletion must retain unresolved initial-authorization and rebind recipes, while a replacement closed during failed rebind recovery terminalizes that attempt so a distinct payer retry can proceed.
