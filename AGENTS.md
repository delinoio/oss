...
- `apps/devhud` contains only the non-product feasibility package, Cargo workspace member, and isolated native macOS CEF gate automation. At the pinned revision, macOS cannot compile because `tauri-runtime-cef` leaves the implementation of its Linux/BSD-only signal type unconditional and references its Linux/BSD-only `libc` dependency. Renderer termination is also not observable through Tauri's public API on Windows/Linux without an upstream source change. Do not shim, patch, or fork around either blocker, claim any macOS gate condition, or add product, mobile/widget, production packaging/updater, release, publisher, or support work until a separate architecture decision resolves them.
