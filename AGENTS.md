...
- Keep unpublished pnport Cargo, lock, and npm source versions at `0.0.0` until the first minor bump to `0.1.0`; reject patch and major bumps from `0.0.0`. Release Project must prove all six native execution, installed npm/Yarn PnP, TypeScript, archive, and installer gates on a disposable candidate before version commit or tag; its tested tree must equal the committed tree. The separate exact-tag workflow defaults to credential-free dry run and grants npm, GitHub Release, and Homebrew publication only after the complete set passes. The public `/pnport` guides remain explicitly unreleased until publication.
