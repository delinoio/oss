...
- DevHud settings schema v6 retains schema-v5 R2 metadata and changes local-agent repository prompts from a legacy boolean to unique bounded repository/body records; v1-v5 snapshots migrate deterministically. Local agents are desktop-only, default-off, exact-version pinned, and never installed by DevHud. Their paths, observed versions/health, consent, and managed full-clone cache remain local. Direct runs receive the selected PAT only as ephemeral `GH_TOKEN` after enablement and per-submission warnings; all modes use immutable bounded envelopes, isolated clones, strict output validation, cancellation/15-minute timeout, redacted diagnostics, and no automatic fallback.
