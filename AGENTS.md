...
- The CLI provides help/version and `wait tcp`, `wait http`, and `wait file` under issue #919, preserving #916/#917 interfaces. Waits are stateless, use immediate nonoverlapping polling and monotonic deadlines, support handled cancellation, and expose only redacted human/quiet/JSON results. Publish crates.io through Release Project, followed by `clibox@v<version>` and the npm workflow. No Homebrew or public GitHub Release assets are added.
