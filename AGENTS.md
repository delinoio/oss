...
- Authoritative Polar refunds and chargebacks continue against retained billing history after organization deletion, and billing-portal session creation uses a distinct typed audit event from subscription checkout creation.
...
- Networking remains deny-by-default except the signed updater, exact Logto/DeliDev auth, exact Deck/RealQA origins, same-origin signed RealQA-assets PUT/public GET, verified mobile auth callback, and exact-origin Chrome Native Messaging boundaries. Add narrow per-window/per-command allowlists without generic frontend filesystem/screen/process/store/network authority.
