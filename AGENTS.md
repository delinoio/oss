...
- DevHud settings schema v6 retains schema-v5 R2 metadata and changes local-agent repository prompts from a legacy boolean to unique bounded repository/body records; v1-v5 snapshots, including dangling agent profile references, migrate deterministically while v6 references remain strict. Local agents are desktop-only, default-off, exact-version pinned, and never installed by DevHud. Their paths, observed versions/health, consent, and managed full-clone cache remain local. Direct Codex runs remain read-only and receive neither the selected PAT nor network access; after strict marker readiness, DevHud owns one exact argv-based `gh api` write. Direct Claude Code and OpenCode runs receive the selected PAT only as ephemeral `GH_TOKEN` and may use only the exact issue command against a safe relative input path after enablement and per-submission warnings. All modes use immutable bounded envelopes, isolated clones, strict output validation, atomically purge-gated cancellation/15-minute timeout, redacted diagnostics, and no automatic fallback.
