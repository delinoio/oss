...
- Delibase Polar startup requires the single provider product ID and pins it together with the provider environment to the database-enforced monthly USD `$10` price and `10,000,000`-micro-unit paid-cycle grant. Production is the default provider environment; sandbox checkout requires the explicit `DELIBASE_POLAR_ENVIRONMENT=sandbox` opt-in, and startup must fail rather than reuse provider state persisted for the other environment.
- Polar subscription checkout creation must hold the organization billing lock through provider creation and atomically persist the active checkout, audit, and idempotency result. A successful checkout blocks distinct idempotency keys until provider expiry; a provider failure must roll back without local checkout state. Billing summaries prefer an active subscription, otherwise report an unexpired checkout as checkout-pending, and select inactive subscription history by provider-event recency. Retained paid cycles block deletion of their referenced subscriptions and billing periods.
- Polar refunds and chargebacks must reconcile against retained financial history after organization deletion. Hosted billing-portal sessions and subscription checkouts use distinct typed audit events.
