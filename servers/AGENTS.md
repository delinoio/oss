...
- Use PostgreSQL and sqlc for persistence, UUID v7 for persisted IDs, signed int64 USD micro-units for money, signed int64 units for usage, non-overlapping effective price/billing-period windows, organization-scoped billing references, and transactional/locked append-only ledger, audit, hierarchy, and reservation invariants. Reservation prices, team snapshots, active memberships, and expiries must match the effective catalog/team state; reservation costs must equal their overflow-safe unit-price product; commits must consume only the held credit/overage split and must not exceed the reserved maximum. Active references block operational deletion while finalized financial and usage snapshots do not.
