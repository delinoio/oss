...
- Shared Logto validation must require the exact configured issuer, canonical `https://delibase.deli.dev` audience, expiry, allowed asymmetric signature/header type, `kid`, route scopes, and the expected user or client-credentials token shape. Unknown JWKS key IDs force refresh; stale-key use is opt-in and bounded.
...
- Server root loggers must use the shared redacting handler, and routine events must use allowlisted safe fields plus keyed actor pseudonyms. Configure pseudonymization with a distinct secret of at least 32 bytes; never log the key or raw Logto subject.
