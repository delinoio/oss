...
- Administrator handlers return only the admin-safe user, usage, metadata-only upload, and audit views defined by the schema. They must never populate settings bodies, secrets, DOM, screenshots, public or signed asset locators, Deck results, agent output, or local paths in messages or errors.
