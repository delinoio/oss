...
- Authenticated invitation preview and acceptance must not require a pre-existing local account, organization membership, or team access. Acceptance creates and locks the active local account when absent, locks the organization before the invitation, and revalidates invitation state before creating memberships.
