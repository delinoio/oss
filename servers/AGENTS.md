...
- Bounded background usage extends only `BillingService` and `UsageService`. Human authorization create/revoke and M2M authorized reserve/commit/release use distinct idempotency operations. Authorized usage uses `ModeM2M`, never requires or persists a forwarded user bearer, and binds the authenticated service plus authorization/purpose/resource/UTC-period/units into its digest. Grants use UUID v7, bind the authorizer, personal/organization owner, organization/team, exact service/meter, `REALQA_STORAGE`, feature resource, `UTC_DAY`, and a per-period maximum, and fail closed on substitution, lost access, non-active status, limit overflow, or replay conflict. Deck must have no purpose value.
