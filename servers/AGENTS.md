...
- `servers/devhud-api`: implemented stateless Go foundation for Bootstrap, Settings, Account, and the account/retention sweeper; Upload, Diagnostics, and Admin remain planned and unregistered.
...
- Administrator handlers require mutation reasons containing non-blank, well-formed UTF-8 text capped at 4 KiB and reject credential and local-path patterns with `InvalidArgument` before persistence. They return only previously validated reasons in the admin-safe user, usage, metadata-only upload, and audit views defined by the schema, and must never populate settings bodies, secrets, DOM, screenshots, public or signed asset locators, Deck results, agent output, or local paths in messages or errors.
