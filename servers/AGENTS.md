...
- Bounded background usage extends only `BillingService` and `UsageService`. Human authorization create/revoke and M2M authorized reserve/commit/release/resource-deletion notification use distinct idempotency operations. Authorized usage uses `ModeM2M` and never requires or persists a forwarded user bearer. Reserve/commit/release digest-bind authenticated service plus authorization/purpose/resource/UTC-period/units, with reserve also binding `client_reference` and commit/release binding the reservation ID; resource deletion binds authorization/service/purpose/resource/expected revision. Reusing one key with a changed payload uses `ERROR_REASON_IDEMPOTENCY_CONFLICT`; using a different key to repeat an already recorded logically unique authorized period/reservation settlement uses `ERROR_REASON_BACKGROUND_USAGE_REPLAY_CONFLICT`. Public catalog meters expose stable service authorization targets only through explicit service/meter `REALQA_STORAGE` capability mappings, not ordinary usage allowlists, and never expose Logto client IDs or credentials. Reserve accepts only the current or immediately preceding UTC day according to server time, while commit/release use the reservation's stored period. Before deleting a feature resource, the bound service durably tombstones it, stops reserving for it, and retries the idempotent deletion notification until delibase acknowledges it. Grants use UUID v7, bind the authorizer, personal/organization owner, organization/team, exact service/meter, `REALQA_STORAGE`, feature resource, `UTC_DAY`, and a per-period maximum, and fail closed on substitution, lost access, non-active status, limit overflow, or replay conflict. Deck must have no purpose value.
