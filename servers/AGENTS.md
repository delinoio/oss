...
- Shared Logto validation must require the exact configured issuer, canonical `https://delibase.deli.dev` audience, expiry, allowed asymmetric signature/header type, `kid`, route scopes, and the expected user or client-credentials token shape. Unknown JWKS key IDs force refresh; stale-key use is opt-in and bounded; final JWKS response URLs after redirects must satisfy the configured HTTPS/no-credentials/no-query/no-fragment boundary.
...
- Shared safe-error mapping may preserve an intentional Connect status code and recognized `delibase.v1.ErrorDetail.reason` only. It must discard arbitrary response metadata, unrecognized detail types, and free-form detail fields; validated request and trace IDs are added by the outer request metadata interceptor.
