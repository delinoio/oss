...
- CORS must use the exact DevHud development and pinned Tauri origins, with explicit Connect preflight methods and headers and no wildcard origin/header policy. The R2 staging bucket separately allows only those origins, `PUT`/`OPTIONS`, `Content-Type`, and `x-amz-checksum-sha256`, with `ETag` exposure.
...
- Uploads are scoped to a server-owned submission UUID spanning all upload groups, capped at 10 finalized images; signed checksums and versioned staging objects are required for conditional promotion, and PNG IHDR dimensions must be rejected above 4096×4096 or 16,777,216 pixels before decoding.
