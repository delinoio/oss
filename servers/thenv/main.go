...
func main() {
	...
	masterKey := strings.TrimSpace(os.Getenv("MASTER_KEY_B64"))
	if masterKey == "" {
		log.Fatal("MASTER_KEY_B64 is required")
	}
	...
	svc, err := service.New(ctx, service.Config{
		...
		BootstrapAdminSubject: strings.TrimSpace(os.Getenv("BOOTSTRAP_ADMIN_SUBJECT")),
	}, logger)
	if err != nil {
