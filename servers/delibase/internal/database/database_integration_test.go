...
func TestPostgreSQLOrganizationDeletionCleanupAndLiveSubscriptionSelection(
	t *testing.T,
) {
	databaseURL := os.Getenv("DELIBASE_TEST_DATABASE_URL")
	if databaseURL == "" {
	...
	if _, err := store.pool.Exec(ctx, `
		...
		VALUES ($1, $2)
	`, accountID, "deletion-cleanup-"+accountID.String()); err != nil {
		t.Fatal(err)
	}
	createTestOrganization(
		...
		"Deletion Cleanup",
		"deletion-cleanup-"+organizationID.String()[24:],
		accountID.String(),
	)
	activeSubscriptionID := uuidv7.MustNew()
	pendingSubscriptionID := uuidv7.MustNew()
	invitationID := uuidv7.MustNew()
	if _, err := store.pool.Exec(ctx, `
		INSERT INTO subscriptions (
			...
			($1, $3, $4, 'active', transaction_timestamp() - interval '1 hour'),
			($2, $3, $5, 'pending', transaction_timestamp())
	`,
		activeSubscriptionID,
		pendingSubscriptionID,
		organizationID,
		"polar-active-"+activeSubscriptionID.String(),
		"polar-pending-"+pendingSubscriptionID.String(),
	); err != nil {
		t.Fatal(err)
	}
	if _, err := store.pool.Exec(ctx, `
	...
	deleted, err := store.Queries().DeleteOrganizationOperationalData(
		ctx,
		pgtype.UUID{Bytes: organizationID, Valid: true},
	)
	if err != nil || !deleted {
		t.Fatalf("operational deletion = %t, %v", deleted, err)
	}
	var teams, memberships, invitations, customers, subscriptions, slugs int
	var name string
	err = store.pool.QueryRow(ctx, `
		SELECT
			...
			(SELECT count(*) FROM polar_customers WHERE organization_id = $1),
			(SELECT count(*) FROM subscriptions WHERE organization_id = $1),
			(SELECT count(*) FROM organization_slug_registry WHERE organization_id = $1)
	`, organizationID).Scan(
		&teams,
		&memberships,
		&invitations,
	...
	if teams != 0 || memberships != 0 || invitations != 0 ||
		name != "Deleted organization" || customers != 1 ||
		subscriptions != 2 || slugs != 1 {
		t.Fatalf(
			"cleanup = teams:%d memberships:%d invitations:%d name:%q customers:%d subscriptions:%d slugs:%d",
			teams,
			memberships,
			invitations,
			name,
...
func TestPostgreSQLSchemaEnforcesOrganizationBoundariesAndRetention(t *testing.T) {
	...
	const (
		...
		expiredInvite  = "0198a000-0000-7000-8000-000000000148"
	...
	if _, err := transaction.Exec(ctx, `
		INSERT INTO accounts (id, logto_subject)
		VALUES ('0198a000-0000-7000-8000-000000000329', 'deleted-account')
	`); err != nil {
		t.Fatal(err)
	}
	if _, err := transaction.Exec(ctx, `
		UPDATE accounts
		SET status = 'deleted'
		WHERE id = '0198a000-0000-7000-8000-000000000329'
	`); err != nil {
		t.Fatal(err)
	}
	requireConstraintFailure(t, ctx, transaction, `
	...
	requireConstraintFailure(t, ctx, transaction, `
		...
		INSERT INTO polar_customers (organization_id, polar_customer_id)
		SELECT organization_id, 'replacement-customer'
		FROM removed_customer
	`, orgA)
	requireConstraintFailure(t, ctx, transaction, `
		UPDATE polar_customers
	...
	if err := transaction.QueryRow(ctx, `
		...
		SELECT accepted_at = statement_timestamp()
		FROM acceptance
	...
	if _, err := transaction.Exec(ctx, `
		INSERT INTO organization_invitations (
			...
			$1, $2, decode(repeat('fb', 32), 'hex'), 'admin', $3,
			statement_timestamp()
		)
	`, expiredInvite, orgA, accountA); err != nil {
		t.Fatal(err)
	}
	requireConstraintFailure(t, ctx, transaction, `
		INSERT INTO organization_invitation_acceptances (invitation_id, account_id)
		VALUES ($1, $2)
	`, expiredInvite, accountB)
	if _, err := transaction.Exec(
		ctx,
	...
	`, orgA, periodA)
	orphanReservationOperations := []struct {
		name   string
		amount int64
	}{
	...
	for index, operation := range orphanReservationOperations {
		requireConstraintFailure(t, ctx, transaction, `
			INSERT INTO ledger_entries (
				id, organization_id, billing_period_id, entry_type,
				amount_micros, balance_after_micros, source_reference
			) VALUES (
				('0198a000-0000-7000-8000-' || lpad(($1 + 340)::text, 12, '0'))::uuid,
				$2, $3, $4, $5, $5, $4
			)
		`, index, orgA, periodA, operation.name, operation.amount)
	}
	...
	if err := transaction.QueryRow(
		ctx,
		"SELECT created_at > transaction_timestamp() AND created_at <= statement_timestamp() FROM usage_reservations WHERE id = $1",
		reserveID,
	...
	if _, err := transaction.Exec(ctx, `
		...
		SELECT
			'0198a000-0000-7000-8000-000000000149',
			$1, 'credit_hold', -3, COALESCE(sum(amount_micros), 0) - 3,
			$2, $3, 'A', 'partial-commit-credit-hold'
		FROM ledger_entries
		WHERE organization_id = $1
	`, orgA, reserveID, teamA); err != nil {
		t.Fatal(err)
	}
	if _, err := transaction.Exec(
		ctx,
	...
	requireConstraintFailure(t, ctx, transaction, `
		INSERT INTO usage_records (
			...
			meter_id, account_id, service_identity_id, committed_units,
			total_cost_micros, credit_applied_micros, overage_applied_micros
		) VALUES ($1, $2, $3, $4, 'A', $5, $6, $7, 4, 4, 3, 1)
	`, recordID, reserveID, orgA, teamA, meterID, historyUser, serviceID)
	requireConstraintFailure(t, ctx, transaction, `
		INSERT INTO usage_records (
			id, reservation_id, organization_id, team_id, team_name_snapshot,
	...
	if err := transaction.QueryRow(ctx, `
		...
		SELECT committed_at = statement_timestamp()
		FROM usage
	`, recordID, reserveID, orgA, teamA, meterID, historyUser, serviceID).
		Scan(&storageCommittedAt); err != nil || !storageCommittedAt {
		t.Fatalf("usage storage commit timestamp = %t, %v", storageCommittedAt, err)
	}
	...
	if err := transaction.QueryRow(
		...
		reserveID,
	).Scan(&reservationStatus); err != nil || reservationStatus != "held" {
	...
	if _, err := transaction.Exec(ctx, `
		...
		SELECT
			$1, $2, $3, 'credit_commit', -1,
			COALESCE(sum(amount_micros), 0) - 1, $4, $5, $6, 'A',
	...
	if _, err := transaction.Exec(ctx, `
		...
		SELECT
			'0198a000-0000-7000-8000-000000000150',
			$1, 'credit_release', 3, COALESCE(sum(amount_micros), 0) + 3,
			$2, $3, 'A', 'partial-commit-credit-release'
		FROM ledger_entries
		WHERE organization_id = $1
	`, orgA, reserveID, teamA); err != nil {
		t.Fatal(err)
	}
	var finalizationUsesStatementTime bool
	if err := transaction.QueryRow(ctx, `
		WITH finalized AS (
			UPDATE usage_reservations
			SET status = 'committed'
		...
		SELECT finalized_at = statement_timestamp()
		FROM finalized
	`, reserveID).Scan(&finalizationUsesStatementTime); err != nil ||
		!finalizationUsesStatementTime {
		t.Fatalf(
			"reservation finalization storage timestamp = %t, %v",
			finalizationUsesStatementTime,
			err,
		)
	}
	...
	if _, err := transaction.Exec(ctx, `
		INSERT INTO ledger_entries (
			...
			'0198a000-0000-7000-8000-000000000328',
			$1, 'credit_release', 1, 0, $2, $3, 'General',
			'retained-organization-release'
		)
	`, orgC, historyReserve, generalC); err != nil {
		t.Fatal(err)
	}
