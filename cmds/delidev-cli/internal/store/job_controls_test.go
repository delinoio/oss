...
func TestCancellationMigratesV4AndPreservesClaimedEnvelopeAcrossRestart(t *testing.T) {
	...
	if _, err := s.db.Exec("DROP TABLE execution_messages; DROP TABLE execution_references; DROP TABLE execution_grants; DROP TABLE job_assignments; DROP TABLE job_cancellations; PRAGMA user_version=4;"); err != nil {
		t.Fatal(err)
	}
	s.Close()
	s, err = Open(ctx, root)
