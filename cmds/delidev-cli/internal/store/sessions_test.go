...
func TestSessionMigrationBacksUpV3AndRejectsAmbiguousQueueOrder(t *testing.T) {
	for _, duplicate := range []bool{false, true} {
		t.Run(map[bool]string{false: "success", true: "duplicate"}[duplicate], func(t *testing.T) {
			...
			_, err = s.db.Exec("DROP TABLE execution_interactions; DROP TABLE execution_messages; DROP TABLE execution_references; DROP TABLE execution_grants; DROP TABLE job_assignments; DROP TABLE job_cancellations; DROP INDEX session_visibility; DROP INDEX queue_sequence; DROP INDEX queue_pending; PRAGMA user_version=3;")
			if err != nil {
				t.Fatal(err)
			}
			if duplicate {
