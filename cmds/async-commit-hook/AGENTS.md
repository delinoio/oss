...
- Every exact-commit gate hashes terminal evidence outside SQLite transactions, then reselects the latest attempt and compares its full metadata snapshot in a short immediate transaction before returning a decision or accepting a replacement. Concurrent clients reuse one pending attempt without blocking unrelated workers during file I/O.
...
- Read both committed and working-tree project configuration through a 1 MiB plus one-byte stream limit before parsing; terminate and reap oversized Git readers rather than buffering the full blob.
...
- Pairing is retired. Require exact Host/Origin, POST and one API version header before every RPC; never grant remote/development CORS or accept legacy browser credentials. Preserve version-rejection diagnostics through Connect serialization for browser recovery. Pair remains a deprecated Unimplemented wire tombstone. Preserve existing state without creating new pairing/browser tables.
