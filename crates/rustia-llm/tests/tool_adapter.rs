...
use rustia::LLMData;
use rustia_llm::{LlmToolBuildError, LlmToolSpec, tool};
...
#[test]
fn tool_coerces_stringified_numbers_with_rustia_harness() {
    let sum_tool = tool::<SumInput, SumOutput, _, HandlerError>(
        LlmToolSpec::new("sum", "Adds two unsigned integers"),
        |input| {
            Ok(SumOutput {
    ...
    let raw = sum_tool
        ...
        .expect("rustia coercion should recover stringified numbers");
    let output: SumOutput = serde_json::from_str(&raw).expect("output json must be valid");
...
fn tool_returns_structured_feedback_on_validation_failure() {
    ...
    assert!(message.contains("rustia-llm input validation failed."));
