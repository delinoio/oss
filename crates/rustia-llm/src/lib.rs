...
//! rustia-powered adapter utilities for aisdk tool calling.

use std::fmt::{Display, Formatter};

use aisdk::core::tools::{Tool, ToolExecute};
use rustia::{LLMData, LlmJsonParseError, LlmJsonParseResult};
use schemars::{JsonSchema, schema_for};
use serde::Serialize;
use serde_json::Value;
use tracing::{error, info};

/// Input contract for rustia-llm tools.
...
/// Output contract for rustia-llm tools.
...
/// Input parse/validation error emitted by rustia harness.
...
impl LlmToolInputError {
    ...
    pub fn to_feedback_string(&self) -> String {
        ...
        output.push_str("rustia-llm input validation failed.\n");
...
impl Display for LlmToolInputError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "rustia-llm input validation failed with {} error(s)",
            self.errors.len()
        )
    }
}
...
/// Builds an aisdk [`Tool`] using rustia parsing/coercion/validation for tool
/// input.
