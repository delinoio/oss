...
fn install(args: InstallArgs, output: OutputMode) -> Result<i32> {
    ...
    if let Some(source) = &args.source {
        ...
        let result = install_local_manifest(frozen_lockfile, args.require_verified, &[], output)?;
        print_mutation_output(result, output)
    }
}
...
fn update(args: UpdateArgs, output: OutputMode) -> Result<i32> {
    ...
    if output.is_json() && args.dry_run && (args.lockfile.no_frozen_lockfile || frozen_lockfile) {
...
fn preview_local_update_result(
    ...
    let mut tools = planned_manifest
        ...
        .filter(|(cmd, _)| selected.is_empty() || selected.contains(cmd))
        .map(|(cmd, tool)| {
            preview_local_update_tool(cmd, tool, &paths, require_verified, current_target.clone())
        })
        .collect::<Result<Vec<_>>>()?;
    ...
    for tool in &tools {
        ...
        if let Some(installed_path) = &tool.installed_path {
            changed_files.push(installed_path.clone());
        }
...
fn preview_local_update_tool(
    ...
    preview_local_update_tool_from_resolved(
        cmd,
        &spec,
        resolved,
        paths,
...
fn local_install_mutation_output(
    ...
    changed_files.extend(local_cache_entry_changed_files(record)?);
    if !frozen_lockfile {
...
fn local_cache_entry_changed_files(record: &PackageRecord) -> Result<Vec<String>> {
...
fn global_cache_entry_changed_files(paths: &ScopePaths, record: &PackageRecord) -> Vec<String> {
    cache_entry_changed_files(&CachePaths::new(&paths.root), record)
}

fn cache_entry_changed_files(cache_paths: &CachePaths, record: &PackageRecord) -> Vec<String> {
    let mut changed_files = BTreeSet::new();
...
mod tests {
    ...
    use super::{
        ...
        cleanup_failed_install_cache, clear_mutation_warnings, command_alias_differs_from_upstream,
