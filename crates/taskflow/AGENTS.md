...
- Go discovery must disable module proxies, checksum databases, VCS, and automatic toolchain downloads; private-module proxy bypass cannot override this boundary. When a resolved module uses an in-repository replacement, query that replacement's own `go.mod` so its dependency edges are retained.
