...
- Notification path normalization must tolerate concurrent removal while preserving broken-link and permission errors; do not separate existence checks from canonicalization. Windows opens preserve native deletion-pending status separately from ACL denial before Win32 error translation. Windows resolution checks deletion state on the same handle after the final-path lookup, including lookup failures, so NTFS deletion-storage paths never become notification identities.
