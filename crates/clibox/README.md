...
These utilities use the current OS user's permissions and desktop session. No authentication service, saved configuration, cache, operation history or telemetry is added. The environment, port, open, and clipboard commands have no fixed execution timeout or automatic retry apart from the shared five-second port-termination verification. Readiness waits use the polling and deadline rules above. Configuration command limits are documented below. Interrupting a command does not undo completed copies, terminations or application launches.
