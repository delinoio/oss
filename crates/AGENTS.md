...
- Configuration file output must use private same-directory staging, preserve destination access permissions, reject replacement links, clean up on handled failures/cancellation, and never duplicate stdout. Keep configuration diagnostics enum-classified and exclude keys, values, content, paths, argv, and dependency/panic text even with RUST_LOG.
