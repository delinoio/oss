...
- pnport Linux runs dynamic and fully static x64/arm64 ELF children under one owned-child seccomp/PTRACE_TRACEME supervisor. Doctor must probe actual tracing capability and the matching `.so`; unsupported kernels or container emulation fail closed with exit 125. Keep the retained fspy Linux reference and its exact adaptation record, and validate offline Ubuntu 22.04 Docker C, static Go, and Yarn inline/split fixtures. Emulated amd64 results never count as native x64 release evidence.
