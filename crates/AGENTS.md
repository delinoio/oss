...
- The manual `Release Project` coordinator validates the exact version commit and publishes only the selected CLI crate without waiting for main CI. After registry publication succeeds, it pushes only that crate’s exact version tag with `delino-release-bot`. It may recover a missing remote tag after an already-published crate, but must reject a conflicting tag. There is no main-push workspace publisher.
