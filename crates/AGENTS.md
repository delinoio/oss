...
- Keep dotenv value tokens literal, validate every record, and count diagnostic columns by Unicode scalar value without changing byte-based limits; YAML must retain scalar precision, resolve document-local references/merges, reject invalid graphs, sort mappings, and be byte-idempotent. Bound raw input and serialized output independently to 64 MiB and collection depth to 128 before excessive expansion.
