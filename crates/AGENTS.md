...
- Windows configuration replacement must close the staging writer before `ReplaceFileW` while retaining cleanup ownership of its temporary path. Configuration file output must use private same-directory staging, preserve destination access permissions, reject replacement links, clean up on handled failures/cancellation, and never duplicate stdout. Failed diagnostic writes must not panic, expose raw fallback errors, or override command/cancellation status. Keep configuration diagnostics enum-classified and exclude keys, values, content, paths, argv, and dependency/panic text even with RUST_LOG.
