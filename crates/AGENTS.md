...
- Ensure cache reuse is always verified against provider asset digests, upstream checksum/signature material, or locally recorded SHA-256 metadata before extraction or install finalization.
...
- Keep strict verification behavior aligned with `--require-verified` and `binpm verify --require-verified`.
