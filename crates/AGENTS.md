...
- Keep explicit global command naming and upstream binary selection stable: `binpm install <source> --as <cmd> --bin <upstream-binary>` uses the explicit global alias while preserving source identity and selected upstream binary in package records, and human install output must show the installed command alias and selected upstream binary as separate fields.
