...
pub fn channel<A: Allocator>(directory: &std::path::Path, capacity: usize, max_paths: usize, allocator: A) -> io::Result<Receiver<A>> {
    let slots = max_paths.min(capacity / 64).max(1);
    let path = directory.join(format!("runlens-{}.shm", Uuid::new_v4().simple()));
    let shm_c_path = os_c_string(path.as_os_str(), allocator)?;
    let handle =
    ...
    Ok(Receiver { keeper, mapping, slots })
}
...
impl ChannelConf<'_> {
    ...
    pub fn sender<A: Allocator>(&self, allocator: A) -> Option<Sender> {
        ...
        let writer = unsafe { ShmWriter::new(mapping, self.slots) }
...
impl<A: Allocator> Receiver<A> {
    ...
    pub fn conf(&self) -> ChannelConf<'_> {
        ChannelConf { shm_id: IpcStr::from_os_c_str(self.keeper.path.as_c_str()), slots: self.slots }
    }
    ...
    pub fn close(self) -> Result<FrameReader, RecordsLost> {
        let Self { keeper, mapping, slots } = self;
