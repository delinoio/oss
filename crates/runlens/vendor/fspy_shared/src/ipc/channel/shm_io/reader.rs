...
impl<M: AsRawSlice> ShmReader<M> {
    pub const fn incomplete(&self) -> bool { self.incomplete }
    ...
    pub unsafe fn seal(mem: M, slots: usize) -> Result<Self, SealError> {
        ...
        // Runlens retains committed evidence even when the gate records loss.
        let incomplete = claims & CLOSED != 0;
        let claims = claims & !CLOSED;
        ...
        Ok(Self { payload_start: mapped.payload_start, table, _mem: mem, incomplete })
    }
