...
// fanotify_mark places its pathname in operand five on both supported 64-bit
// Linux ABIs. Keep that operand typed and bounded like the existing tuples.
impl<T1: FromSyscallArg, T2: FromSyscallArg, T3: FromSyscallArg, T4: FromSyscallArg, T5: FromSyscallArg> FromNotify
    for (T1, T2, T3, T4, T5)
{
    fn from_notify(notif: &seccomp_notif) -> io::Result<Self> {
        Ok((
