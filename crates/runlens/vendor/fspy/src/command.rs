...
use crate::{TrackedChild, error::SpawnError};
...
impl Command {
    ...
    pub async fn spawn_in(
        ...
        let spy = crate::os_impl::SpyImpl::init_in(directory).map_err(SpawnError::OsSpawn)?;
        spy.spawn(self, directory, capacity, max_paths, cancellation_token).await
    }
