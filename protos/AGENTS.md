...
- DeliDev `SessionChange.execution_job` joins the current immutable first execution assignment in the same read transaction as session/input/preparation references, including exact receipt replay. First Resume may queue a never-executed prepared session only after current readiness validation; Restore and old control receipts never implicitly resume or cancel new work. Preserve later-turn unsupported behavior until its continuation contract is implemented.
