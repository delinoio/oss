...
- Use root `pnpm generate:proto` for generation and `pnpm check:proto` for Buf lint, the checked-in `delibase.v1` descriptor compatibility check, and a clean regeneration diff. TypeScript checks use `pnpm --filter @delinoio/delibase-connect typecheck`; Go checks include `go test ./protos/delibase/...` and `go vet ./protos/delibase/...`.
