...
| `devhud` | Local-only DevHud feasibility scaffold with an exact Tauri CEF pin that includes the upstream macOS compilation fix; product, mobile/widget, CI, packaging, and release work remain blocked by the Windows/Linux renderer-termination limitation. | `apps/devhud` | `gate-blocked` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md) |
