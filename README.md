...
| `devhud` | Developer-tool shell whose implemented signed-out base remains bundled and usable without an account on the exact pinned sandboxed desktop CEF/mobile system-webview foundation; planned bounded authenticated features are Deck for desktop/mobile/native widgets and desktop-only RealQA. The unpublished exact-origin Chrome MV3/Native Messaging capture bridge and private versioned Deck and RealQA Connect source packages are implemented; both feature servers and the authenticated product integrations remain planned. Bundled webviews remain offline; future feature RPC/upload access uses closed native transports, and future service origins, separate GitHub Apps, disabled catalog records, and Chrome production identity are contracts only—not deployments, registrations, publications, or activations. | `apps/devhud`<br>`servers/devhud-deck` (planned)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (planned)<br>`protos/devhud-realqa` | `active foundation, unpublished Chrome capture bridge, and Deck/RealQA API contracts; planned product features` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
