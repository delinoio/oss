...
| `devhud` | Developer-tool shell whose bundled signed-out base runs on the pinned sandboxed desktop CEF/mobile system-webview foundation. Deck's private API, inactive Go/PostgreSQL GitHub.com provider, client-owned five-minute refresh controller, live forwarded-user 50-USD-micros billing path, snapshots, and notification resolution are implemented without a scheduler, polling worker, background authorization, or post-client continuation; live native transport registration, push delivery, packaging, catalog activation, and rollout remain planned. The unpublished exact-origin Chrome MV3/desktop Native Messaging capture bridge and inactive RealQA provider/image-transfer/capture/editor/encrypted-draft foundations are also implemented, while end-to-end RealQA submission and billing remain planned. Bundled webviews remain offline and future authenticated feature traffic crosses closed native transports only. Future origins, fixture manifests, production GitHub Apps, disabled catalog records, and the Chrome production identity are contracts—not deployments, registrations, publications, or activations. | `apps/devhud`<br>`servers/devhud-deck` (inactive implemented provider/refresh slice)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (inactive foundation/provider slice)<br>`protos/devhud-realqa` | `active foundation, unpublished Chrome capture bridge, encrypted local drafts, implemented Deck client/server refresh and provider slices, RealQA provider slices, and private API contracts; planned product activation` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
