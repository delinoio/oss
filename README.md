...
| `devhud` | Developer-tool shell whose implemented signed-out base remains bundled and usable without an account on the exact pinned sandboxed desktop CEF/mobile system-webview foundation; planned bounded authenticated features are Deck for desktop/mobile/native widgets and desktop-only RealQA. The unpublished exact-origin Chrome MV3/desktop Native Messaging capture bridge, private versioned Deck and RealQA Connect source packages, isolated descriptors, and deterministic Go/TypeScript artifacts are implemented, including Deck's structured request-only shortcut inputs, server-authored shortcut/widget state, stale-device reload, mutation-candidate discovery, displayable installation identities, and all-origin refresh preflight. Deck includes its bounded Go/PostgreSQL foundation plus a GitHub.com-only signed connection/lifecycle and user-attributed PR-mutation adapter with an explicit follow-up-refresh success state; billed refresh, notifications, packaging, and client product integration remain planned. The inactive RealQA Go/PostgreSQL/sqlc preset/tracker/auth/deletion, internal GitHub.com provider, and R2-backed image-transfer/public-delivery foundation are also implemented, including stable-ID repository rename synchronization, issue type, Issue Form prefill, textarea render-language, and dropdown multiplicity schema fidelity plus permission-safe preset metadata dispatch; end-to-end submission, billing, and remaining client product integrations remain planned. Bundled webviews remain offline; future feature RPC/upload access uses closed native transports, Deck manual refresh displays its non-dispatching server-authoritative billing quote before confirmation, and delibase deletion jobs durably trigger both feature cleanup paths. Future service origins, fixture-only separate GitHub App manifests, disabled catalog records, and the Chrome production identity are contracts only—not deployments, registrations, publications, or activations. | `apps/devhud`<br>`servers/devhud-deck` (inactive foundation/provider slice)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (inactive foundation/provider slice)<br>`protos/devhud-realqa` | `active foundation, unpublished Chrome capture bridge, Deck/RealQA provider slices and API contracts, and inactive feature server foundations; planned product features` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
...
- `servers/internal` is repository-shared Go infrastructure currently consumed by `delibase`; planned DevHud servers require an explicit compatibility review before reuse. It is intentionally not a project catalog entry.
