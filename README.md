...
| `devhud` | Developer-tool shell whose signed-out base remains bundled and usable without an account on the exact pinned sandboxed desktop CEF/mobile system-webview foundation. Deck is registered as the sole closed internal production tool on desktop, iOS, and Android with provider-owned React Query state, personal/organization views, lossless raw/visual GitHub queries, sorting/grouping, cursor/truncation and failure states, permission-filtered PR actions, explicit merge/manual-refresh confirmation, revision compare/reapply, exact GitHub handoff, client-owned eligible five-minute refreshes, a closed native Connect transport, desktop tray entries, and unified generic/per-view shortcuts. Its server, catalog, push, native-widget distribution, packaging, and rollout remain inactive. The private Deck/RealQA APIs and bounded server foundations are implemented; RealQA's unpublished capture bridge, encrypted drafts, provider/submission/storage foundations remain unregistered product slices. Bundled webviews stay offline and no public plugin, CLI, arbitrary transport, deployment, publication, or activation is implied. | `apps/devhud`<br>`servers/devhud-deck` (inactive implemented provider/refresh slice)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (inactive foundation/provider/submission slice)<br>`protos/devhud-realqa` | `active signed-out foundation and closed internal Deck client; inactive services/catalog/widgets/push and planned product activation` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
