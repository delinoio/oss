...
| `devhud` | Local-only DevHud feasibility scaffold with an isolated macOS x64/ARM64 gate; the exact CEF pin includes the upstream macOS compilation fix, while Windows/Linux renderer-termination observation remains blocked, so product and release work cannot proceed. | `apps/devhud` | `gate-blocked` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md) |
