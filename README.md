...
| `devhud` | Developer-tool shell whose signed-out base remains bundled and usable without an account. Its closed internal RealQA production tool is registered only on macOS, Windows, and Ubuntu, with authenticated entry, prior-bound offline encrypted capture/edit drafts, accessible capture-to-issue management, exact-origin native Connect/upload transport, and no iOS or Android entry. Complete change-scoped RealQA CI builds and signs only local fixture images and signed-host-ready desktop/extension artifacts; it has no publication, deployment, provisioning, registration, production-identity, or catalog authority. Deck's private API, server/provider slices, and client-owned refresh controller are implemented but remain unregistered; widgets, live native Deck transport, push delivery, catalog enablement, service deployment, production GitHub/Chrome identities, and operational rollout remain separately controlled by their canonical contracts. | `apps/devhud`<br>`servers/devhud-deck` (inactive implemented provider/refresh slice)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (inactive foundation/provider/submission slice)<br>`protos/devhud-realqa` | `active signed-out shell and closed internal desktop RealQA; implemented unregistered Deck refresh foundation; planned Deck UI and widgets` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
