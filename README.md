...
| `devhud` | Local-only React/TypeScript/Rsbuild and Tauri application with a closed internal registry, the exact pinned CEF runtime on desktop, maintained standard system-webview mobile hosts, and compile-only native widget foundations that are excluded from release registration and embedding. | `apps/devhud` | `active` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md) |
