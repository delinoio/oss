...
| `devhud` | Local-only React/TypeScript/Rsbuild and Tauri application with a production tray-resident shell on the exact pinned sandboxed desktop CEF runtime, bundled-only resources, deny-by-default networking, window-specific capabilities unchanged by DevTools, typed bounded diagnostics with explicit native export, maintained standard system-webview mobile hosts, and compile-only native widget foundations excluded from release registration and embedding. | `apps/devhud` | `active` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md) |
