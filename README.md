...
| `devhud` | Developer-tool shell whose implemented signed-out base remains bundled and usable without an account on the exact pinned sandboxed desktop CEF/mobile system-webview foundation; planned bounded authenticated features are Deck for desktop/mobile/native widgets and desktop-only RealQA with an exact-origin Chrome native host. The private RealQA Connect package and inactive Go/PostgreSQL/sqlc preset/tracker/auth/deletion server foundation are implemented, while submission, image, billing, provider, and client product integration remain planned. Bundled webviews remain offline; future feature RPC/upload access uses closed native transports, Deck manual refresh obtains a non-dispatching server-authoritative billing quote before confirmation, and delibase deletion jobs durably trigger both feature cleanup paths. Future service origins, separate GitHub Apps, and disabled catalog records are contracts only—not deployments, registrations, publications, or activations. | `apps/devhud`<br>`servers/devhud-deck` (planned)<br>`protos/devhud-deck` (planned)<br>`servers/devhud-realqa` (inactive foundation)<br>`protos/devhud-realqa` | `active base, RealQA API, and inactive server foundation; planned features` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
