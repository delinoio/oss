...
| `devhud` | Developer-tool shell whose implemented signed-out base remains bundled and usable without an account on the exact pinned sandboxed desktop CEF/mobile system-webview foundation. The private versioned Deck source contract, isolated descriptor, generated Go/TypeScript Connect package, structured shortcut inputs, and server-authored shortcut/widget state are implemented; Deck product clients/server and desktop-only RealQA remain planned. Bundled webviews remain offline; future feature RPC/upload access uses closed native transports, Deck manual refresh obtains a non-dispatching server-authoritative billing quote before confirmation, and delibase deletion jobs durably trigger both feature cleanup paths. Future service origins, separate GitHub Apps, and disabled catalog records are contracts only—not deployments, registrations, publications, or activations. | `apps/devhud`<br>`servers/devhud-deck` (planned)<br>`protos/devhud-deck`<br>`servers/devhud-realqa` (planned)<br>`protos/devhud-realqa` (planned) | `active foundation and Deck API; planned product features` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md), [Deck server](docs/servers-devhud-deck-foundation.md), [Deck API](docs/protos-devhud-deck-api-contract.md), [RealQA server](docs/servers-devhud-realqa-foundation.md), [RealQA API](docs/protos-devhud-realqa-api-contract.md) |
