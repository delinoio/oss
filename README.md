...
| `devhud` | Local-only React/TypeScript/Rsbuild and Tauri application with a production tray-resident desktop shell, closed empty registry, and mobile empty-state foundation, using the exact pinned upstream CEF runtime on desktop. | `apps/devhud` | `active` | [project-devhud](docs/project-devhud.md), [apps-devhud-foundation](docs/apps-devhud-foundation.md) |
