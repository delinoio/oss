...
Before every page injection, verify that Chrome still grants the exact configured origin permission; `activeTab` alone is insufficient. After interactive selection, refresh the current configuration before selecting the mapping from the complete captured URL so SPA navigation and concurrent settings changes cannot use a stale mapping, then discard that transient URL rather than sending it to the host. Abandoned interactive selections cancel and remove their page listeners after 30 seconds.
