...
Validate manifest permissions, stale-origin grant removal only after successful configuration refresh, extension ID/origin pairing, malformed/replayed/oversized messages including the shared 256 KiB pre-framing ceiling, no-active-tab and sensitive-form behavior, absent/denied/revoked permissions, unsupported incognito behavior, pointer-sequence isolation during selection, element/attribute allowlist enforcement, selector omission, path-segment redaction, URL-value redaction (including signed URLs and `data-*` secrets), sanitized DOM cap, reproducible ZIP parity, and Web Store packaging.
