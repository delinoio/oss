...
- Catalog: anonymous public app/meter listing and details, including public effective-dated USD micro-unit prices, their version metadata, and stable service authorization target UUIDs/safe names only for explicit service/meter `REALQA_STORAGE` capabilities, never for ordinary usage allowlists and never with Logto client IDs or credentials; no runtime mutation API.
...
- The wire wrappers are `UuidV7`, `UsdMicros`, and `UsageUnits`; protobuf-es represents both signed-int64 values as TypeScript `bigint`, while generated Go uses `int64`. Idempotent mutations carry `IdempotencyKey` and return `IdempotencyResult` alongside the original typed result so exact-payload replays are observable without changing the result. Keys are scoped to the authenticated user subject and operation for human RPCs, or the authenticated service identity and operation for M2M RPCs. Live-user usage digests also bind the forwarded user subject. Authorized reserve/commit/release digests bind authorization, authenticated service, purpose, resource, UTC period, and units, with reserve also binding `client_reference` and commit/release binding the reservation ID; resource-deletion digests bind authorization, service, purpose, resource, and expected revision. A reused key with any changed payload returns the shared `ERROR_REASON_IDEMPOTENCY_CONFLICT`; a different key that repeats an already recorded logically unique authorized period/reservation settlement returns `ERROR_REASON_BACKGROUND_USAGE_REPLAY_CONFLICT`. Invitation acceptance/revocation, background-authorization creation/revocation, and authorized reserve/commit/release/resource-deletion notification have distinct `IdempotentOperation` values; invitation creation does not carry idempotency fields.
