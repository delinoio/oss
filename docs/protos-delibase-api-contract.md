...
- The released `delibase.v1` live-usage wire contract has no single-reservation finalization grant: `ReserveUsage`, `CommitUsage`, and `ReleaseUsage` all require the forwarded user bearer. Planned Deck activation therefore remains blocked on a future synchronized additive protobuf/server change that returns an opaque grant from successful reservation and accepts it only for same-service, exact-reservation, at-most-reserved, pre-expiry idempotent commit/release with a fresh M2M bearer. The current source and generated artifacts must not claim that future capability before it is implemented and validated.
...
- A new `ReserveAuthorizedUsage` request accepts only a canonical period start for the current or immediately preceding UTC day according to server time, preventing arbitrary past/future quota buckets while permitting completed-day settlement. An exact completed reserve replay returns the original typed reservation before applying that period-window check, so an accepted response can be recovered after rollover. `CommitAuthorizedUsage` and `ReleaseAuthorizedUsage` use the period stored with their reservation. An authorized-usage access failure caused specifically by an `OWNER_DELETED` grant keeps the existing access-lost reason and includes diagnostic-safe `authorization_status=owner_deleted` error metadata so the bound service can wait for the authoritative lifecycle cutoff instead of treating it as an ordinary access revocation. `MarkBackgroundUsageResourceDeleted` lets only the authenticated bound service idempotently transition an `ACTIVE` matching grant to `RESOURCE_DELETED` when the expected revision is current. The same exact bound-service request returns the current closed status and revision without transition when the grant is already `REVOKED`, `ACCESS_LOST`, `RESOURCE_DELETED`, or `OWNER_DELETED`, including when the request's expected revision predates that closure; a future/unknown expected revision or any authorization/service/purpose/resource substitution fails closed.
