...
2. Determine whether API, sweeper, stores (`apple`, `google-play`, `chrome-web-store`), GitHub Release, updater, or `/devhud` is public. During a delayed or historical DevHud recovery, also verify that the deployed project documentation subpaths came from the fresh current-main tree rather than the release-bound candidate.
