...
PR CI deliberately defers the ten desktop and seven mobile packaging entries while retaining affected tests, static mobile/widget checks, deterministic frontend output, CEF pins, and OCI validation. Full native evidence comes from a related main push or unconditional manual CI dispatch. Consult the execution-plan summary: `CI Result` accepts only planned skips and fails on missing, failed, cancelled, or unexpectedly skipped validation.
...
2. Determine whether API, sweeper, stores (`apple`, `google-play`, `chrome-web-store`), GitHub Release, updater, or `/devhud` is public. During a delayed or historical DevHud recovery, also verify that the deployed package-local documentation subpaths came from the fresh current-main aggregate rather than the release-bound candidate.
