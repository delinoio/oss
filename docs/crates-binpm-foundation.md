...
- Implemented safe commands may perform read-only or bootstrap behavior when they do not violate storage or mutation contracts:
  ...
  - `binpm cache key` may print a deterministic key for the current target and project-root `binpm.lock`, using a visibly labeled missing-lockfile key with the empty lockfile digest when the file is absent.
...
- Stable source enum values:
  ...
  - `gitlab:<host>/<namespace...>/<project>[@version]` addresses GitLab Releases on GitLab.com, GitLab Self-Managed, or GitLab Dedicated hosts. GitLab always requires an explicit host; use `gitlab:gitlab.com/<namespace...>/<project>[@version]` for GitLab.com. `gitlab:group/project` is intentionally invalid because the first path segment is always the host.
...
- GitLab URL shorthands, arbitrary HTTP(S) URLs, registries, and package-manager backends are out of scope until documented separately. Recognizable unsupported URL input must fail with a precise canonical replacement hint instead of silently adding a new backend. Recognizable package-manager backend prefixes such as `npm:`, `pnpm:`, `yarn:`, `bun:`, `cargo:`, `cargo-binstall:`, `brew:`, `homebrew:`, `apt:`, and `rpm:` must fail with an explicit diagnostic that binpm v1 installs provider release assets, not package-manager backends.
- When `@version` is omitted, `binpm` must select the latest stable release exposed by the source provider:
  ...
  - GitLab sources must list releases in descending `released_at` order and choose the first release whose `released_at` is not in the future, whose API response does not set `upcoming_release = true`, and whose normalized tag does not contain a known SemVer prerelease identifier such as `-alpha`, `-beta`, `-pre`, `-preview`, or `-rc`. Non-SemVer stable tags, including date-style tags and stable tags with target suffixes such as `v1.2.3-linux-x64`, must remain eligible.
- Version selectors are intentionally exact-tag-only in v1:
  ...
  - When an exact tag misses but the available release list contains the same tag with only a leading `v` added or removed, diagnostics may suggest the exact alternative, such as retrying `@v14.1.1` or `@14.1.1`, but must not resolve by prefix fallback.
...
- Commands with both local and global scope must default to local when a local `binpm.toml` is discovered; otherwise they must default to global. Such commands must document `--local` and `--global` overrides. Source-form `binpm install <source>` is a global install form; before mutation, human output must print `install scope: global` and `install mode: global source install`. When a local `binpm.toml` is discovered, that output must also state that the project manifest is not modified and guide users to `binpm add <cmd> <source>` for project-local declaration. `binpm install <source> --local` is unsupported and must fail with guidance to use `binpm add <cmd> <source>`.
- `--frozen-lockfile` on local `binpm install`, `binpm update`, and `binpm x` must fail when the command would need to create or modify `binpm.lock`; `CI=true` must enable this behavior by default, and `--no-frozen-lockfile` is the explicit local-development escape hatch. Frozen mode is a lockfile write guard, not an offline or cache-only mode. Empty-manifest local updates that do not require lockfile changes must succeed without creating `binpm.lock` and must report that no tools are declared and no lockfile was created instead of printing file-change plans; JSON dry-run output must expose `no_op.reason = "empty_manifest_no_tools_no_lockfile_changes"`.
- Frozen-lockfile failures must include stable human and JSON diagnostics. The diagnostic must include `mode`, `reason`, `file`, `record`, `on_demand_install_attempt`, `would_change`, `safest_next_command`, and `local_development_escape_hatch` fields, distinguishing CI-enabled frozen mode (`mode = "CI=true"`) from explicit `--frozen-lockfile`, missing `binpm.lock`, missing target records, stale records, and `binpm x` on-demand executable or package-record sync attempts. Frozen restore download diagnostics must use `kind = "frozen_restore"` and expose `network_access_attempted`, `provider_authentication_attached`, `offline_or_cache_only = false`, `cache_state`, `cache_path`, `locked_asset_url`, and `release_list_pagination_attempted = false`.
- Frozen local install and `x` must restore missing `.binpm/bin` executables and `.binpm/packages` package records from existing target lock records when the global cache asset is present and SHA-256-valid for the locked digest. If the cache asset is absent or corrupt, frozen restore may download only the lockfile's persisted sanitized asset URL and must validate the recorded SHA-256 before populating cache or installing; this restore path must not require provider release-list pagination. Same-origin locked GitHub or GitLab provider asset URLs may receive runtime-only host-scoped provider authentication when configured, so private release assets and provider URLs that require authenticated redirects can be repaired without persisting credentials or expiring signed URLs. External locked asset URLs must not receive provider credentials.
- Scoped mutating commands must print the selected local or global scope before mutation in human mode. `binpm update` and scoped `binpm remove` must support `--dry-run` previews that validate the selected tools, print the selected scope and planned file/runtime changes in human mode, and then exit without mutating manifests, lockfiles, package records, cache references, or executables. When `binpm update` receives no command names, human output must state that all tools in the selected scope are targeted before mutation. `binpm update --global [cmd...]` must plan selected or all global package records without mutation in dry-run mode; non-dry-run updates must preserve each recorded command alias and selected binary, resolve the latest stable release for the recorded source, and finalize through the same global cache, install, rollback, and verification behavior as `binpm install <source> --as <cmd> --bin <selected_binary>`.
...
- Cache management commands for v1:
  ...
  - `binpm cache prune` must remove stale structured local-project cache references and then remove only cache entries that are not referenced by installed package manifests or by active references in the user-level local-project cache reference index under `~/.binpm/cache/refs`. A structured reference is stale when its project-local package record is absent or no longer points at the referenced cache key. Legacy plain-text references remain preserving until a future install, update, or removal in the referenced project rewrites them. Human and JSON output must report removed cache entries, removed stale structured refs, preserved legacy refs, removed asset boundaries, preserved package/executable/ref boundaries, and migration guidance for legacy refs.
  - `binpm cache clean` must remove all global cache asset entries under `~/.binpm/cache/sha256` while preserving `~/.binpm/cache/refs`, installed package records, and executable links or copies under `~/.binpm/bin`. Human and JSON output must state the removed asset-entry count, the removed asset boundary, and the preserved boundaries.
  - `binpm cache key` must print a stable CI cache key derived from the current target and `binpm.lock`; it must not download, install, modify package records, or populate cache entries. With a present `binpm.lock`, human output must remain a single concise cache key line. When `binpm.lock` is absent, human output must warn that the empty lockfile digest is used, label the printed key as `missing-lockfile`, and recommend `binpm install` as the next command. `--json` output must expose `lockfile = "missing"`, `status = "missing-lockfile"`, and `recommended_next_command = "binpm install"`.
...
- Non-dry-run `binpm remove` output must distinguish immediate state cleanup from later cache maintenance. Removal cleans selected manifest, lockfile, package-record, cache-reference, and executable state for the chosen scope, but it must preserve global cache asset entries; users should run `binpm cache prune` to remove now-unreferenced cache assets or `binpm cache clean` to remove all cached assets.
...
- `binpm update [cmd...] [--local|--global] [--dry-run]` must update selected tools or all tools in the selected scope to the latest stable release allowed by their source records. Local updates must update `binpm.lock` and installed project-local executables; when a selected local manifest record has an exact version, update must advance `binpm.toml` to the latest stable release tag so `outdated` no longer reports the same stale state after a successful update. Global updates must use existing global package records and update `~/.binpm/packages` and `~/.binpm/bin` through the global install finalization path while preserving command aliases and selected upstream binaries. Before mutation it must print `update scope: local` or `update scope: global`, an all-tools or selected-tools mode line, and a planned update list; with `--dry-run` it must print the same plan and leave state unchanged.
...
- `binpm explain <cmd-or-source> [--local|--global]` must explain source parsing, release selection, target normalization, asset candidate scoring, binary discovery, verification decisions, and target override remediation without mutating state. Human and JSON output must distinguish read-only operation from network-free operation.
- Source-form `binpm explain <source>` may perform read-only GitHub or GitLab release API lookup and must print the normalized source, provider API URL, release decision, skipped release tags and skip reasons, normalized target, selected asset when one is eligible, candidate scores, rejection reasons, and a valid `[tools.<cmd>.targets.<target-key>]` override snippet when inspected data can identify an asset candidate. Source-form explain is read-only but not network-free. Local command explanation must be read-only and network-free: it inspects existing package records and prints source, release, target, selected asset, selected binary, archive format, checksum source, verification state, and a valid exact override snippet without installing new bytes or contacting providers.
...
- Diagnostic commands `binpm list`, `binpm info`, `binpm outdated`, `binpm doctor`, `binpm explain`, `binpm verify`, `binpm cache list`, `binpm cache key`, and cache cleanup summary commands `binpm cache prune` and `binpm cache clean` must support `--json` as a stable machine-readable output mode. JSON mode must emit one compact JSON object to stdout on success and must avoid ANSI color in command output. Human output remains concise line-oriented text by default.
- Mutating commands `binpm install`, `binpm add`, `binpm update`, and `binpm remove` must also support `--json` as a stable final-result mode. This mode reports only the final mutation result, not progress or tracing. The success object must include `command`, `scope`, `dry_run`, `changed_files`, and `tools`. Tool entries must include `cmd`, `action`, canonical `source` when known, `requested_version`, `release_tag`, `selected_asset`, `selected_binary`, `installed_path`, `checksum_source`, and `verification` when those values are available for the mutation or dry-run plan. `update --dry-run --json` and `remove --dry-run --json` must return parseable plans and leave state unchanged. Mutating JSON mode must keep stdout to one compact JSON object; progress, human diagnostics, and tracing must stay on stderr or be suppressed so stdout remains parseable.
- JSON diagnostic envelopes must include a `command` field and stable command-specific arrays or records. Scoped diagnostics must include `scope` using the enum values `local` or `global`, and human output for scoped diagnostics such as `list`, `info`, `outdated`, `explain`, and `verify` must state the selected scope when scope affects results. Target fields must reuse the target enum values through a structured `target` object with `os`, `arch`, and `libc` fields. Checksum and verification fields must reuse the documented checksum source values (`github-digest`, `sidecar`, `manifest`, `signature`, `local`) and verification states (`verified`, `unverified`).
- `binpm explain <source> --json` must expose `read_only = true`, `network_free = false`, selected asset details, skipped releases with reasons, and all candidate asset decisions with parseable `score`, `eligible`, `recognized_pattern`, optional `cpu_feature`, and `rejection_reason` fields. `binpm explain <cmd> --json` must expose `read_only = true` and `network_free = true`. When release-level remediation is available, it must expose a parseable `release_diagnostics` array with structured target fields, unsupported installer asset names, source archive names when relevant, sidecar asset names when relevant, target mismatch asset names when relevant, and remediation text. Diagnostic `kind` values must distinguish `no-downloadable-assets`, `source-archive-only`, `unsupported-installers`, `target-mismatch`, `target-scoring-remediation`, and `gitlab-https-rejection`. `binpm verify --json` must expose successful verification checks and must use a parseable stderr error envelope with `error.message` and `error.exit_code` when verification fails before a success envelope can be produced.
- JSON output is a command output API for read-only diagnostics, cache cleanup summaries, and mutating final-result envelopes only. Mutating command progress logs and structured `tracing` logs are not stable machine-readable output contracts.
- Verification JSON diagnostics must expose unsupported verification sidecar asset names safely, without URLs, credentials, query strings, fragments, headers, or provider auth metadata. Strict verification failure envelopes must distinguish `missing_trusted_evidence` from `unsupported_sidecar_presence`.
- `binpm init` must create a minimal `binpm.toml` with `version = 1` at the project root when one does not already exist; it must not install tools by default. Before creating or refusing to overwrite the manifest, it must print the resolved full `binpm.toml` destination path. After a successful creation, it must print a clear created-manifest line with the same path. Existing manifests must not be overwritten, and `--force` is not a supported init flag.
- `binpm init` manifest creation root selection is fixed by default: current Git worktree root when available, otherwise the nearest ancestor containing `binpm.toml` when present, otherwise the current directory. `--manifest-path <PATH>` is the documented explicit destination escape hatch for creating a new manifest somewhere else, including nested current-directory workflows. The explicit destination must be named `binpm.toml` and must still refuse existing files.
- `binpm env [--shell <shell>] [--global|--local]` must print shell-specific environment commands for adding binpm-managed binary directories to `PATH`; it must not modify shell profiles by default. The printed global bin command must be labeled as suitable for shell profiles, while the project-local command must be labeled for the current project or shell session only. `--global` prints only the global bin command for explicit profile setup; `--local` prints only the project-local command for the current project or shell session.
- `binpm env setup --shell <shell> [--dry-run]` is the explicit opt-in profile mutation workflow for global PATH persistence. It must require `--shell`, preview the exact profile file and global `~/.binpm/bin` PATH line, apply only that global line when not in `--dry-run`, print rollback guidance, and remain idempotent when the exact line is already present. It must never persist project-local `.binpm/bin` paths. Supported setup shells are `bash`, `zsh`, `fish`, `powershell`, and `pwsh`; on Windows, `powershell` targets the Windows PowerShell profile while `pwsh` targets the PowerShell 7 profile, so PowerShell 7 setup guidance must tell users to pass `--shell pwsh`. `cmd` must be refused with guidance to use Windows Environment Variables. Setup must refuse ambiguous or unsupported profile targets such as missing/relative home directories, non-directory profile parents, symlinked profile files, existing non-regular profile paths, and Linux Bash homes that have an existing login profile but no `.bashrc` for non-login interactive shells.
- Supported `binpm env --shell` values are `bash`, `zsh`, `fish`, `powershell`, and `pwsh`. `PowerShell` is accepted case-insensitively for user ergonomics, and `pwsh` renders PowerShell syntax with its own profile target for setup. When `--shell` is omitted, binpm may infer `bash`, `zsh`, `fish`, `powershell`/`pwsh`, or `cmd` from `SHELL` or `ComSpec`; if inference fails, it must ask the user to pass `--shell`.
- `binpm env --shell cmd` is a recognized but explicitly deferred shell value; it must fail with an unsupported-shell diagnostic that lists supported shells, says cmd support is deferred, and gives a cmd.exe PATH story using the resolved global bin path, including any absolute `BINPM_HOME` override: run the printed `set "PATH=<global-bin>;%PATH%"` command for the current cmd.exe session or add that global bin path to the user PATH through Windows Environment Variables for persistence.
- After global installs, when `~/.binpm/bin` is not visible on `PATH`, binpm must print guided PATH setup messaging that points to `binpm env --global --shell <bash|zsh|fish|powershell>` for non-mutating output and keeps profile changes opt-in through `binpm env setup --shell <bash|zsh|fish|powershell|pwsh>`.
- After successful local `binpm add` installs, binpm must print `binpm x <cmd>` as the default project-local invocation path and point users who want direct shell access to `binpm env --local --shell <bash|zsh|fish|powershell>`. Existing commands must not mutate shell profile files or imply project-local `.binpm/bin` entries are suitable for profile persistence.
...
- `binpm install <source> [--as <cmd>] [--bin <upstream-binary>]` must install globally, even when a local `binpm.toml` exists. Without `--as`, the installed command name defaults to the source repository basename. With `--as`, global package records, executable paths, and removal keys use the explicit command alias while preserving the normalized source identity, requested version, release, asset, and selected upstream binary in the package record. With `--bin`, global installs use the explicit upstream executable name or archive member path to resolve ambiguous multi-binary archives without manual package-record edits. Human source-install output must print `install scope: global`, `install mode: global source install`, and the normalized source before mutation. If a project manifest is detected, it must state that the manifest is not modified and point to `binpm add <cmd> <source>`. Successful human install output must print the installed command alias and selected upstream binary as distinct fields, and global installs whose alias differs from the selected upstream binary basename must tell users to choose aliases with `--as <cmd>` and upstream binaries with `--bin <upstream-binary>`. Passing `--local` with a source must be rejected with a clear `binpm add <cmd> <source>` hint.
...
- `binpm install` without a package spec must print `install scope: local` and `install mode: local manifest sync` before mutation, then sync the local `binpm.toml` manifest into `$repoRoot/.binpm/bin` and update `binpm.lock`; `binpm install <source>` keeps the global install behavior and must not accept `--local`.
...
- `binpm x --package <source> [--bin <upstream-binary>]` without `CMD` is the safe one-off shortcut only. It must keep the source explicit and must not perform package search or infer a repository from a command name. Without `--bin`, the exposed command defaults to the source repository basename and normal archive binary discovery can still fail safely with candidate retry suggestions. With `--bin`, the exposed command defaults to the selected upstream binary basename. The shortcut form must not accept forwarded args; when forwarded args are needed, users must provide an explicit `CMD` before those args, such as `binpm x --package <source> <cmd> -- <args...>`.
...
- If `CMD` is absent from the local manifest and no explicit `--package` is provided, `binpm x` and its execution aliases must fail with a clear hint that binpm will not infer a source repository from the command name. The diagnostic must point to explicit remediation with `binpm add <cmd> <source>` or one-off execution with `binpm x --package <source> <cmd>`.
...
- Asset selection must be score-based, deterministic, and stable across identical release asset lists:
  ...
  - On Linux `musl` hosts, missing-libc candidates must not be accepted unless the asset has an explicit `static`, `portable`, `universal`, or `any` signal; otherwise resolution must fail instead of installing a likely glibc-linked binary. Rejection diagnostics must name the rejected missing-libc assets, say that a Linux musl target requires an explicit libc or portability signal, and provide a safe remediation path: ask upstream to rename or publish `musl`/`static`/`portable`/`universal`/`any` assets, inspect the binary outside binpm with tools such as `file` and `ldd` or `readelf`, and use a canonical `[tools.<cmd>.targets.<target-key>]` override only after verifying musl or static compatibility.
...
- Source archives and GitLab generated `assets.sources` entries may appear in explain candidate diagnostics as rejected `source-archive` artifacts, but they must never be selected automatically or by manifest target override as installable assets. When the visible release downloads are source archives plus optional sidecars only, `binpm explain` and install-time selection failures must report `source-archive-only`, list the ignored source archive names when safe, and ask upstream for target-specific portable binary archives or bare executables instead of describing the release as empty or as a generic target mismatch.
- When all visible release assets are unsupported desktop or system packages, `binpm explain` and install-time selection failures must distinguish that installer-only boundary from a release with no assets. They must list the unsupported installer asset names and suggest upstream portable archive or bare executable assets instead of implying target scoring could not find any release asset. When installable archives or bare executables exist but none match the requested target, diagnostics must report `target-mismatch` and list the mismatching asset names instead of conflating the failure with no assets or source-archive-only releases.
...
- If archive member permissions are missing and filename/target signals do not identify one binary, `binpm` must fail with an actionable diagnostic instead of guessing. If an archive contains multiple plausible executables, `binpm` must prefer a binary whose basename matches the repository name; otherwise it must fail with an ambiguity error that lists candidates and includes concrete retry suggestions such as `binpm install <source> --as <cmd> --bin <candidate>`, `binpm add <cmd> <source> --bin <candidate>`, or `binpm x --package <source> --bin <candidate> <cmd>`. The same diagnostic must mention repeated `--also <cmd=upstream-binary>` values for local multi-binary archives while preserving separate `[tools.<cmd>]` manifest tables.
...
- Target-specific manifest overrides must use `[tools.<cmd>.targets.<target-key>]` tables with canonical target keys such as `linux-x86_64-gnu`; alias keys such as `linux-amd64-glibc` are invalid in persisted manifests even though aliases are accepted while scoring release asset names. Each override must include `asset = "<release-asset-name>"`, `bin = "<asset-member-or-bare-binary>"`, and may include `checksum_source = "<checksum-source>"` when the automatic checksum source must be overridden. Override validation must reject non-installable asset kinds with a diagnostic that tells the user to choose an archive or bare executable.
- Override snippets emitted by diagnostics must use canonical target keys, TOML-escaped command keys and string values, exact release asset names from inspected release metadata, and exact selected binary paths when the local package record already knows them. Source-form explain must not include credential-bearing URLs, query strings, fragments, expiring download URLs, absolute cache paths, or other transient machine-local fields in snippets. Source-form snippets that are based only on release metadata must be labeled unverified because `binpm explain <source>` has not downloaded the archive, inspected archive members, or verified modern CPU compatibility. When source-form explain has not downloaded an archive and therefore cannot inspect archive members, the snippet may use the repository command name as the `bin` value and the user must adjust it if the archive member differs.
...
## Storage
- Global home directory: `~/.binpm`. This is a fixed binpm-owned home, not an XDG cache/data split. `BINPM_HOME` may override it for tests, isolated automation, or portable environments, but the override must be an absolute path.
...
- Legacy cache-reference migration is guided, not destructive: `binpm cache prune` and `binpm doctor` report legacy reference counts, preserve those references, and tell users to run a local install, update, or removal in the affected projects to rewrite them as structured references.
...
- The current implementation records downloaded assets with `checksum_source = "local"` only when provider digests, upstream checksum sidecars, upstream checksum manifests, and supported package signature verification are unavailable.
...
- A fully downloaded and SHA-256-recorded cache asset may be preserved after post-download install finalization fails, including archive binary-selection failures. That preserved entry may be reused only after the retry validates the asset with the strongest available integrity evidence: provider digest, upstream checksum material, successfully verified signature, or locally recorded SHA-256 metadata.
...
- Committed `binpm.lock` target records must preserve deterministic resolution metadata only: package spec, normalized source, source provider, source host, source path, requested version when present, release tag, asset name, sanitized canonical asset URL, target OS, target architecture, target libc/ABI, archive format, selected binary path inside the archive or bare asset, installed binary path, SHA-256 digest, checksum source, whether upstream signature material was available, whether signature verification succeeded, and unsupported verification sidecar metadata when present.
...
- Release lookup errors must distinguish missing authentication, insufficient permissions, and rate limiting. Private or hidden repositories returning `401`, `403`, or `404` without a configured token must report missing authentication and print the exact expected environment variable name for explicit GitHub Enterprise and self-managed GitLab hosts, such as `BINPM_GITHUB_TOKEN_GHE_2E_EXAMPLE_2E_COM` for `github:ghe.example.com/owner/repo` and `BINPM_GITLAB_TOKEN_GITLAB_2E_INTERNAL_2E_EXAMPLE` for `gitlab:gitlab.internal.example/group/project`. The same statuses with a configured token must report insufficient permissions. `429` responses, GitHub/GitLab error responses with remaining rate-limit quota `0`, and equivalent rate-limit error responses must report rate limiting.
- JSON-mode release lookup diagnostics must expose safe auth setup data as fields, including `expected_auth_env_var`, `expected_auth_env_vars`, and `configured_auth_env_var`, without exposing token values.
...
- Detected unsupported verification sidecars such as `.asc`, `.sig`, `.minisig`, raw Sigstore metadata, certificate files, attestations, SBOMs, and provenance files must be reported separately from accepted provider digest, checksum, manifest, or verified signature evidence. Human install and verify output may name the unsupported sidecars, and JSON diagnostics must expose only safe sidecar asset names and stable sidecar kinds.
...
- `--require-verified` and `binpm verify --require-verified` must fail unless the selected asset has at least one trusted verification source: provider digest, upstream checksum sidecar, upstream checksum manifest, or successfully verified signature under the documented package trust policy. The failure diagnostic must distinguish releases with no trusted evidence from releases that contain only unsupported sidecars.
...
- The first-party binpm release matrix is distinct from the broader target parser used for third-party package asset scoring. Target parser support for values such as `freebsd`, `i686`, and `armv7` must not imply first-party binpm prebuilt artifact availability.
...
- Direct installers must remain available at `scripts/install/binpm.sh` and `scripts/install/binpm.ps1`; direct installs must verify `SHA256SUMS` entries and Sigstore bundle sidecars via `cosign verify-blob --bundle`, and must fail before artifact download when `cosign` is missing from `PATH`.
- Direct installers must detect unsupported operating systems and unsupported CPU architectures, including x86 hosts, before release tag resolution or artifact download. Unsupported-host diagnostics must include the detected OS and architecture, list supported direct-install targets (`darwin/amd64`, `darwin/arm64`, `linux/amd64`, `linux/arm64`, `windows/amd64`, and `windows/arm64`), distinguish the missing direct-installer artifact from general binpm project support, and recommend using a supported x64/arm64 host or CI image, Homebrew or cargo-binstall where supported, or a source build. These diagnostics must not include release lookup URLs, artifact download URLs, query strings, fragments, credentials, or tokens.
...
- Homebrew release automation must validate that binpm formula URLs point to the expected prebuilt archive names before rendering or pushing tap updates.
- Homebrew installation must consume prebuilt `binpm` release archives for `darwin/amd64`, `darwin/arm64`, `linux/amd64`, and `linux/arm64`; the formula is prebuilt-only and must not add a source build fallback without a new distribution contract.
- `cargo-binstall` metadata must resolve only first-party GitHub Release assets and disable `quick-install` and `compile` strategies; unsupported cargo-binstall targets must fail rather than using third-party binary indexes or source compilation.
...
- Direct-install documentation must make `cosign` a prerequisite before remote installer commands, link to official installation guidance, include latest and pinned first-party raw GitHub installer command patterns, and describe missing `cosign` as a prerequisite failure rather than a verification bypass opportunity.
...
- Does not integrate with npm, pnpm, yarn, Bun, Cargo install, cargo-binstall, Homebrew, apt, rpm, or system package managers as install backends in v1. Source-like inputs that name those backend prefixes must be rejected with explicit unsupported-backend diagnostics rather than interpreted as release sources or command names.
