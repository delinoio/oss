...
- Commands with both local and global scope must default to local when a local `binpm.toml` is discovered; otherwise they must default to global. Such commands must document `--local` and `--global` overrides. Source-form `binpm install <source>` is a global install form; `binpm install <source> --local` is unsupported and must fail with guidance to use `binpm add <cmd> <source>` for project-local declaration.
...
- Supported `binpm env --shell` values are `bash`, `zsh`, `fish`, and `powershell`. `PowerShell` is accepted case-insensitively for user ergonomics, and `pwsh` is accepted as an alias that renders PowerShell syntax. When `--shell` is omitted, binpm may infer `bash`, `zsh`, `fish`, `powershell`/`pwsh`, or `cmd` from `SHELL` or `ComSpec`; if inference fails, it must ask the user to pass `--shell`.
...
- After global installs, when `~/.binpm/bin` is not visible on `PATH`, binpm must print guided PATH setup messaging that points to `binpm env --global --shell <bash|zsh|fish|powershell>`, says persistent profile changes are opt-in, and tells users to persist only the global bin line.
...
- `binpm install` without a package spec must sync the local `binpm.toml` manifest into `$repoRoot/.binpm/bin` and update `binpm.lock`; `binpm install <source>` keeps the global install behavior and must not accept `--local`.
