...
- After global installs, when `~/.binpm/bin` is not visible on `PATH`, binpm must print guided PATH setup messaging that points to `binpm env --shell <bash|zsh|fish|powershell>` and says persistent profile changes are opt-in.
