...
- `binpm x CMD [args...]` must resolve `CMD` from `binpm.toml`, install it on demand when the lockfile or local executable is missing or stale, prepend `$repoRoot/.binpm/bin` to `PATH`, preserve the caller's current working directory, and forward every argument after `CMD` to the executed command. When this on-demand install is blocked by frozen mode, including the default `CI=true` frozen mode, the diagnostic must say that `binpm x` attempted on-demand sync and point to `binpm install --local` as the safest next command before retrying execution.
...
- `binpm add <cmd> <source> --manifest-only` must preserve the same manifest serialization rules as normal add while skipping all lockfile and runtime installation side effects. This is an advanced declaration-only review workflow, not the default add path. Until `binpm install` runs, `binpm list --local` and `binpm doctor` must identify affected tools as declared-but-not-installed and include the install follow-up.
...
- Direct installers must remain available at `scripts/install/binpm.sh` and `scripts/install/binpm.ps1`; direct installs must verify `SHA256SUMS` entries for the selected artifact and must not require `cosign`, download artifact Sigstore sidecars, or run `cosign verify-blob`.
...
- Direct-installer verification applies to binpm's own release artifact and is checksum-only; it must not be described as implementing signature verification for packages installed by binpm.
...
- Direct-install documentation must describe checksum verification through `SHA256SUMS`, include latest and pinned first-party raw GitHub installer command patterns, and keep binpm release verification separate from package verification.
