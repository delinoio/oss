...
- `binpm env --shell cmd` is a recognized but explicitly deferred shell value; it must fail with an unsupported-shell diagnostic that lists supported shells, says cmd support is deferred, and gives a cmd.exe PATH story: use `set "PATH=%USERPROFILE%\.binpm\bin;%PATH%"` for the current cmd.exe session or add `%USERPROFILE%\.binpm\bin` to the user PATH through Windows Environment Variables for persistence.
