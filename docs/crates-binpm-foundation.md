...
- `binpm doctor` must inspect manifest discovery, lockfile readability, package records, cache state, cache-reference stale/legacy counts, installed executable records, PATH visibility, and provider configuration without mutating state. When `~/.binpm/bin` is not visible on `PATH`, doctor must print guided setup messaging that points to `binpm env` and keeps profile modification opt-in.
