...
- After global installs, when `~/.binpm/bin` is not visible on `PATH`, binpm must print guided PATH setup messaging that points to `binpm env --global --shell <bash|zsh|fish|powershell>` for non-mutating output and keeps profile changes opt-in through `binpm env setup --shell <bash|zsh|fish|powershell>`.
