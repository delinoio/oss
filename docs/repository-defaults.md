...
- Root `pnpm dev` is reserved for the DevHud team workflow. Documentation apps retain root entry points through `pnpm dev:public-docs`, `pnpm dev:nodeup-docs`, `pnpm dev:binpm-docs`, and `pnpm dev:runmoor-docs`; package-local development continues to use `pnpm dev`. Every development command uses its app's documented fixed loopback host and port, prevents CLI address overrides from changing that binding, and fails when that port is occupied rather than automatically selecting another port. Development wrappers forward termination signals to the child server and wait for it to exit so stopped commands do not leave orphaned listeners.
