...
- Root `pnpm exec vp run dev` is reserved for the DevHud team workflow. Documentation apps retain root entry points through `pnpm exec vp run dev:public-docs`, `pnpm exec vp run dev:nodeup-docs`, and `pnpm exec vp run dev:binpm-docs`; package-local development continues to use `pnpm exec vp run dev`. Every development command uses its app's documented fixed loopback host and port, prevents CLI address overrides from changing that binding, and fails when that port is occupied rather than automatically selecting another port. Development wrappers forward termination signals to the child server and wait for it to exit so stopped commands do not leave orphaned listeners.

## Repository task execution

Use the pinned local Vite Task runner (`pnpm exec vp run`) for workspace orchestration, following `repository-task-contract.md`. Package installation remains pnpm-owned; application bundlers and test tools remain package-owned.
