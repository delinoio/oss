...
Source-replacing exports record the document ID, revision, original fingerprint and output fingerprint in a durable journal before publication. Under the document lock, the next operation reconciles an interrupted export only when the source still matches either its original bytes or that exact managed revision. It repairs fingerprint persistence without advancing the revision; other source changes remain conflicts. Cancellation or a failed state-pointer write after publication therefore remains recoverable after restart.

Atomic file publication syncs file bytes before rename, then syncs the destination parent directory on Unix. Windows uses same-volume `MoveFileExW` with `MOVEFILE_WRITE_THROUGH` and explicit replacement policy. Thus journal publication returns only after its directory entry is flushed, before source replacement may begin; flush failures propagate. Recovery tests cover interrupted publication states, not physical power-cut certification. See [fsync directory durability](https://www.man7.org/linux/man-pages/man2/fsync.2.html) and [Windows move guarantees](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-movefileexw).
