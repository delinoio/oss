...
- Native binaries and npm expose the same `wait tcp HOST:PORT`, `wait http URL`, and `wait file PATH` interfaces from [the Rust contract](crates-clibox-foundation.md), including unlimited default waiting, bounded network attempts, human/quiet/JSON output, redacted errors, and exit codes 0/1/2/130/143. No launcher-side parsing or network implementation is added. #916 utilities and the seven #917 transformations remain implemented alongside readiness waits.
...
- CI's Linux/macOS/Windows `node-clibox-test` runs `cargo test --locked -p clibox -p clibox-system -p clibox-transform -p clibox-wait` for unit/process/adapter and readiness coverage and participates in the shared change planner and `CI Result` aggregation. Release CI builds, runs Rust command/process/adapter tests, and smoke-tests all eight targets; Linux musl execution is also checked in Alpine.
- Native CI/release tests select `clibox`, `clibox-system`, `clibox-transform`, and `clibox-wait`. Changes to any of these directories select clibox consumer/native checks, and all four are explicit inputs to the package-owned Turbo tests.
