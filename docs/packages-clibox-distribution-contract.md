...
- Release verification requires exactly nine expected packages with matching source version, revision, metadata, and computed SHA-512 integrity. Publish platform packages first and confirm each registry integrity before publishing the main package. Existing identical versions are reused; conflicting versions fail without overwriting. After a successful upload, make at most 61 registry metadata checks with five-second delays and a 30-second timeout per request, emitting structured pending-confirmation events. A transient missing version never causes a second upload within that attempt, and an observed integrity conflict fails immediately.
