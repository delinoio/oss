...
- Preserve command-specific native help on stderr and exit code 2 when `run`, `port`, `clipboard`, `wait`, `text`, `time`, `base64`, or `hash` is missing a subcommand. Root no-argument and explicit help remain successful stdout output; the launcher adds no argument parsing or help rendering.
...
Windows console Ctrl+C and Ctrl+Break are broadcast to both Node and the attached native child. The launcher handles `SIGINT`/`SIGBREAK` by awaiting native completion so cleanup and numeric status (130 for configuration, 1 for transformations) survive. It must not call Node's kill API for these console events because that forcibly terminates Windows children; [Node documents this platform behavior](https://nodejs.org/api/child_process.html#subprocesskillsignal). Unix `SIGINT`, `SIGTERM`, and `SIGHUP` remain explicitly forwarded. Forced process termination and console teardown cannot promise graceful cleanup.
...
- `pnpm --filter @delino/clibox test:package` builds the host CLI with `cargo build --locked --release`, creates tarballs, and installs them in temporary npm and pnpm consumers with scripts disabled. Use the stripped release profile so debug symbols cannot overflow the archive reader's 64 MiB decompressed-size bound. Release matrix calls that supply `--binary` use that exact prebuilt artifact without rebuilding or modifying it. Installed-launcher checks exercise JSON file readiness, environment execution, empty argv and delegated status, all seven issue #917 commands, binary bytes, timezone output, and checksum verification as well as help/version and missing-subcommand help on stderr with exit code 2. Help assertions use the selected target's binary name, including `clibox.exe` on Windows.
...
- Native CI/release tests select `clibox`, `clibox-config`, `clibox-system`, `clibox-transform`, and `clibox-wait`. Changes to any of these directories select clibox consumer/native checks, and all five are explicit inputs to the package-owned Turbo tests.
