...
- CI's Linux/macOS/Windows `node-clibox-test` runs `cargo test --locked -p clibox` for unit/process/adapter and readiness coverage and participates in the shared change planner and `CI Result` aggregation. Release CI builds, runs Rust command/process/adapter tests, and smoke-tests all eight targets; Linux musl execution is also checked in Alpine.
...
`Release Project` synchronizes the Cargo and npm source versions in one version-only commit, validates that exact release source, publishes crates.io, and pushes the matching tag without waiting for main CI. Main CI runs independently; native build, test, and package validation remain enforced by the downstream workflow. `release-clibox.yml` accepts the exact version tag or a manual dry run. Publication requires the exact tag at the selected source commit and a matching crates.io version. The guarded publish job explicitly installs npm `11.6.2` before checking the OIDC minimum of `11.5.1` and publishing; Node.js 24's bundled npm is not the publication version contract.
