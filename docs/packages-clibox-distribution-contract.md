...
- CI's `node-clibox-test` runs `cargo test --locked -p clibox` unit/process/adapter, readiness, and configuration-conformance and transformation tests on Linux, macOS, and Windows, and participates in the shared change planner and `CI Result` aggregation. Release CI runs Cargo unit/process tests, builds, and smoke-tests all eight targets; Linux musl execution is also checked in Alpine.
...
- Isolated Windows console integration drives the Node launcher and real native configuration/transformation commands with Ctrl+C and Ctrl+Break, checking numeric status, destination preservation, and staging cleanup. The test owns its console and requires no interactive runner session. It runs in repository native CI/release tests; standalone crates omit this npm-source-dependent integration.
