...
- `pnpm --filter @delino/clibox test:package` builds the host CLI with `cargo build --locked --release`, creates tarballs, and installs them in temporary npm and pnpm consumers with scripts disabled. Use the stripped release profile so debug symbols cannot overflow the archive reader's 64 MiB decompressed-size bound. Release matrix calls that supply `--binary` use that exact prebuilt artifact without rebuilding or modifying it. Installed-launcher checks exercise JSON file readiness, environment execution, empty argv and delegated status, all seven issue #917 commands, binary bytes, timezone output, and checksum verification as well as help/version.
