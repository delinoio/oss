...
- Public documentation: `apps/public-docs/docs/runmoor` at `https://oss.delino.io/runmoor`.
...
- [Integrated Runmoor documentation section](apps-runmoor-docs-foundation.md).
...
- Runmoor public guides are owned by the shared Public Docs Rspress app and deployed as Cloudflare Pages static output. Stable routes are `/runmoor`, `/runmoor/install`, `/runmoor/configuration`, `/runmoor/commands`, `/runmoor/docker`, `/runmoor/tart`, and `/runmoor/operations`. The old `runmoor.delino.io` host is redirect-only.
- Integrated documentation validation preserves the credential and private-path publication safeguards previously applied by the standalone app, while retaining public configuration placeholders and user-facing storage guidance. See `docs/apps-runmoor-docs-foundation.md`.
- Development uses the Public Docs fixed port `46302` through `pnpm dev:public-docs`; the root entry point does not require the DevHud team environment.
