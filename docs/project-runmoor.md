...
- Public documentation: `apps/runmoor-docs`, assembled at `https://oss.delino.io/runmoor`.
...
- Runmoor public guides are owned by the Rspress app and assembled under `/runmoor` by `public-docs`. Stable package-local routes are `/`, `/install`, `/configuration`, `/commands`, `/docker`, `/tart`, and `/operations`; the canonical public routes use the `/runmoor` prefix.
