...
- PostgreSQL constraints, transactions, and organization-scoped locks enforce append-only ledger and audit behavior, validated running balances, serialized team hierarchy moves, organization-scoped billing references, non-overlapping effective price/billing-period windows, effective catalog price and meter-TTL reservation snapshots, immutable reservation transitions, held credit/overage commit splits, and concurrent reservation limits against available credits plus configured overage. Active reservations retain live organization/team/membership references; finalized usage history retains snapshots without blocking operational deletion.
