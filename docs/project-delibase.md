...
- Repository-shared packages under `servers/internal` validate and context-bind Logto user/M2M authentication, require explicit fail-closed route policies, rotate cached JWKS keys without accepting unsafe redirect targets, strip raw credential headers, propagate request/trace IDs across unary and streaming failures, map authentication failures to vetted stable reasons while treating unavailable signing keys as a retryable dependency failure, enforce conservative HTTP handler/CORS defaults under partial configuration, recursively redact diagnostic containers, emit allowlisted shape-validated pseudonymous `slog` events, and generate UUID v7 IDs. They do not own or infer delibase authorization or billing state.
