...
- Repository-shared packages under `servers/internal` validate and context-bind Logto user/M2M authentication, require explicit fail-closed route policies, rotate cached JWKS keys, strip raw credential headers, propagate request/trace IDs, map safe errors, preserve conservative HTTP/CORS defaults under partial configuration, recursively redact diagnostic containers, emit allowlisted shape-validated pseudonymous `slog` events, and generate UUID v7 IDs. They do not own or infer delibase authorization or billing state.
