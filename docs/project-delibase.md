...
- PostgreSQL constraints, transactions, and organization-scoped locks enforce at least one Owner for every committed organization, append-only ledger, reservation, and audit behavior, validated running balances, serialized team hierarchy moves, organization-scoped billing references, non-overlapping effective price/billing-period windows, valid invitation acceptance state, effective catalog price and meter-TTL reservation snapshots, storage-derived active reservation creation times, immutable usage-record-driven reservation commits, immutable integration-outbox event bodies and deletion-job targets, held credit/overage splits, and concurrent reservation limits against available credits plus active-subscription current-period configured overage. Deleting organizations cannot accept new reservations. Active reservations retain live organization/team/membership, service-allowlist, and Polar-meter references; finalized usage history retains snapshots without blocking operational deletion. Webhook inbox rows retain bounded validated payloads for retries, while immutable audit rows accept only empty system or pseudonymous actor references and allowlisted safe request metadata.
