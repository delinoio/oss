...
- PostgreSQL constraints, transactions, and organization-scoped locks enforce append-only ledger and audit behavior, validated running balances, serialized team hierarchy moves, organization-scoped billing references, non-overlapping effective price/billing-period windows, valid invitation acceptance state, effective catalog price and meter-TTL reservation snapshots, immutable usage-record-driven reservation commits, held credit/overage splits, and concurrent reservation limits against available credits plus current-period configured overage. Active reservations retain live organization/team/membership, service-allowlist, and Polar-meter references; finalized usage history retains snapshots without blocking operational deletion.
