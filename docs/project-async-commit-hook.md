...
Release preparation is available through `Release Project` with project `async-commit-hook`: it synchronizes all seven source/installer version fields and creates the exact version tag. Signing, GitHub Release, and Homebrew remain separate manual operations through the existing release workflow; documentation validation stays in that workflow, while publication is owned by the consolidated `public-docs` pipeline. See the release contract for exact-commit validation and recovery boundaries.
