...
- Nodeup, binpm, Runmoor, async-commit-hook, clibox, and pnport are major projects exposed through same-origin navigation to `/nodeup`, `/binpm`, `/runmoor`, `/async-commit-hook`, `/clibox`, and `/pnport`. Their Markdown is owned directly by `apps/public-docs/docs/{nodeup,binpm,runmoor,async-commit-hook,clibox,pnport}`, while this project builds and publishes the complete tree. pnport remains explicitly unreleased until its complete native gate passes.
...
- `public-docs` uses Rspress clean URLs, builds the six project content roots below their canonical subpaths, and publishes the complete `doc_build` tree to the single `public-docs` Cloudflare Pages project.
