...
- Package-local `pnpm exec vp run dev` and root `pnpm exec vp run dev:public-docs` bind to loopback on fixed port `46302`, reject host overrides, preflight that exact port, and fail on conflicts without automatic remapping.
