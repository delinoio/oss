...
- Nodeup, binpm, Runmoor, async-commit-hook, and clibox are major projects exposed through same-origin navigation to `/nodeup`, `/binpm`, `/runmoor`, `/async-commit-hook`, and `/clibox`. Their Markdown is owned directly by `apps/public-docs/docs/{nodeup,binpm,runmoor,async-commit-hook,clibox}`, while this project builds and publishes the complete tree.
...
- `public-docs` uses Rspress clean URLs, builds the five project content roots below their canonical subpaths, and publishes the complete `doc_build` tree to the single `public-docs` Cloudflare Pages project.
