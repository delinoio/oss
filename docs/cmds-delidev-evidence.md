...
| Workers, executables, outbound jobs, processes, updates, services | Durable outbound job API, instance leases and uncertainty on replacement implemented; explicit foreground Worker loop, private pairing and execution journals, capped reconnect and CLI commands implemented; native process ownership with a durable start barrier, separate interactive streams and Git integration implemented; harness discovery/execution, updates and services pending | Real Connect pairing/dispatch/report/revocation tests; CLI pairing to a real Worker and real temporary Git inspection; persisted completion reuse and interrupted-journal uncertainty; no real harness execution |
