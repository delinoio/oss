...
- Default PR result rows contain repository, number, title, author,
  individual/team reviewer identities, review decision, checks, mergeability,
  explicit open/closed/merged lifecycle state, independent draft state,
  updated time, the synchronized PR revision required by mutations, and the
  currently supported mutations/available merge methods. Reviewer identities
  make reviewer grouping deterministic, and action metadata is server-authored
  before a client constructs a mutation. Additional provider metadata is
  fetched only for the action that needs it.
