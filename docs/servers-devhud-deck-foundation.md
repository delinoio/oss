...
- Default PR results/details contain repository, number, title, author, review decision, checks, mergeability, draft state, updated time, and the synchronized PR revision required by mutations. Additional provider metadata is fetched only for the action that needs it.
