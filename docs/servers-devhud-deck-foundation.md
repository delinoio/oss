...
- Each widget selects one view. Supported families are WidgetKit small/medium/large for iPhone and through macOS's iPhone-widget availability, plus responsive Android compact/wide/list. New widgets show counts only; repository and PR titles require per-widget privacy opt-in. Device registration/update accepts only widget identity, selected view, family, and privacy configuration.
