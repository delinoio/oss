...
`.github/workflows/CI.yml` validates every implemented DevHud path with read-only jobs selected before runner allocation by a shared change plan. Across PR and main execution it exercises schema and generated freshness; Go, Rust, frontend, native-host, desktop, mobile/widget, extension, security, adapter, updater, installer, supply-chain, OCI, public-route, and release fixtures. The desktop matrix covers macOS, Windows, and Ubuntu x64/arm64 where GitHub-hosted runners and cross-build tooling make the package feasible; Linux includes X11 smoke. The mobile matrices cover the production iOS/Android architectures and their simulator/emulator paths. Exact CEF pins are checked before native work.

PRs retain affected validation, including the complete frontend test command, native-script fixtures, clean desktop/mobile frontend output checks, static mobile/widget contracts, immutable CEF pins, and OCI checks. Relevant main pushes run eight Windows/Linux desktop package entries and four Android entries; macOS desktop packages and the three iOS native builds are deferred. Unconditional manual dispatch runs all ten desktop, three iOS, and four Android entries. The signed private candidate requires both macOS desktop packages, signed iOS arm64, and separate arm64/x64 simulator builds before assembly. CI configuration changes force checks within the event's mode; path-rule edits force only changed jobs. No nightly schedule is added.

Use the execution-plan summary to distinguish intentionally deferred native packaging from missing validation. `CI Result` rejects failed or cancelled dependencies, unauthorized matrices, and unexpected skips. A green PR establishes the PR validation boundary only; inspect the exact signed private candidate or manually dispatch full CI before citing macOS/iOS native evidence. See `docs/repository-workflow-contract.md` for comparison ranges, successful-main-only cache writes, and performance measurements.
...
The immutable desktop pin gate includes the x64 and arm64 sharun 3.0.0 AppImage launcher assets and their exact SHA-256 digests. Every AppImage build downloads the selected asset, verifies it in memory, and stages the verified executable in a private temporary directory, and injects an app-owned AppImage file mapping for `sharun` before packaging. The upstream packager preserves that existing executable; its separate helper-library archive retains its own checksum validation. Ambient `SHARUN_LINK` and `SKIP_INTEGRITY_CHECKS` overrides are removed, and staging is deleted after the packaging child finishes or fails. No remote launcher URL reaches the packaging subprocess.
...
3. Treat the private jobs as the exact evidence pipeline: `plan`, `preflight`, `desktop`, `ios-simulators`, `extension`, `mobile`, `oci`, and `assemble`. Both simulator rows must succeed before `assemble`; they require no signing credentials and produce no public artifact. The complete artifact is named `devhud-v<version>-private-signed-candidate-<sha>-<run_id>-<run_attempt>` and is retained briefly for recovery.
