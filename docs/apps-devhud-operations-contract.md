...
PRs retain affected validation, including the complete frontend test command, native-script fixtures, clean desktop/mobile frontend output checks, static mobile/widget contracts, immutable CEF pins, and OCI checks. The ten-entry desktop and seven-entry mobile packaging matrices run on relevant main pushes or unconditional manual dispatch. CI configuration changes force checks within the event's mode; no nightly schedule is added.

Use the execution-plan summary to distinguish intentionally deferred native packaging from missing validation. `CI Result` rejects failed or cancelled dependencies and unexpected skips. A green PR establishes the PR validation boundary only; inspect the related main run or manually dispatch full CI before citing native packaging evidence. See `docs/repository-workflow-contract.md` for comparison ranges, successful-main-only cache writes, and performance measurements.
...
The `docs_candidate` artifact is intentionally limited to the release-bound `/devhud` page, its route assets, and shared static assets. `public_docs` checks out current `main`, rebuilds the complete consolidated documentation tree, and overlays only those DevHud-owned files before the Cloudflare Pages deployment. This keeps delayed or historical DevHud recovery from publishing stale Runmoor, Nodeup, binpm, or async-commit-hook subpaths.
