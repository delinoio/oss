...
CI never builds a signed private candidate and never publishes. It receives no release secrets, uploads no store or release payload, pushes no OCI layout, deploys no service or documentation, changes no updater state, and creates no tag or GitHub Release. SPDX SBOMs and provenance are generated only as local validation evidence. Release workflow behavior is evaluated by deterministic fixtures. Run `pnpm exec vp run ci:workflows`, `pnpm exec vp run ci:contracts`, and `pnpm exec vp run ci:release-fixtures` locally; package-local commands are listed in the applicable README files.
