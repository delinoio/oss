...
The current implementation exposes `record`, `compare`, `assetcov`, `latencylab`, `fbreak`, and `autowatch` in the clibox executable. The Linux ptrace collector supplies operation entry/completion, actual read results, per-calling-thread pre-operation delay, and terminal-controlled pre-operation pause. Autowatch retains observed-input dependencies across failed child runs and suppresses output-only self-writes; an observed input also written by the child fails with ambiguity guidance. macOS and Windows execution fail explicitly while their complete backends remain outstanding. `min-repro` remains outstanding. The feature is not complete or released while any workflow or target gate below remains outstanding.
