...
The macOS injected preload has a private side-channel foundation for paired open/read/write/close results and before-operation acknowledgment. It is dormant outside an explicit clibox socket and excluded from pnport mode. The receiving collector and complete operation boundary are not implemented yet, so macOS handlers continue to reject execution instead of claiming a complete trace.
