...
The macOS injected preload has a private side-channel foundation for paired open/read/write/close, metadata, directory, and pathname-mutation results with before-operation acknowledgment. It is dormant outside an explicit clibox socket and excluded from pnport mode. A native fixture confirms actual read bytes and successful metadata, directory, and rename results. The receiving collector and complete operation boundary are not implemented yet, so macOS handlers continue to reject execution instead of claiming a complete trace.
