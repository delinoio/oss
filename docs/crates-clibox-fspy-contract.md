...
The Linux ptrace supervisor is a private low-level foundation. It uses `PTRACE_GET_SYSCALL_INFO` to pair selected syscall entries and completions, follows fork/clone/vfork descendants, serializes local sessions sharing `waitpid`, and bounds cancellation, timeout, and surviving-descendant cleanup. Its callback may hold the calling thread before a syscall. It does not classify file paths or create complete product records by itself; the command family must not expose it as a complete tracer until those layers and Linux GNU/musl target tests are connected.
