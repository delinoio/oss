...
The current implementation exposes all seven commands in the clibox executable. The Linux ptrace collector supplies operation entry/completion, actual read results, per-calling-thread pre-operation delay, and terminal-controlled pre-operation pause. Autowatch retains observed-input dependencies across failed child runs and suppresses output-only self-writes; an observed input also written by the child fails with ambiguity guidance. Min-repro snapshots selected inputs privately, verifies both runs against explicit failure predicates, blocks original-tree access in the candidate, and publishes a bounded bundle with an input manifest. macOS and Windows execution fail explicitly while their complete backends remain outstanding. The feature is not complete or released while those backends and the remaining validation gates below remain outstanding.
