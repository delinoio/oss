...
Selected environment values are never retained; their names alone cannot establish compatibility in saved-report or repeated-output comparison.
Combined verification preserves definite policy failures even when other evidence is inconclusive.
Executable metadata is revalidated after snapshots at the launch boundary; detected replacement cannot inherit the inspected digest.
