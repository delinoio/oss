...
`release-clibox.yml` builds eight native binaries (macOS and Windows MSVC x64/arm64; Linux glibc/musl x64/arm64). GNU builds use Ubuntu 22.04 x64 and Ubuntu 24.04 arm64 runners; the corresponding glibc baselines are 2.35 and 2.39. musl binaries use native musl linkers and run npm/pnpm consumer smoke tests in Alpine. Windows uses the MSVC toolchain. macOS binaries use the deployment baseline of the repository-pinned Rust target. Each build checks executable version and creates a platform npm tarball. The assembly job adds the launcher, verifies exactly nine source-commit/version-bound tarballs, and uploads `clibox-npm-<version>-<revision>` with 30-day retention. Native intermediate artifacts are named `clibox-native-<suffix>`. There is no Homebrew or public GitHub Release asset publication.
...
For the first release leave `CLIBOX_NPM_PUBLISH_ENABLED` unset or `false`. The tag workflow creates the validated artifact and reports pending manual bootstrap without claiming npm publication. Publish the exact nine artifact tarballs manually, configure every package's Trusted Publisher for `delinoio/oss` and `release-clibox.yml`, then set the variable to `true`. Details and ordering are in [the npm distribution contract](packages-clibox-distribution-contract.md). No setup or validation command dispatches a workflow or publishes a registry version.
