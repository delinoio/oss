...
`CI.yml` runs `node-clibox-test` on Linux, macOS, and Windows, with one frozen script-disabled workspace install, the shared affected planner/Turbo comparison, clibox Rust unit/process/adapter tests, package fixtures, and temporary npm/pnpm consumer installs exercising environment execution as well as help/version. Each OS also exercises the affected runner against temporary workspaces, including empty selection, forced execution, and failure propagation. External Cargo and release inputs force this workspace's checks. The job participates in `CI Result`. Native integration tasks are not cached; successful-main-only dependency cache rules remain unchanged.
