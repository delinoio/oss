...
`apps/devhud` now contains one non-product feasibility package: a minimal React/TypeScript/Rsbuild bundled-asset probe, a target-selecting Tauri Rust crate, a reusable typed gate harness, and isolated native macOS CEF gate automation. Native execution stops while compiling the pinned upstream `tauri-runtime-cef`, before runtime or packaging evidence can be produced. It does not create a production tool, mobile/widget foundation, release/publication workflow, or support/publisher artifact. The complete cross-platform gate remains blocked, so `0.1.0` product and release work must not proceed.
...
- `apps/devhud` (`app`): the sole canonical implementation path. It currently owns only the one-package React/TypeScript/Rsbuild plus Tauri feasibility scaffold; future product, mobile/widget, and release sources remain blocked.
...
- The macOS portion is isolated in `.github/workflows/devhud-macos-cef-gate.yml` and runs native x64 and ARM64 jobs on macOS 14 or newer. At the pinned revision both jobs fail inside upstream `tauri-runtime-cef`: its Linux/BSD-only `TerminationSignals` type has an unconditional implementation that references a Linux/BSD-only `libc` dependency. Fixing that requires an upstream source change, so the gate must stop before DMG, updater, signing, runtime, or process evidence is claimed.
