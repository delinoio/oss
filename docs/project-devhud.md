...
- Application networking remains deny-by-default. Authorized future exceptions are the existing signed GitHub Releases updater boundary; Logto/DeliDev authentication; native Connect calls to the exact Deck or RealQA origin from the corresponding authenticated feature; the exact native `DeckDeviceService.UnregisterDevice` cleanup call authenticated only by its OS-vault single-registration revocation grant; the native RealQA same-origin signed upload PUT; verified Deck mobile auth callbacks; and the exact-origin RealQA Chrome native-host bridge. Public RealQA image GET is for ordinary issue readers and is not a remote-webview resource exception. GitHub.com provider calls, delibase usage calls, and R2 administration are server-side feature boundaries. Feature servers do not grant `http://tauri.localhost` CORS access; no arbitrary browsing, remote UI, remote configuration, client-side provider token use, or wildcard origin is allowed.
