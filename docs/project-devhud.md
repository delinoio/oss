...
- `protos/devhud/v1` — implemented versioned Connect RPC schemas and generated Go bindings.
- `packages/devhud-api-client` — implemented generated TypeScript client and Connect Query bindings.
...
No remaining planned path is a Cargo workspace member or an implemented runtime until its skeleton and contract exist.
...
- Upload finalization is submission-scoped: `CreateUpload` with no IDs creates a server-owned UUID v7 submission and first group, submission-only creates a later group, both IDs reuse an owned group, and group-only is invalid. No submission may finalize more than 10 images across its groups. Signed checksums and staging versions are immutable finalization inputs; PNG dimensions are checked before decoding and must be at most 4096×4096 and 16,777,216 total pixels, and promotion is conditional on the recorded version/checksum.
