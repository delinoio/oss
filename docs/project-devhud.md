...
No DevHud feature implementation belongs under `apps/delidev-app`, `servers/delibase`, `protos/delibase`, `crates/`, `cmds/`, or a public plugin/package path. DeliDev may expose connection management only through its existing `/account` and `/o/:orgSlug/settings` sections. It also owns the planned `/auth/devhud/callback` static route and exact-path Apple/Android association artifacts required to verify the Deck mobile link; that narrow hosting responsibility does not transfer DevHud ownership or authorize another top-level DeliDev feature route.
