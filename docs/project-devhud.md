...
- Non-gating performance instrumentation owns schema-validated, redacted evidence for desktop HUD/startup/package/memory and mobile startup measurements. Each result binds application, Tauri/CEF, and source-build revisions; aggregation accepts only the current provenance and keeps mobile target kinds distinct.
