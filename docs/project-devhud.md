...
- Browser/API CORS uses the exact origins `http://localhost:46305`, `http://127.0.0.1:46305`, `http://localhost:46306`, `http://127.0.0.1:46306`, and pinned Tauri `http://tauri.localhost`. Connect preflight methods and headers are explicit; wildcard origins and headers are not allowed.
...
- Account recovery is exposed as authenticated, ownership-checked, idempotent `AccountService.RestoreAccount` during the 30-day window; restore and final purge serialize on the account state, and after purge claims the account no restore can succeed. Calls after the window fail with `FailedPrecondition`.
...
- CI and integration validation must cover upload-group binding, concurrent CreateUpload signed-URL reservation and issuance failure rollback, finalization non-double-charging, upload-finalization rejection and staging cleanup, concurrent per-group image quota enforcement, restore-versus-purge races at the recovery boundary, idempotent multi-instance sweeper execution, exact CORS origin rejection and Connect preflight behavior, origin replacement before CDN purge/revalidation for user deletion/quarantine/account purge, stale widget presentation, recovery-window boundaries, and updater signature/key-rotation/rollback behavior.
