...
- Desktop uses `tauri-runtime-cef` pinned exactly to Tauri commit `4af26a3f7f8b692d62cca549bbacd93f5ce90b41`; mobile uses WKWebView/Android System WebView and native Swift/Kotlin widgets. The bundle identifier is `io.delino.devhud` and the deep-link scheme is `devhud`.
...
- Official uploads use R2 signed direct uploads, with 50 MiB/object, 10 images/issue, 1 GiB rolling-24-hour, 20 GiB stored, 120 signed URLs/rolling-hour, and 300 public GETs/IP/minute limits. `FinalizeUpload` revalidates authentication, block status, ownership, exact staging-key binding, size, checksum, allowed image content, PNG signature, and safe raster dimensions; it rejects replay and removes invalid staging objects. Staging expires after 24 hours. API logs and crash reports retain 30 days; pseudonymized security/audit events retain at most 180 days. Account deletion purges user data, official-upload/tombstone metadata, and public cached bytes after a 30-day recovery period; upload metadata is deleted or irreversibly pseudonymized, except for the explicit audit-retention boundary.
...
- CI and integration validation must cover upload-finalization rejection and staging cleanup, CDN purge/revalidation for user deletion/quarantine/account purge, stale widget presentation, recovery-window boundaries, and updater signature/key-rotation/rollback behavior.
