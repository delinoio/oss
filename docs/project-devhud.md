...
`apps/devhud` contains one active pnpm foundation package: a React/TypeScript/Rsbuild bundled-asset application, a target-selecting Tauri Rust application crate, a private Rust/native mobile widget bridge, maintained iOS and Android host projects, and build-only WidgetKit/AppWidgetProvider projects. Desktop builds use the exact pinned upstream CEF runtime directly. Mobile builds use only standard Tauri WKWebView and Android System WebView. The implemented UI includes provider-owned local settings/widget state, empty desktop HUD/settings and stable Home, Widgets, Settings, and Diagnostics mobile screens, and a closed internal tool registry. Native widget sources compile and test but are neither embedded nor registered in release apps. The foundation does not create a production tool, visible native widget, packaging or release workflow, or support/publisher artifact.
...
- The application package, native mobile hosts, Cargo workspace membership, deterministic Turborepo output contract, build-only native widget sources, and native widget CI jobs are present. Add product behavior, widget distribution, packaging, release workflows, signing/publisher material, and support material only with their corresponding documented contracts and validation.
