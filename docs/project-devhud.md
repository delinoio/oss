...
`apps/devhud` contains one active foundation package: a React/TypeScript/Rsbuild bundled-asset application, a target-selecting Tauri Rust crate, and maintained iOS and Android host projects. Desktop builds use the exact pinned upstream CEF runtime directly. Mobile builds use only standard Tauri WKWebView and Android System WebView. The implemented UI includes provider-owned local settings/widget state, empty desktop HUD/settings and stable Home, Widgets, Settings, and Diagnostics mobile screens, and a closed internal tool registry. It does not create a production tool, visible native widget, packaging or release workflow, or support/publisher artifact.
...
- The application is one pnpm package: a React and TypeScript frontend built with Rsbuild and a Tauri Rust crate under `src-tauri`. Root package scripts remain delegators; deterministic validation tasks are package-local.
...
- The active immutable identifiers are application ID `dev.deli.devhud`, settings key `devhud.settings.v1`, and future widget-state key `devhud.widget-configuration.v1`. No App Group entitlement or widget extension/provider identifier is present in a distributed mobile target.
...
- Production tool registration and user-visible mobile widget state are empty in `0.1.0`; fixture tools and fixture widget state may exist only in tests. No WidgetKit extension or Android `AppWidgetProvider` source, target, embedding, or registration is included.
...
- The application package, native mobile hosts, Cargo workspace membership, and deterministic Turborepo output contract are present. Add product behavior, native widget sources, CI workflows, packaging, release workflows, signing/publisher material, and support material only with their corresponding documented contracts and validation.
