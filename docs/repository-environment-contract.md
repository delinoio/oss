...
- The workspace-local `$run-dev` Codex skill is explicit-invocation-only. It asks for a credential mode when none is provided, maps user-facing `Delino` and internal `team` to the ordered `pnpm env:login`, `pnpm env:doctor`, and `pnpm dev` sequence, and maps `OSS` to `pnpm dev:oss` without invoking Infisical. The skill delegates all secret injection and validation to the existing service-owned wrappers and must not export provider values or write them to environment files.
