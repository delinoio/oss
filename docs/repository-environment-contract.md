...
Base child environments preserve only the documented non-secret platform and tool context, including custom `CARGO_HOME` and `RUSTUP_HOME` locations needed by the Rust toolchain. The API owns path `/devhud/api`. Its required names are:
