...
- `pnpm dev:public-docs`, `pnpm dev:nodeup-docs`, `pnpm dev:binpm-docs`, and `pnpm dev:runmoor-docs` are the repository-root documentation development entry points. Each delegates directly to the named workspace package and its fixed-port development wrapper without requiring the DevHud team environment.
