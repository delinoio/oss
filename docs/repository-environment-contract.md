...
The async-commit-hook local UI/API remain on configurable api_port (default 46309), with frontend development on fixed 46308. Its separate public-docs app uses fixed development 46311 and preview 46281 through the shared Rspress wrapper; root `pnpm dev:async-commit-hook-docs` delegates through Turbo to the package and never invokes Infisical. No new environment input is required.
