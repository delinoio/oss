...
No-network tests use fake Infisical, Docker, Go, and Turbo executables, an injected temporary generated-state directory, and an injected Infisical project-config directory outside the checkout; they never delete or replace the checkout's `.dev-environment` identity or `.infisical.json` binding. They cover first/repeat team setup; exact provider flags, paths, and service allowlists; complete provider-pipe marker handling; missing/unknown/partial configuration; canary non-disclosure; fail-closed authentication/path behavior; OSS ordering and lack of Infisical; matching administrator/API issuer overrides; local Docker endpoint enforcement; immutable pins; every fixed port and bind host; generated key permissions and checkout-scoped Compose identity; migration-before-Turbo ordering; unavailable OSS authenticated/admin/official-upload capability; interruptions and process-tree cleanup; and volume-preserving down. CI installs this suite's workspace dependencies without lifecycle scripts and runs it on Linux, macOS, and Windows.
