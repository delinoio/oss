...
- `pnpm dev` selects `team`, fails non-interactively when authentication or local project binding is missing, validates both service environments, privately pins their matched issuer comparison, requires migration and both service launches to match that pin, and starts the DevHud frontend, administrator, and API through `turbo run dev` on fixed ports `46305`, `46306`, and `46307`.
...
The administrator owns path `/devhud/admin` and receives only `DEVHUD_LOGTO_ISSUER`. The wrappers reject missing required names, every unknown name, an invalid value, and every partial optional group before migration or service start. Raw HTTP(S) configuration must contain an explicit `scheme://authority`; parser-normalized spellings without an authority delimiter are invalid. HTTP loopback IPv4 authorities must use the canonical four-part decimal notation accepted by Go; shorthand, integer, octal, and hexadecimal spellings are invalid even when WHATWG normalizes them to loopback. The public asset base must have no raw path or exactly `/`; dot segments and encoded equivalents are rejected before WHATWG normalization so preflight matches Go. The wrappers rebuild each child environment from a small platform/tool allowlist and the accepted service values. Raw values never enter a file, log, diagnostic, argument, or sibling process. Team preflight uses an ephemeral keyed comparison from each wrapper to require the API and administrator issuer values to match exactly before migration without exposing either value. Root then writes only that key, its matched digest, and an ownership token to the private team pin. Every later API migration, API serve, and administrator serve wrapper recomputes its issuer comparison and fails closed if the injected generation changed; the pin never crosses Turbo and remains until Turbo exits so task launch cannot bypass parity. In OSS mode, a contributor who overrides the issuer must record the exact same value in `servers/devhud-api/.env` and `apps/devhud-admin/.env` so API Bootstrap and the administrator CSP remain aligned.
