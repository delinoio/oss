...
- Local shell links use the system browser only through a native closed allowlist: the validated configured authentication origin and the approved GitHub PAT and issue destinations. The shell waits for a successful opener exit before reporting a link as opened; unsuccessful or unobservable opener exits return an error and record a structured diagnostic. Documentation remains unavailable until a curated public DevHud destination exists. Arbitrary browser navigation is rejected.
