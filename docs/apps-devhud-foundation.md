...
- Performance measurements must record HUD display latency, cold startup, package size, and idle memory per supported desktop platform, plus mobile startup time. Performance builds and evidence require a clean Git worktree; desktop profiling uses a release-profile binary with an instrumentation-only feature and an isolated local lock, CEF cache, and diagnostics namespace, while normal releases omit that feature. Publish these measurements with the release; `0.1.0` defines no numeric pass threshold.
