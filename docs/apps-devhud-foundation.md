...
- Generated PNGs are 8-bit RGBA with the same blue/white treatment in System, Light, and Dark contexts. iOS catalog icons and Android launcher icons are opaque platform inputs; the Google Play listing uses a distinct 512×512 icon and an opaque 1024×500 feature graphic. macOS tray templates preserve transparent pixels for native menu-bar tinting, while Windows and Linux use opaque high-contrast tray assets. `pnpm check:assets` validates dimensions, formats, decoded alpha coverage, contrast, safe names, source hash, deterministic regeneration, Tauri inclusion, native host inclusion, and accessible store metadata.
