...
- Deck is planned for desktop, iOS, Android, desktop tray, global shortcuts, opt-in notifications, WidgetKit on iPhone/macOS, and Android widgets. It consumes `devhud.deck.v1` at the future exact origin `https://deck.deli.dev`.
...
- Networking is deny-by-default. Future allowlists are limited to: the separately scoped unauthenticated signed GitHub Releases updater; Logto/DeliDev authentication; authenticated Deck Connect calls to `https://deck.deli.dev`; authenticated RealQA Connect calls to `https://realqa.deli.dev`; RealQA short-lived signed uploads/public images at `https://assets.realqa.deli.dev`; the verified mobile auth callback; and exact-origin Chrome Native Messaging. Provider GitHub.com, delibase usage, and R2 administration remain server-side. No wildcard host, arbitrary remote navigation/resource, remote UI, remote configuration, or client-side GitHub token is allowed. The current updater and both features perform no network access because they are not implemented.
...
Feature implementation must add non-placeholder package commands `test:deck`, `test:deck:widgets`, `test:realqa`, `test:realqa:native`, `test:realqa:extension`, and `check:realqa:package`. The baseline invocation remains `pnpm --filter devhud <command>`. `test:deck` covers signed-out isolation, auth, views, refresh/billing states, shortcuts, privacy, and offline/deletion UI; `test:deck:widgets` compiles and tests Deck WidgetKit/Android widget families and release-artifact boundaries without promising OS refresh timing or publishing them. `test:realqa` covers presets, templates/forms, editor/body/size limits, drafts, account isolation, and submission states; `test:realqa:native` covers all supported capture platforms, native-host pairing, secure storage, and reset; `test:realqa:extension` covers MV3 permissions, visible-viewport/DOM sanitization, message limits, restricted pages, and fixture-ID behavior; `check:realqa:package` fails on an absent externally injected approved production extension ID for release packaging. Until implementation adds them, their absence is expected and must not be represented by passing stubs.
...
- Static and built-artifact checks for malicious external navigation, popup, download, remote resource, undeclared IPC, generic filesystem/process/network/store authority from normal views and DevTools; exact per-window capabilities; CEF sandbox/bundled origins; only the verified mobile callback/associated-domain and exact feature endpoint allowlists; no unintended endpoint, wildcard origin, arbitrary remote UI, client GitHub token, or sensitive Chrome permission; and no release widget/extension registration, dependency, embedding, provisioning identity, production tool activation, or visible sample state before the corresponding distribution change.
