...
- Navigation is restricted to bundled `http://tauri.localhost` content in production and exact `http://127.0.0.1:46305` content in development. Popups and downloads are denied. The Rsbuild development launcher binds only `127.0.0.1:46305`, preflights collisions, and never chooses another port. The Tauri command wrapper rejects all `-c` and `--config` overrides, including attached short values, so callers cannot replace the pinned application configuration; it forwards `SIGINT` and `SIGTERM`, mirrors signal-based exits, and on every supported desktop platform terminates and awaits the complete Cargo, Tauri CLI, desktop-host, and Rsbuild process tree. Timed-out Windows platform smokes use the same complete-tree termination before temporary cache and log cleanup.
- Structured diagnostics cover platform/display selection, resource discovery, frontend readiness, renderer termination, fatal initialization, smoke shutdown, and clean host shutdown. After React mounts, the frontend changes the document title from `DevHUD Loading` to `DevHUD`; the host consumes CEF's title-change notification as its readiness signal without installing a DevTools observer. An independent five-second wall-clock watchdog emits `frontend_readiness_timeout` and terminates with exit code 1 if readiness does not complete. macOS uses the typed CEF renderer-termination callback; Windows and Linux enable the CEF Inspector domain and register the DevTools protocol crash listener only for the deliberate renderer-crash smoke. Desktop release hosts retain timestamped, redacted JSON diagnostics in `%LOCALAPPDATA%\io.delino.devhud\logs\devhud.jsonl` on Windows, `~/Library/Logs/io.delino.devhud/devhud.jsonl` on macOS, and `${XDG_STATE_HOME:-~/.local/state}/io.delino.devhud/logs/devhud.jsonl` on Linux while preserving stderr output for managed launches and smoke capture; operator `RUST_LOG` filters may tune dependency verbosity but cannot suppress error-level DevHUD diagnostics. Debug builds and CEF subprocesses remain stderr-only, and a file-sink setup failure falls back to stderr without logging the resolved path.
