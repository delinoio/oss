...
- Persist only the refresh token and device-session key in the OS secure vault. Access and ID tokens remain in memory. Permit one active DeliDev account per OS user/device.
...
- Notifications are view-specific opt-in and DND-aware. Default text is exactly `Deck view updated`; detailed repository/PR titles require per-device opt-in, and push payloads contain opaque event IDs only.
...
- Logout and `Reset DevHud` first call `DeckDeviceService.UnregisterDevice` while authenticated, then delete local Deck tokens, PR data, snapshots, and shortcut effective state without deleting server views/connections. If unregister is offline or ambiguous, locally disable that account/device registration, delete the platform push token, retain only a credential-free cleanup tombstone, and retry unregister before any later Deck device registration. The client never displays an opaque push event unless it still has the matching active account/device registration. Disconnect and owner-scoped idempotent `DeckViewService.DeleteFeatureData` follow the Deck server contract.
...
- Networking is deny-by-default. Future native allowlists are limited to: the separately scoped unauthenticated signed GitHub Releases updater; Logto/DeliDev authentication; authenticated Deck Connect calls to `https://deck.deli.dev`; authenticated RealQA Connect calls to `https://realqa.deli.dev`; the RealQA short-lived signed PUT at `https://assets.realqa.deli.dev`; the verified mobile auth callback; and exact-origin Chrome Native Messaging. Public image GET remains an ordinary issue-reader server surface, not a DevHud webview permission. Bundled webviews never fetch these remote origins, feature servers do not allow `http://tauri.localhost` CORS, and no CORS exception weakens the navigation/resource guard above. Provider GitHub.com, delibase usage, and R2 administration remain server-side. No wildcard host, arbitrary remote navigation/resource, remote UI, remote configuration, or client-side GitHub token is allowed. The current updater and both features perform no network access because they are not implemented.
...
Feature implementation must add non-placeholder package commands `test:deck`, `test:deck:widgets`, `test:realqa`, `test:realqa:native`, `test:realqa:extension`, and `check:realqa:package`. The baseline invocation remains `pnpm --filter devhud <command>`. `test:deck` covers signed-out isolation, auth, views, refresh/billing states, shortcuts, privacy, offline/deletion UI, and the closed native Connect procedure/origin transport; `test:deck:widgets` compiles and tests Deck WidgetKit/Android widget families and release-artifact boundaries without promising OS refresh timing or publishing them. `test:realqa` covers presets, templates/forms, editor/body/size limits, drafts, account isolation, and submission states; `test:realqa:native` covers all supported capture platforms, native-host pairing, secure storage, reset, the closed native Connect transport, and signed-PUT validation/rejection; `test:realqa:extension` covers MV3 permissions, visible-viewport/DOM sanitization, message limits, restricted pages, and fixture-ID behavior; `check:realqa:package` fails on an absent externally injected approved production extension ID for release packaging. Until implementation adds them, their absence is expected and must not be represented by passing stubs.
