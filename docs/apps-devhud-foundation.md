...
Performance evidence preserves the frequency of independent samples while ignoring repeated input paths, rejects duplicate target identities and metric names within a target, verifies mobile installed-app versions and desktop debug-binary provenance before timing them, and records `unknown` rather than guessing an architecture when target inspection fails. Each desktop and mobile invocation writes a distinct result file so aggregation can retain repeated runs. A failed desktop performance build emits `build-failed` evidence rather than omitting the host.
