...
Performance evidence preserves the frequency of independent samples while ignoring repeated input paths, rejects duplicate target identities and metric names within a target, verifies mobile installed-app versions and desktop debug-binary provenance before timing them, detects Android architecture from the installed application's package ABI rather than the device-wide preferred ABI, and records `unknown` rather than guessing an architecture when target inspection fails. Mobile target kind is a required, non-sensitive target identity dimension, so physical-device and emulator samples remain separate during aggregation. Complementary failed evidence becomes available when it contains every required available measurement. Each desktop and mobile invocation writes a distinct result file so aggregation can retain repeated runs. A failed desktop performance build emits `build-failed` evidence with independently verifiable package-size evidence and a sanitized failure status rather than omitting the host or exposing captured build output.
