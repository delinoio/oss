...
The package provides local `dev`, `build`, `typecheck`, `lint`, `test`, `test:a11y`, `check:diagnostics`, `test:diagnostics`, deterministic rebuild, contract/pin, lockfile, Rust, debug desktop, sign-ready preview bundle, and host-appropriate repeated desktop lifecycle smoke commands. Non-gating performance instrumentation uses `perf:package`, `perf:desktop`, `perf:mobile -- <android|ios> <target>`, `perf:aggregate`, `perf:validate`, and `test:performance`. It writes schema-validated, redacted evidence only; `perf:desktop` owns its cross-platform build/failure orchestration and emits only a sanitized failure status before writing `build-failed` evidence, desktop profiling rejects unsupported host OS/architecture combinations, uses the debug executable (the macOS `.app` bundle executable), bounds graceful shutdown before forcefully terminating the profiler process tree, and keeps a startup timeout distinct from a malformed measurement protocol. `perf:package` builds the release artifact and writes its application-provenance/SHA-256 sidecar; package size uses only a host-architecture artifact whose sidecar still matches, and remains independently collectible when runtime profiling is unavailable. Mobile architecture is detected from the selected target; Android device evidence requires an explicit selector and Android target kind is verified before timing. Aggregation validates matching application provenance before preferring available evidence for duplicate platform/architecture/target-kind results; mobile target kind is a non-sensitive required identity dimension, so physical-device and emulator samples never mix. Every validated result has at least one target; targets may contain only their platform's metrics, and metric notes are bound to their metric names. An available target contains every required available measurement; incomplete evidence stays unavailable unless failed evidence collectively contains every required available measurement, in which case aggregation marks it available. `check:diagnostics` statically validates the allowlist, rotation constants/tests, exact runtime versions, native picker scope, capability exclusions, fatal/redaction coverage, and absence of remote transport; `test:diagnostics` adds the focused frontend and Rust suites. Mobile hosts add `mobile:generate:android`, `mobile:generate:ios`, `build:android`, `build:android:ci`, `build:ios`, `build:ios:ci`, `check:mobile`, `test:mobile`, and `test:android:native`. Native widget commands are `build:widget:android`, `build:widget:ios`, `test:widget:android`, `test:widget:ios`, and `check:widget-artifacts`. The artifact command checks canonical projects plus discovered release merged manifests and accepts explicit Android manifest/APK and iOS `.app` paths; it requires at least one built release artifact and fails if any receiver is registered or an extension/provisioning identity is embedded. Deterministic frontend output is declared in `apps/devhud/turbo.json`. `test:a11y` exercises component keyboard/focus and screen-reader semantics plus automated WCAG checks with `axe-core`; the full desktop OS/architecture/display matrix, signature validation, and release-validation tasks must be added with their corresponding implementations and must not be represented by passing placeholders.

Performance evidence preserves the frequency of independent samples while ignoring repeated input paths, rejects duplicate target identities and metric names within a target, verifies mobile installed-app versions and desktop debug-binary provenance before timing them, detects Android architecture from the installed application's package ABI rather than the device-wide preferred ABI, and records `unknown` rather than guessing an architecture when target inspection fails. Mobile target kind is a required, non-sensitive target identity dimension, so physical-device and emulator samples remain separate during aggregation; iOS launch methods are likewise bound to simulator or physical-device kind. Aggregation combines all evidence for an identity before deriving its status, making release output independent of input order. Complementary failed evidence becomes available when it contains every required available measurement. Each desktop and mobile invocation writes a distinct result file so aggregation can retain repeated runs. Desktop performance builds stream their output rather than buffering it, while `build-failed` evidence keeps only independently verifiable package-size evidence and a sanitized failure status.
