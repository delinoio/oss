...
- Deck calls GitHub directly, preserves raw `is:pr` queries, caches at most 100 results/Deck, permits 25 Decks/user, refreshes at 1/5/15/30 minutes, and uses local notifications and native widgets. Widgets show one Deck and three PRs and deep-link to `devhud://deck/<deck-id>`.
