...
- Logout deletes device Deck tokens, PR data, and snapshots. `Reset DevHud` deletes device tokens, snapshots, and shortcut effective state but not server views/connections. Disconnect and server feature deletion follow the Deck server contract.
...
- The signed Chrome MV3 extension targets stable Chrome, excludes Incognito, requests `tabs`, never requests `desktopCapture` or `<all_urls>`, and obtains origin-specific optional host permission only when DOM selection is activated. Capture is visible viewport only. It collects only a boundary annotation, sanitized stable selector, tag/role/accessible name, and viewport dimensions—never arbitrary page HTML/text—and every metadata field is removable.
