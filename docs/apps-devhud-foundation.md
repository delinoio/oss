...
- RealQA supports display/window/all-display/region/toolbar capture, editing, encrypted drafts, URL mappings, optional sanitized Chrome context, GitHub issue creation, official/BYO R2, and explicit Codex/Claude Code/OpenCode draft/direct modes. Official uploads use a server-issued UUID v7 upload group created by the first `CreateUpload`; subsequent upload requests use that group and the server atomically enforces 10 finalized images per group before issue creation. It excludes mobile capture, screen recording, full-page/console/network/cookie/storage capture, comments, assignees, milestones, projects, and updating existing issues.
