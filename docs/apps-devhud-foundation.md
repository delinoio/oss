...
- Structured diagnostics cover platform/display selection, resource discovery, frontend readiness, renderer termination, fatal initialization, smoke shutdown, and clean host shutdown. After React mounts, the frontend changes the document title from `DevHUD Loading` to `DevHUD`; the host consumes CEF's title-change notification as its readiness signal without installing a DevTools observer. An independent five-second wall-clock watchdog emits `frontend_readiness_timeout` and terminates with exit code 1 if readiness does not complete. macOS uses the typed CEF renderer-termination callback; every packaged Windows and Linux launch enables the CEF Inspector domain and registers the DevTools protocol crash listener, while the deliberate `Page.crash` request and its watchdog remain exclusive to the renderer-crash smoke. Desktop release hosts retain timestamped, redacted JSON diagnostics in `%LOCALAPPDATA%\io.delino.devhud\logs\devhud.jsonl` on Windows, `~/Library/Logs/io.delino.devhud/devhud.jsonl` on macOS, and `${XDG_STATE_HOME:-~/.local/state}/io.delino.devhud/logs/devhud.jsonl` on Linux while preserving stderr output for managed launches and smoke capture; operator `RUST_LOG` filters may tune dependency verbosity but cannot suppress error-level DevHUD diagnostics. Debug builds and CEF subprocesses remain stderr-only, and a file-sink setup failure falls back to stderr without logging the resolved path.
