...
IPC authentication is mutual: the host proves the fresh challenge and the app returns a distinct secret-bound proof over that challenge and the new session ID. If an authenticated connection was closed while idle, the host reauthenticates and retries the pending request once. The idempotent unregister command removes both host registration and pairing credentials.
