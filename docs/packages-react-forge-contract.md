...
The supported-host package suite now includes all four workspace CLI examples and an actual packed-package installation with its installed binary (25 checks). Typed session errors survive the TSX module namespace boundary. Rich examples include native Word charts, all spreadsheet rule families and independent PDF authoring. Reproducible renderer/benchmark/CI acceptance remains ongoing.


Pinned exports snapshot font registrations along with model/assets, and font changes participate in revision identity. Preparing an export rechecks mounts/assets that arrive while Suspense is pending. Disposal owns in-flight mounts and returns one shared cleanup promise. Concurrent imported-source exports compare their starting fingerprint, so an older export cannot accept a newer save's fingerprint. Symlink output publication does not rewrite the imported source fingerprint when only the link is replaced. Explicit file reads reject non-regular files without blocking on FIFOs. CLI loading supports the Node 24 CommonJS default-export namespace as well as ESM; process-signal tests verify cleanup and redacted task failures. Native failures expose bounded whitelisted model locations, never parser/source paths.
