...
Windows Job lifetime checks query bounded membership snapshots, verify handle membership and test process signaling. An empty scan is confirmed against a second membership snapshot so a newly spawned descendant is not missed. Accounting ActiveProcesses can include terminated process objects while handles remain, per [Microsoft](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-jobobject_basic_accounting_information); retained-handle native regressions distinguish those objects from real lingering children. Query failures and unstable/budget-exhausted membership remain cleanup failures.

Windows `NtQueryInformationFile` observes file metadata reads even on write-only handles, including failed output-buffer calls. Unresolvable handles make collection incomplete; pipe/device metadata does not become a file dependency. A scoped thread-local guard excludes only collector-owned path resolution queries. Native Windows fixtures compare results/status with direct execution and require deny-read failures without persisting file bodies. The ABI and write-only `FileStandardInformation` behavior follow [Microsoft](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntifs/nf-ntifs-ntqueryinformationfile).

Access ancestry is checked against every observed ancestor replacement/removal attempt after collection. An otherwise in-scope descendant of a replaced/removed ancestor becomes unsupported/out-of-scope and collection becomes incomplete, even if before/after directories match. Aggregate observations cannot disprove a rename → external symlink → read → restore sequence; failed replacement/removal attempts remain conservative false positives until the engine binds ordered resolved identities. This check uses a redacted spillable replacement index and four million lookup/Windows ordinal-alias comparisons; exhaustion is unknown. Dot/separator mutation aliases are normalized, while parent traversal cannot certify collection. Ordinary file writes and mkdir attempts do not carry the private replacement marker and preserve stable ancestry. Native libc and Linux static syscall regressions restore the original directory and content while rejecting workspace-only read verification.
