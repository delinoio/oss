...
## Unresolved Unix process ownership

PR #901 review thread `PRRT_kwDORRAKg86j9n3x` is valid and unresolved. A temporary Go-overlay reproduction started a process that double-forked, called `setsid`, changed directory and reparented before the first sample. The existing `terminate` returned success while the descendant remained alive; the reproduction explicitly killed that child during cleanup. Existing process-group and birth-identity tests do not cover this case, and full descendant-safe cancellation/replacement remains required.

A verified replacement backend across the supported Unix versions is still missing. Linux provides [subreaper ownership](https://man7.org/linux/man-pages/man2/PR_SET_CHILD_SUBREAPER.2const.html), which does not itself solve macOS. Apple's [XNU process event implementation](https://github.com/apple-oss-distributions/xnu/blob/f6217f891ac0bb64f3d375211650a4c1ff8ca1ea/bsd/kern/kern_event.c#L1099) explicitly rejects `NOTE_TRACK`, `NOTE_TRACKERR` and `NOTE_CHILD`; the installed SDK also marks those flags unsupported since macOS 10.5. A full [Endpoint Security client entitlement](https://developer.apple.com/documentation/bundleresources/entitlements/com.apple.developer.endpoint-security.client) requires approval from Apple and would introduce a new deployment/permission contract. No such entitlement, elevated monitor or unsupported compatibility exception was introduced. Faster polling and inherited environment markers were rejected as insufficient proof of ownership. This blocker must remain visible until a supported backend and its lifecycle/recovery tests establish the required guarantees.
