...
- Limits and releases document typed recovery, resource ceilings, first functional npm version `0.1.1`, availability of GLB/FBX/SFX/Sprite since `0.2.0`, six-host release validation, and the absence of direct Microsoft Office validation or PDF/UA certification. Current install guidance should resolve npm's published dist-tag instead of claiming an old version is perpetually latest.
