...
Settings are schema-versioned full snapshots with monotonic revisions and expected-revision replacement; stale writes return `Aborted` and a conflict payload. Service identifiers use UUID v7; Logto and GitHub identifiers are external IDs. Protocols must not include secrets, PATs, R2 keys, DOM, screenshots, Deck results, agent output, or local paths. `FinalizeUpload` is the validation boundary after direct R2 upload: it revalidates authentication, block status, and ownership; binds the request to the exact one-time staging key issued by `CreateUpload`; verifies declared and observed size, checksum, allowed image content type, PNG signature, and safe raster dimensions; rejects replay or already-finalized keys; and removes invalid staging objects before returning `FailedPrecondition`.
