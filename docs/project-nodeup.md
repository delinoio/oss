...
- Package-local `pnpm exec vp run dev` and root `pnpm exec vp run dev:nodeup-docs` for `apps/nodeup-docs` bind to loopback on fixed port `46303` by default, reject host overrides, preflight the exact selected port, and fail on conflicts without automatic remapping. The existing explicit `NODEUP_DOCS_DEV_PORT` override remains supported.
