...
The shared prerequisite/companion fixture `scenarios_08_15_16_19_watch_and_timer_companions_preserve_server` starts with an absent watched input. Its real server can start only after the initial check succeeds; that barrier verifies one shared activation before the fixture creates the input and expects a second check. This prevents delayed macOS events from pre-session fixture creation from being mistaken for duplicate activation while retaining the runtime's conservative pre-baseline event handling.
