...
The Linux image-seal regression must use the writable descriptor retained from image creation, whose write capability remains subject to the seals after the file mode becomes 0500. Reopening the path for writing fails with `EACCES` for ordinary owners and had only passed locally because the earlier Linux container ran as root. GitHub's Linux x64/arm64 native jobs and root Rust job failed at that test's open, before seal assertions. The corrected test preserves mode 0500 and verifies write/resize denial through a duplicated creation descriptor; local Linux verification runs as UID/GID 1000.


The next five review repairs preserve durable cache-reuse success after late cancellation, bound all persisted shard metadata, retain Docker shard port mappings, validate reserved CI environment names with runner-platform semantics, and retain invalidated session roots across installation phases. `persisted_cache_reuse_survives_late_cancellation` injects cancellation immediately after receipt persistence for both cache hits and restored outputs. `persisted_shard_metadata_is_bounded_and_regular` rejects oversized inventories and reports, including extra report files, directories, dangling links, and Unix FIFOs. The real Docker execution case serves both generic shard units through a declared host port and verifies cleanup permits rebinding. `ci_reserved_environment_names_follow_the_runner_platform` covers 144 combinations of runner OS, reserved-name casing, task secrets, and remote credential references, with no generated files on rejection. `session_retains_invalidated_watch_roots_across_install_phases` exercises two installation phases with and without a later phase repairing an invalidated initial-disabled watched root.

Final verification passed root `cargo test --locked` on macOS arm64 with eight test threads and a short canonical private temporary root; its sibling canary remained intact. TaskFlow passed 145 default conformance cases, all seven real native/Docker/S3 suites (152 total), 17 library tests, and one CLI test. The Linux arm64 source-copy build ran as UID/GID 1000 with a noexec temporary control mount and a separate fixture directory outside its workspace; it passed 146 default conformance cases, 18 library tests, and one CLI test. This isolated build used the cached registry offline and pruned only its copied workspace lockfile; the repository lockfile remained unchanged and root verification remained locked. TaskFlow Clippy with warnings denied passed on both platforms. Workspace formatting, 49 CI contract tests, and repository/generated workflow actionlint passed. Public documentation was unchanged in this repair, so its previously passing package tests were not repeated. Required DevHud frontend/client outputs were generated before root compilation and repository-owned generated `dist` directories were removed afterward. New remote platform CI remains unverified before the final push.

The new installation-phase regression failed against the prior implementation before the fix: the session completed its bootstrap without repairing the invalidated watched root. Initial Docker port regression runs also failed because Node HTTP keepalive connections delayed the first unit's shutdown; the fixture now explicitly closes its HTTP response connection so both units can run and cleanup can be checked. The final focused Docker test and all seven external suites passed. The previous revision's Linux x64/arm64 and root Rust CI failures shared the mode-0500 reopen assumption described above; the corrected seal test and full Linux suite passed as an ordinary owner. The root job's cache-path annotation was not an independent failed step: its cache step succeeded and its Rust test step failed at the same seal assertion.


The next review repairs add bounded execution reports, preserve stopped-service evidence during installation, and propagate Windows Job Object cleanup failures. `task_result_reports_are_bounded_regular_execution_files` runs the real CLI against absent, valid, exactly 1 KiB, oversized sparse, directory, malformed, and foreign-execution reports, plus Unix regular/dangling links and FIFOs. A separate CLI deadline proves that post-exit report validation cannot block, invalid reports block consumers, and no successful baseline is published. `bootstrap_service_evidence_requires_current_semantic_identity` verifies that changed service inputs invalidate installation evidence transitively. `installation_service_evidence_never_reuses_a_stopped_owner` runs two installers and a final consumer against three separately owned TCP-ready service processes, requires each finite task to execute once, and verifies every owner is reaped and its port reusable.

Windows `failed_job_cleanup_preserves_ownership_for_retry_and_drop` duplicates real job handles with restricted query-only or termination-only access to exercise actual API failures after normal exit, cancellation, and timeout. Failures must retain the cleanup error and uncleared ownership state; explicit retry or destructor cleanup must release a descendant's exclusive TCP listener. The fixture intentionally leaves its descendant running after the direct parent exits so parent waiting cannot satisfy the assertion. The implementation checks [TerminateJobObject's failure result](https://learn.microsoft.com/en-us/windows/win32/api/jobapi2/nf-jobapi2-terminatejobobject) and [active-process job accounting](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-jobobject_basic_accounting_information), then marks cleanup complete only after the job is empty and the direct child is reaped. Finite tasks, readiness failures, native metadata, and services release the owner's final fallback before awaiting descendant-held log pipes. The existing readiness grace-timing assertion now skips its Unix-only check explicitly on Windows instead of compiling a constant-true assertion.


The installation-service regression reproduced the old stabilization error before the fix. An intermediate version retained service evidence but still propagated initial service activation into already completed installers; the final regression requires one execution each and three distinct service owners. Initial fixture compilation used an incorrect receipt helper name and omitted a required readiness timeout; both were corrected before focused validation. The first macOS root run, concurrent with external and container verification, failed two existing two-second timeout fixtures: service cleanup expected a PID file for a task that had not started, and shard execution expected an execution timeout after inventory had already exhausted the deadline. Both focused cases passed separately. These failures do not show that the timeout reason or process cleanup was incorrect; the initial root run remains recorded as failed.

The initial macOS-to-MSVC check could not compile `ring` because Windows C headers were unavailable. A Linux source-copy build with MinGW subsequently checked every Windows x64 GNU target, including the new Windows tests, and passed Clippy with warnings denied after correcting test imports, test-module placement, the intentional unwaited-child fixture annotation, and the existing constant-true Windows timing assertion. This is compile/lint evidence only: no new Windows runtime result is claimed locally. Linux verification used UID/GID 1000, offline cached dependencies, noexec temporary control storage, and separate fixtures outside the isolated workspace. Only the copied workspace lockfile was pruned; the repository lockfile stayed unchanged.


Final root `cargo test --locked` passed on macOS arm64 with four test threads, a short canonical private temporary root, and an intact sibling canary. TaskFlow passed 147 default conformance cases and all seven real native/Docker/S3 suites (154 total), 18 library tests, and one CLI test. The Linux arm64 source-copy build passed 148 default conformance cases, 19 library tests, and one CLI test. Both hosts passed TaskFlow Clippy with warnings denied; Windows x64 GNU cross-compilation and Clippy also passed for every TaskFlow target. Workspace formatting, 49 CI contract tests, repository workflow actionlint, and generated-workflow actionlint passed. Public documentation was unchanged, so its previously passing package tests were not repeated. Required DevHud frontend/client outputs were generated before root compilation and repository-owned generated `dist` directories were removed afterward. The revision reviewed at the start of this repair had 46 passing and four skipped checks; new remote CI and the new Windows runtime tests remain unverified before the final push.

The shared prerequisite/companion fixture `scenarios_08_15_16_19_watch_and_timer_companions_preserve_server` uses two isolated real sessions. An initial-enabled watcher with no positive input roots proves exactly one execution across its activation, prerequisite, and companion roles after shutdown. A separate watched-input session proves a fresh successful input receipt consumed the new content while its server PID stays unchanged. Both require timer progress and actual server cleanup. Actions run `35517880430` exposed a delayed pre-baseline root-directory event even with absent inputs; that legitimate input cause must not be counted as duplicate activation, so an absent file alone is not an isolation barrier.

The same companion fixture records checks and timer ticks in separate files so concurrent child writes cannot interleave one task's marker into the other task's line. Completion assertions remain task-specific and retain the real server PID and shutdown checks.

Repair-pass validation: the revised companion fixture passed ten consecutive native macOS arm64 runs. The macOS x64 failure was observed in Actions run `35515738389`; a local arm64 repetition is not x64 runner evidence.

The Windows job fault fixture now retains the exact descendant process handle and checks its job membership and live state before fault injection, then requires an immediate signaled process state after explicit/destructor cleanup. This prevents PID reuse or a port-only observation from substituting for actual process termination. The child binds its own ephemeral socket and atomically publishes its PID/address. After the kernel exit assertion, address reuse retries only `AddrInUse` for at most ten seconds, without socket-reuse flags; diagnostics retain the termination reason, restricted access mask, PID, attempt count, and elapsed time. Actions run `35515738389` failed both Windows architectures at the former immediate rebind assertion; those logs did not measure descendant state or socket-release latency, so the new independent assertions must establish both on native runners. See Microsoft's [process termination state](https://learn.microsoft.com/en-us/windows/win32/procthread/terminating-a-process) and [socket address reuse behavior](https://learn.microsoft.com/en-us/windows/win32/winsock/using-so-reuseaddr-and-so-exclusiveaddruse).

Validation for the initial CI fixture repairs passed root `cargo test --locked` on macOS arm64 with `RUST_TEST_THREADS=4` and a private short `TMPDIR` (TaskFlow: 18 library tests, one CLI test, and 147 default conformance tests; seven external conformance tests remained ignored). The parent-directory canary remained intact. TaskFlow Clippy passed on the host and for all targets under the Windows GNU cross target; formatting and all 49 CI contract tests passed. The DevHud frontend was generated before root testing and generated repository `dist` output was removed afterward. Windows native execution and the revised macOS x64 fixture are not claimed as locally verified; no external conformance services were rerun for these test-only changes.
...
After the additional receipt/cache bounds, libtest inventory, and manifest-path review fixes, root `cargo test --locked` passed again on macOS arm64 with the same private temporary-directory canary and four test threads. TaskFlow passed 20 library tests, one CLI test, and 148 default conformance tests; seven external tests remained ignored. Host and Windows GNU cross-target all-target Clippy passed after simplifying the missing-file predicates. The Windows checks establish compilation/lint evidence only, not native runtime success. Generated DevHud frontend/client `dist` directories and the owned cross-compilation container/volume were removed after verification.
...
Actions run `35517880430` proved that both Windows architectures could report empty job accounting before the fixture descendant process object signaled. Cleanup now seals new child associations using the documented [active-process limit](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-jobobject_basic_limit_information), retains job-member process handles before termination, and waits for their signals as required by [asynchronous process termination](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-terminateprocess). The existing native fault fixture keeps its immediate exact-handle exit assertion and additionally makes a live parent attempt a child spawn after the capture barrier, requiring rejection before any command executes. Restricted query/termination faults occur after capture to exercise both failure APIs while proving handles survive explicit and destructor retries.

2026-09-21 repair validation: root `cargo test --locked` passed on macOS arm64 with four test threads, a short canonical private temporary root, and an intact sibling canary. TaskFlow passed 20 library tests, one CLI test, and 151 default conformance tests; seven external native/Docker/S3 cases remained ignored and are not claimed as rerun. The split activation/mutation fixture also passed ten consecutive focused runs; its final receipt-race refinement passed focused and root testing. Host and Windows x64 GNU cross-target all-target Clippy passed with warnings denied, as did workspace formatting, all 49 CI contract tests, and actionlint for the owning `CI.yml` and the generated distributed workflow. Repository-wide actionlint v1.7.12 separately rejected the pre-existing `concurrency.queue` key in `release-linux-packages.yml`; the unrelated release workflow was not changed. Windows native runtime and post-push CI results remain unverified locally. Required DevHud frontend/client output was generated before root tests, then repository-owned `dist` output and the isolated cross-compilation container/volume were removed. Public documentation and frontend source were unchanged.

`shard_duration_history_rejects_unbounded_and_nonregular_state` launches the real CLI beneath an independent process deadline with valid, malformed, oversized sparse, linked, and Unix FIFO duration histories. Valid hints affect inventory durations; rejected histories use deterministic defaults, all units still aggregate, and successful execution replaces invalid history with regular state.
