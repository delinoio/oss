...
- Runmoor guides are integrated at `apps/public-docs/docs/runmoor` and published at `https://oss.delino.io/runmoor`, with their complete content and public constraints governed by `docs/apps-runmoor-docs-foundation.md`. Build validation must require all seven Runmoor route artifacts, route-specific navigation/sidebar links, and the integrated URL in discovery pages. The retired `https://runmoor.delino.io` host must be configured outside the repository as a permanent suffix-preserving redirect to `https://oss.delino.io/runmoor`.
...
- External top-level major project links include Nodeup at `https://nodeup.delino.io` and binpm at `https://binpm.delino.io`; Runmoor is an in-site `/runmoor` route.
...
- Nodeup and binpm public documentation are maintained outside this repository at their external origins; Runmoor public documentation is owned by this app under `/runmoor`.
- The shared `@delinoio/docs-site-switcher` theme component must expose Delino OSS, Runmoor, Nodeup, and binpm with the current product marked by `aria-current="page"`; it must remain keyboard and mobile accessible without replacing the complete Rspress navigation component.
...
- Local validation: `pnpm --filter public-docs test`, which builds Rspress, runs `scripts/validate-clean-urls.mjs`, and runs `scripts/runmoor-public-content.test.mjs` to verify every stable route artifact, required headings/links/accessibility landmarks, public-content limits, generated internal `.html` links across navigation-bearing HTML attributes, and forbidden paths or URL credentials in HTML attributes, HTML resources, and CSS `url()` values.
