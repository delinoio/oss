...
- React Forge's nine format guides use `/react-forge/formats/<format>/`. The former same-origin `/react-forge/<format>` paths have explicit permanent redirects to the new guides; this migration does not create a duplicate guide or a retired-host alias.
