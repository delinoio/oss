...
- A coordinated DevHud release may publish its release-bound `/devhud` page, route assets, and shared static assets, but its retained candidate must not contain the four package-local documentation subpaths. The `public_docs` release job rebuilds the complete aggregate from current `main` and overlays only those DevHud-owned candidate files before deploying, so a delayed or recovered DevHud release cannot roll back Runmoor, Nodeup, binpm, or async-commit-hook documentation.
