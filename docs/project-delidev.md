...
The private Codex continuation boundary blocks every resumed binding until the latest native terminal turn, ordered input digests and original effective defaults/permissions match retained execution evidence. Process-replacement continuation after Execute, Plan, Steer and interruption is verified with a private scripted provider on macOS; failed/interrupted outcomes require explicit intent. Public later-turn FIFO/Resume still requires the coordinator and Worker ownership/credential integration, and native history does not resolve missing semantic interaction acceptance or prove cleanup.
