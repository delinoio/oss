...
Private Codex approval adapters now preserve command, file and additional-permission request scopes and validate one exact original response without replay. Installed macOS tests cover real approval/cancellation, a private file patch and an additional permission response. Request closure and generic tool completion remain distinct from exact semantic acceptance. The production Worker now durably publishes original approvals and unanswered closure into server interactions and independent unread inbox entries. Existing authenticated resource/inbox reads expose them, and an installed macOS Worker test verifies their retention through public Stop and owned cleanup. Dedicated owner approval-response RPC/CLI acceptance, Worker claims/control and semantic acceptance/recovery remain pending.
