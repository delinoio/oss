...
- List APIs use opaque cursors. Errors use Connect status codes and vetted stable enum-based reasons for auth, authorization, slug, invitation, team depth/cycle, subscription, overage, reservation, idempotency, deletion, and resource-state failures. The shared safe-error boundary drops arbitrary response metadata, unrecognized protobuf details, and free-form detail fields; the outer request metadata interceptor adds only validated request and trace IDs.
