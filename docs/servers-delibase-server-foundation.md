...
- Invitations are hashed bearer links, seven-day valid, revocable, and reusable by distinct users. Acceptance and revocation use distinct idempotency operations; invitation creation is not idempotent. An invitation may grant only Organization Admin or Member; Member invitations must target a team and assign Team Admin or Member. Owner may be granted only by an existing Owner after the user has joined, and existing membership roles are not silently changed.
