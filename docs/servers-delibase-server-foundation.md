...
- Database constraints prevent overlapping price windows for one meter and overlapping billing periods for one organization, keep subscriptions and ledger periods in the billed organization, validate each ledger entry's running balance under an organization lock, bind reservation price snapshots to the effective catalog version, bind expiry to the meter TTL, require overflow-safe reservation unit-price totals, and cap committed units at the reserved maximum.
...
- `ReserveUsage` requires organization/team/meter IDs, max units, client reference, and idempotency key; atomically verifies the active organization membership, enabled service/meter mapping, current team-name snapshot, effective catalog price, and meter TTL before holding credit/overage portions. Reservation identity, snapshots, limits, holds, references, and timestamps are immutable after creation; only a storage-timestamped held-to-committed/released/expired transition is allowed. Commit applies actual units (never above reserved), consumes no more held credit or overage and uses credit first, derives the commit timestamp in storage, releases unused holds, attributes actor/team/service, and queues Polar usage. Release/expiry returns holds; late commit fails.
