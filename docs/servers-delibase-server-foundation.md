...
- Organization roles: Owner (full access and deletion), Admin (management, billing, limits, full usage, but no Owner management or organization deletion), Member (app use, shared credits, and accessible team usage, but no invoices/full ledger/subscription/overage controls). Preserve at least one Owner.
