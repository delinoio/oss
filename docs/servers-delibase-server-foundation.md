...
- List APIs use opaque cursors. Errors use Connect status codes and vetted stable enum-based reasons for auth, authorization, slug, invitation, team depth/cycle, subscription, overage, reservation, idempotency, deletion, and resource-state failures. Shared authentication distinguishes bearer and forwarded-user reasons; unavailable signing keys fail closed as retryable `Unavailable` dependency failures instead of invalidating the session. The shared safe-error boundary drops arbitrary response metadata, unrecognized protobuf details, and free-form detail fields; the outer request metadata interceptor adds only validated request and trace IDs to unary and streaming failures.
