...
`session create` now atomically retains a session, initial input and workspace preparation job; `session prepare` retries only confirmed failed/canceled preparation using its original request. `session enqueue`, `queue list/edit/remove`, session rename and preparation-aware Stop/Archive/Restore are available through dedicated owner/client RPCs. `session recover-workspace [--cleanup]` reconciles uncertain preparation through its exact original assignment and Worker journal, preserving ready content and requiring explicit cleanup for partial work. Create/prepare/recover-workspace support bounded `--wait`; ready manifests bind all repository identities/commits and are published atomically. Stop/Archive cancel only their own preparation, and Archive waits for completion or confirmed cleanup while preserving ready files. Creation currently reports blocked native execution, and Resume is explicitly unsupported until the execution adapter is integrated. Input modes and transaction-assigned order remain immutable; removed content cannot return through old receipts. Restore preserves pause and outcome. See the [session contract](cmds-delidev-sessions-contract.md) for selections, bounds, Local origin verification, pagination and remaining native lifecycle requirements.
