...
- Passed public-docs `pnpm test`, including the production build and clean-URL/content validation. This fix's validation used temporary state and deterministic adapters; no live GitHub job assignment, real Docker/Tart execution, user-service installation, signing or release publication was performed. Existing preview compatibility limits remain in force.
