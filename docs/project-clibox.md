...
- Issue #971 adds unreleased `clibox fspy record`, `compare`, `assetcov`, `latencylab`, `fbreak`, and `autowatch` on Linux; `min-repro` and macOS/Windows execution backends remain incomplete. See the dedicated file-access contract before describing this family as released.
