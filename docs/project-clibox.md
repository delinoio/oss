...
- The public commands are `run env`, `port list`, `port kill`, `open`, `clipboard copy`, `clipboard paste`, `dotenv list`, `dotenv merge`, `yaml normalize`, `wait tcp`, `wait http`, `wait file`, `text replace`, `time format`, `time add`, `base64 encode`, `base64 decode`, `hash compute`, and `hash verify`; no public Rust/JavaScript library API is provided.
