...
- Cached keys are reused within the TTL. An unknown `kid` forces one immediate refresh even while the cache is fresh so normal Logto signing-key rotation can converge promptly. Remote refreshes are serialized and atomically installed without holding the cache read lock during network I/O, so a slow refresh does not block known fresh-key validation.
- A successful refresh atomically replaces the key set. Duplicate key IDs, empty signing sets, unsupported key types/curves, ES algorithm/curve mismatches, other algorithm/key mismatches, oversized/invalid responses, and unknown keys fail closed.
