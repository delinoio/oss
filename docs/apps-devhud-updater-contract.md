...
Native tests cover all six target mappings, valid and tampered signatures, valid and invalid rotation, signed and unsigned rollback, visible/unminimized/focused scheduling, worker-generation cancellation races, streaming artifact verification, installation/restart failure, single-instance handoff restoration, restart-only retry, macOS atomic bundle rollback, and running-version preservation. Frontend tests cover bilingual accessible notes, all three focus-trapped approvals, opener focus restoration, and the package-installed restart-retry state. Release-gate tests cover exact active native trust-root declarations and reject comments, duplicates, and missing declarations. API tests cover exact routing, package selection, HEAD, malformed/missing targets, and browser-origin denial. `verify:pins`, mobile closure checks, Rust tests/clippy, frontend tests, Go tests/vet, and the release gate are required at their respective CI or release boundary.
