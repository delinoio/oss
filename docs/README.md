...
- `docs/apps-devhud-foundation.md` (`apps/devhud`, sole canonical implementation path; production desktop CEF shell, maintained mobile system-webview hosts, typed local diagnostics/export, and build-only native widget foundation)

DevHud is local-only and independent from DeliDev. `apps/devhud` is the active React/TypeScript/Rsbuild and Tauri application, with a tray-resident production desktop shell on the exact sandboxed upstream CEF runtime and maintained standard WKWebView/System WebView native hosts for mobile. Bundled-only resource policies, deny-by-default networking, and exact desktop-HUD/desktop-settings/mobile capability manifests apply unchanged to normal views and DevTools; generated bundles and mobile manifests/entitlements have package-local security inspection. Typed local diagnostics rotate at seven days/20 MB and can be exported only through a user-activated native picker; export is recursively revalidated, path-free across IPC, preserved by reset, and never remotely transmitted. Package-local WidgetKit and Android AppWidgetProvider targets compile only as non-distributed foundations; the iOS release app embeds no extension and the Android release manifest registers no receiver, so neither widget gallery lists DevHud. Mobile record storage is excluded from iOS device backups and Android cloud backup/device transfer. The closed internal registry, production widget configuration, and user-visible widget state stay empty; no CLI, backend, public API, plugin SDK, deep link, telemetry, account system, remote tool, or DeliDev integration is authorized.
