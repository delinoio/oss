...
- `docs/servers-devhud-realqa-foundation.md` (implemented inactive preset/tracker/auth/deletion and internal GitHub.com provider foundation with stable-ID repository rename synchronization at `servers/devhud-realqa`; future submission/image/billing slices and inactive `https://realqa.deli.dev` / `https://assets.realqa.deli.dev` origins)
