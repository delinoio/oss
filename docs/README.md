...
### devhud
The deterministic bilingual frontend, target-isolated Tauri desktop CEF plus iOS/Android system-webview hosts, production WidgetKit/AppWidgetProvider Deck widgets, synchronized Settings boundary, direct-client GitHub.com provider/setup foundation, and desktop RealQA capture/editor/encrypted drafts are implemented in `apps/devhud`; the administrator SPA and Admin API are implemented in `apps/devhud-admin` and `servers/devhud-api`; issue submission, other product-result surfaces, and the remaining DevHUD domains are planned.
