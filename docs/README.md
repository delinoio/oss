...
DevHud is local-only and independent from DeliDev. `apps/devhud` contains only the bundled-asset and typed CEF feasibility probe plus isolated native macOS x64/ARM64 gate automation; it is not a production application. The macOS jobs stop while compiling pinned upstream `tauri-runtime-cef` because a Linux/BSD-only signal type has an unconditional implementation and target-only dependency. The same revision separately does not expose renderer-termination observation on Windows or Linux. Both require upstream source changes, so no platform gate, product, mobile/widget, production packaging, publishing, or release work may proceed. Production tools and user-visible widgets remain empty; no CLI, backend, public API, plugin SDK, deep link, telemetry, account system, or DeliDev integration is authorized.
