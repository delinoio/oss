...
- `docs/apps-devhud-foundation.md` (`apps/devhud`, sole canonical implementation path; active desktop CEF, mobile system-webview, and build-only native widget foundation)

DevHud is local-only and independent from DeliDev. `apps/devhud` is the active React/TypeScript/Rsbuild and Tauri application foundation, with the pinned upstream CEF runtime selected directly for desktop builds and maintained standard WKWebView/System WebView native hosts for mobile. Package-local WidgetKit and Android AppWidgetProvider targets compile only as non-distributed foundations; the iOS release app embeds no extension and the Android release manifest registers no receiver, so neither widget gallery lists DevHud. Mobile record storage remains device-local. The closed internal registry, production widget configuration, and user-visible widget state stay empty; no CLI, backend, public API, plugin SDK, deep link, telemetry, account system, remote tool, or DeliDev integration is authorized.
