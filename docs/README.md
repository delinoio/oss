...
### devhud
The deterministic bilingual frontend, target-isolated Tauri desktop CEF plus iOS/Android system-webview hosts, synchronized Settings boundary, direct-client GitHub.com provider/setup foundation, least-privilege Chrome context picker, and authenticated Native Messaging broker are implemented; full RealQA capture/product-result surfaces, native widgets, and the remaining DevHUD domains are planned.
