...
### devhud
The deterministic bilingual frontend, target-isolated Tauri desktop CEF plus iOS/Android system-webview hosts, synchronized Settings boundary, and direct-client GitHub.com provider/setup foundation are implemented in `apps/devhud`; capture/product-result surfaces, native widgets, and the remaining DevHUD domains are planned.
