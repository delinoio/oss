...
`packaging/async-commit-hook/release-metadata.json`, the Go version constant, all three package versions and both public installer defaults must match. Release identity is `async-commit-hook@v<MAJOR.MINOR.PATCH>`. Archives are named `ach-<goos>-<goarch>.tar.gz` or `ach-windows-<goarch>.zip`.
...
Direct self-update refuses active components, pending work and Homebrew-owned paths. Candidate `ach version --json` must match the requested release. A lifecycle lock excludes concurrent starts and mode/port/state changes; a pending journal requires explicit recovery. Windows uses a separate replacement helper after the original process exits. Recovery verifies journal-bound digests and restores the previous executable when replacement was interrupted. When the executable is absent, its recorded backup must be a regular nonsymlink file, at most 128 MiB, with bytes matching `OriginalSHA256`. Recovery atomically creates the executable from that authenticated snapshot without overwriting a concurrently restored path; it retains the backup rather than renaming a mutable backup path. Missing, changed, nonregular or oversized backups and failed publication keep the original recovery journal intact. No background automatic update is installed.
...
After authenticity and archive validation, self-update compares the extracted executable SHA-256 with the installed bytes under the lifecycle lock. Identical bytes are version-probed in place and return status `up-to-date` without creating a candidate, state backup, binary backup or replacement journal. Temporary downloads are removed. Matching version strings alone do not skip replacement: a different local build follows candidate version validation, state backup and the existing journaled replacement/recovery path.

Without `--version`, self-update scans all release pages and compares canonical stable ach versions semantically, independently of publication order. Drafts, prereleases, foreign tags and malformed/noncanonical versions are ignored. Discovery failures abort rather than using a partial inventory. An implicit downgrade below the installed version is rejected before asset downloads or backups; intentional rollback requires an explicit canonical `--version` and the existing binary/state recovery precautions.

Update preparation exclusively creates its private backup directory and removes it on VACUUM or evidence-copy failure. A completed backup and candidate are also discarded when preparation fails before journal publication; prior backups are untouched. Cleanup failures are returned with a structured diagnostic. If journal rename succeeded before durability confirmation failed, retain the journal and its referenced files and require explicit recovery rather than leaving a journal pointing to deleted data.
