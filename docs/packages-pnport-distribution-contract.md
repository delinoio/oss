...
GitHub archives, checksums, Sigstore material, POSIX/PowerShell installers and prebuilt-only Homebrew cover the same six targets. Changes to either installer select the six-host native CI execution matrix on main so each host runs installation smoke, including the Windows command launcher. Explicit installation controls updates/rollback. No crates.io, APT/RPM, Apple notarization or Windows Authenticode.
