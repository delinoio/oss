...
- `binpm init`, `binpm env`, `binpm doctor`, source-form and local-command `binpm explain`, `binpm cache key`, `binpm cache list`, `binpm cache prune`, `binpm cache clean`, `binpm list`, `binpm remove`, `binpm info`, `binpm outdated`, `binpm update`, `binpm verify`, bare-executable and archive install flows, checksum sidecar/manifest discovery, and `binpm x` execution have concrete runtime behavior. Signature verification remains implementation work.
