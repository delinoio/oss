...
- `binpm init`, `binpm env`, `binpm doctor`, source-form and local-command `binpm explain`, `binpm cache key`, `binpm cache list`, `binpm cache prune`, `binpm cache clean`, `binpm list`, `binpm remove`, `binpm info`, `binpm outdated`, `binpm verify`, bare-executable and archive install flows, package Sigstore bundle verification for GitHub.com releases, and `binpm x` execution have concrete runtime behavior. Checksum sidecar/manifest discovery and global update remain implementation work.
...
- Local `binpm update` and scoped `binpm remove` must print the selected local or global scope before mutation and support `--dry-run` previews that leave manifests, lockfiles, package records, cache references, and executables unchanged. `binpm update --global` remains pending implementation and must fail with a workaround that points users to `binpm outdated --global`, `binpm info --global <cmd>` for the stale command's recorded source and selected binary, and `binpm install <source> --as <cmd> --bin <selected_binary>` for each stale global tool. `binpm outdated` must report the source needed for that reinstall path. `--global` remains an explicit scope override even inside a project and does not currently require an interactive confirmation prompt.
