...
- CI must validate the production static artifact, SPA fallback, manifest/service worker, sensitive-cache exclusions, responsive accessibility, and Connect client generation compatibility. The deterministic build must leave the checked-in `apps/delidev-app/dist` artifact unchanged. The self-gated app job installs frozen dependencies, runs the dedicated automated WCAG suite, and executes Playwright smoke coverage at representative desktop and mobile sizes for public, signed-out, authenticated/onboarding, empty, validation, permission-denied, offline, and dependency-error states; server-backed authenticated states use deterministic fixtures and fail closed when dependencies are unavailable.
