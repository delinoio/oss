...
- Background-authorization UI remains within the billing/usage sections of `/account` and `/o/:orgSlug/settings`; no new top-level route is allowed. It uses only the human `BillingService` create/get/opaque-list/revoke methods, keeps grants and summarized current-period usage network-only and memory-only, disables mutation offline, and retains distinct create/revoke idempotency keys across ambiguous retries. Owners/Admins receive organization-wide list/revoke visibility, Members only grants they created, and RealQA may show only its own grant status, payer, current daily units, billing failure, and rebind/revoke controls. Deck has no v1 purpose and cannot use this flow.
