...
- The organization shell loads the server-authoritative caller role. Team hierarchy creation, rename, depth-safe move, and confirmed subtree deletion controls render only for Owners and Admins; deletion retries retain one key until success or dialog cancellation. The member surface lets the same roles create reusable invitation links, page through active invitations, and revoke them with replay-safe retries; creation bearer tokens bypass React Query and remain only in component memory. Subscription, billing-portal, overage-limit, and complete ledger controls follow the same role boundary; missing available/held balance wrappers render as unavailable while explicit zero remains zero, overage limits retain exact USD micro-unit handling, a configured limit with a missing value wrapper renders as unavailable instead of becoming editable zero, and ledger reads use opaque cursor pagination.
