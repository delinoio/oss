...
- CI must validate the production static artifact, SPA fallback, manifest/service worker, sensitive-cache exclusions, responsive accessibility, and Connect client generation compatibility. The deterministic build must leave the checked-in `apps/delidev-app/dist` artifact unchanged. The self-gated app job installs frozen dependencies, runs the dedicated automated WCAG suite, and executes Playwright smoke coverage at representative desktop and mobile sizes for public, signed-out, offline, and dependency-error states; protected routes fail closed when Logto is not configured.
