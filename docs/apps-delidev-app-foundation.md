...
- The member surface lets Owners and Admins create reusable invitation links, page through active invitations, and revoke them with replay-safe retries; creation bearer tokens bypass React Query and remain only in component memory. Invitation preview distinguishes invalid, revoked, and expired server states and explains that an existing member's organization role is unchanged. Subscription, billing-portal, overage-limit, and complete ledger controls follow the same role boundary; missing available/held balance wrappers render as unavailable while explicit zero remains zero, overage limits retain exact USD micro-unit handling, a configured limit with a missing value wrapper renders as unavailable instead of becoming editable zero, and ledger reads use opaque cursor pagination.
- Connect error details are decoded through the generated `ErrorDetail` schema for permission, last-Owner, invitation, hierarchy, protected-General, active-reservation, slug, and deletion states. Dialogs are rendered as body-level modals, make the background inert and hidden from assistive technology, contain focus, close on `Esc`, and restore the invoking control. Account deletion announces request and accepted progress before invoking Logto sign-out.
