...
- Missing balance wrappers render as unavailable while explicit zero remains zero. A configured overage limit with a missing value wrapper is not editable. The overage dialog focuses its critical input after remembering the opener, contains keyboard focus, closes with `Esc`, restores focus, retains one idempotency key across ambiguous retries for unchanged input, and disables submission offline. Billing, balance, ledger, usage, checkout, portal, and overage request data remain network-only and are excluded by the explicit service-worker allowlist and all browser persistence.
