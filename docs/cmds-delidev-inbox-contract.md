...
Inbox documents retain references and bounded terminal metadata, not copies of question/approval/answer/transcript content. Current response validity must be resolved from the original interaction plus current session state. A stale notification or inbox reference never renews execution authority. Completion/failure records remain alongside questions even when the desktop is disconnected or notifications are disabled. Notification presentation, permission/delivery handling, client activation and per-device delivery deduplication are separate pending contracts; no desktop delivery is claimed from an inbox write.

`InboxService.GetInboxEntry`, `ListInbox` and `SetInboxReadState` are owner/paired-client-only and revalidate client authority within each transaction. Get/list join the inbox entry, current session and original interaction when present without implicitly marking anything read. Mark-read/unread binds the actor, inbox ID/revision and explicit target state into a durable reference-only receipt. A repeated accepted request returns the current joined view, never reapplies an old state or retrieves content for a revoked client. A current no-op state change retains its existing revision.

The CLI exposes `inbox list [--session-id ID] [--project-id ID] [--read-state all|read|unread] [--source all|interaction|execution-terminal] [--limit N] [--page-token TOKEN]`, `inbox get|inspect --id ID`, and `inbox mark-read|mark-unread --id ID --revision N`. Mutations use the common durable `--request-id`; their revision always belongs to the inbox entry. List results contain `entries` of joined views plus `next_page_token`; inspect/mutation results contain `view`, with `replayed` for a mutation. The view's current `interaction` provides the original ID/revision for question `interaction respond` or approval `interaction approve`; an inbox ID is never substituted for native response authority. Closed/archived sources remain inspectable, and reading cannot restore response eligibility. `inbox snapshot` and generic resource/event RPCs continue to expose the retained metadata records.
