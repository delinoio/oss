...
- **Implemented**: All enums, UnitTask/SubTask/Workspace messages with full fields, List/Create/Update RPCs, EventStreamService streaming RPC. New enums: `SessionForkIntent`, `SessionForkStatus`, `WorkspaceWorkStatus`, `AgentCliType`, `ReviewCommentStatus`. New enum values: `NOTIFICATION_TYPE_AGENT_INPUT_REQUIRED`, `STREAM_EVENT_TYPE_SESSION_FORK_UPDATED`, `STREAM_EVENT_TYPE_WORKSPACE_WORK_STATUS_UPDATED`. New messages: `SessionSummary` (with lineage fields and `usage` field referencing `UsageMetrics`), `AgentCapability` (with `supports_plan_mode` field), `SessionForkUpdatedEvent`, `WorkspaceWorkStatusUpdatedEvent`, `UsageMetrics` (normalized token/cost counters), `WorkspaceSettings`. `ReviewComment` message includes anchor fields (`file_path`, `side`, `line_number`). `UnitTask` message: `title`/`description` replaced by `prompt`, added `agent_cli_type` and `plan_mode` fields. `CreateUnitTaskRequest`: `title`/`description` replaced by `prompt`, added `agent_cli_type` and `plan_mode`. `StartExecutionRequest`: added `plan_mode` field. New RPCs on SessionService: `ListSessionCapabilities`, `ForkSession`, `ListForkedSessions`, `ArchiveForkedSession`, `GetLatestWaitingSession`, `SubmitSessionInput`. New RPC on WorkspaceService: `GetWorkspaceWorkStatus`. New RPC on NotificationService: `MarkNotificationRead`. New RPCs on PrManagementService: `ListPullRequests`, `UpdatePullRequest`. New RPCs on RepositoryService: `CreateRepositoryGroup`, `UpdateRepositoryGroup`, `DeleteRepositoryGroup`, `ListRepositories`, `CreateRepository`, `UpdateRepository`, `DeleteRepository`, `ListRepositoryGroups`. New service: `SettingsService` (`GetWorkspaceSettings`, `UpdateWorkspaceSettings`). New RPCs on WorkerExecutionService: `StartExecution` (server-streaming), `SubmitWorkerInput`, `CancelExecution`. New service: `WorkerSessionAdapterService` (`GetAgentCapabilities`, `ForkSessionAdapter`). New RPCs on ReviewCommentService: `CreateReviewComment`, `UpdateReviewComment`, `DeleteReviewComment`, `ResolveReviewComment`, `ReopenReviewComment`. `Repository` entity added as independent message with `repository_id`, `workspace_id`, `repository_url`, `default_branch_ref`, `display_name`.
- **Generated**: Go code (`protos/dexdex/gen/`), TypeScript code (`apps/dexdex/src/gen/`).

### Main Server (`servers/dexdex-main-server`)
- **Implemented**: Connect RPC server with WorkspaceService (GetWorkspace, ListWorkspaces, GetWorkspaceWorkStatus with priority-based status computation), TaskService (ListUnitTasks, ListSubTasks, CreateUnitTask with prompt-only input and agent type resolution from request → workspace settings → CLAUDE_CODE fallback, UpdateUnitTaskStatus, GetUnitTask, GetSubTask, SubmitPlanDecision), SessionService (GetSessionOutput, ListSessionCapabilities, ForkSession with execution dispatch, ListForkedSessions, ArchiveForkedSession, GetLatestWaitingSession, SubmitSessionInput), NotificationService (ListNotifications, MarkNotificationRead with stream event publishing), EventStreamService (fan-out, replay, heartbeat, SESSION_FORK_UPDATED and WORKSPACE_WORK_STATUS_UPDATED events), RepositoryService (GetRepositoryGroup, ListRepositoryGroups, CreateRepositoryGroup, UpdateRepositoryGroup, DeleteRepositoryGroup, ListRepositories, CreateRepository, UpdateRepository, DeleteRepository), SettingsService (GetWorkspaceSettings, UpdateWorkspaceSettings), PrManagementService (GetPullRequest, ListPullRequests, UpdatePullRequest), ReviewAssistService (ListReviewAssistItems), ReviewCommentService (ListReviewComments, CreateReviewComment, UpdateReviewComment, DeleteReviewComment, ResolveReviewComment, ReopenReviewComment), BadgeThemeService (GetBadgeTheme handler with badge theme store methods). EventBroadcaster interface with Redis-backed `RedisFanOut` implementation for scale-mode deployment. Worker client with agent capability caching (5-minute TTL). Worker dispatch via `internal/worker/dispatch.go` (Dispatcher manages execution goroutines, dispatches to worker via StartExecution streaming RPC with plan_mode field, consumes events including WorktreeStatusEvent, publishes through FanOut). `DispatchForkExecution` dispatches fork execution with parent_session_id and fork_intent to worker. Worktree orchestration via `internal/worker/worktree_coordinator.go` (periodic stale worktree cleanup, tracks WorktreeAssignment state from worker-emitted WorktreeStatusEvent lifecycle events). PR polling via `internal/polling/pr_poller.go` (polls GitHub via `gh api`, detects status changes, creates notifications). Runtime configuration via `internal/config/config.go` (centralized env var parsing). Session summary store for fork orchestration and lineage tracking. FanOut event publishing on mutations. In-memory store with session output storage, worktree assignment tracking, and rich seed data support (`DEXDEX_SEED_DATA=true`). PostgreSQL persistence layer via sqlc with conditional store selection (`DEXDEX_DATABASE_URL`). `UpdatePullRequest` and `FindSubTaskBySessionID` added to Store interface. CORS middleware for dev. Default addr `127.0.0.1:7878`.

### Worker Server (`servers/dexdex-worker-server`)
- **Implemented**: Connect RPC server with SessionService (GetSessionOutput) and WorkerSessionAdapterService (GetAgentCapabilities for CLAUDE_CODE, CODEX_CLI, OPENCODE with `supports_plan_mode` field — true for CLAUDE_CODE, false for others; ForkSessionAdapter). WorkerExecutionService with `StartExecution` (server-streaming with worktree lifecycle events, fork execution support via `--resume`, and `--plan` flag for Claude Code when `plan_mode` is true), `SubmitWorkerInput`, and `CancelExecution` RPC implementations. `StartExecution` accepts `parent_session_id`, `fork_intent`, and `plan_mode` fields. Worktree lifecycle events emitted during execution (PREPARING→READY→EXECUTING→CLEANING_UP→CLEANED, FAILED on error). Runtime configuration via `internal/config/config.go` (centralized env var parsing). Worktree orchestration via `internal/worktree/manager.go` (bare clone/fetch, worktree add/remove, stale cleanup, concurrency semaphore). Agent execution via `internal/handler/agent_exec.go` (spawns Claude Code/Codex CLI/OpenCode processes, parses NDJSON output, normalizes to proto; fork mode uses `--resume` for Claude Code; plan mode passes `--plan` flag to Claude Code). UsageAccumulator for cost/usage normalization from agent NDJSON output into `UsageMetrics` proto. Plan YAML parser for structured plan rendering from agent output. Session output normalization (raw kind → proto enum). In-memory session store with lineage tracking. Commit chain validation. Default addr `127.0.0.1:7879`.

### Desktop App (`apps/dexdex`)
- **Implemented**: Linear-style task management UI with light mode default. Connect RPC integration via React Query + @connectrpc/connect-query (replaces mock data). Proto-to-view adapter layer. react-router for page navigation (`/tasks`, `/tasks/:taskId`, `/inbox`, `/prs`, `/settings`, `/repositories`, `/repository-groups`). Sidebar navigation with Pull Requests (collapsible, Cmd+B), Repositories, and Repository Groups pages. Task list with status filters, keyboard navigation, and truncated prompt display (replacing title). Task detail with subtask timeline (fetched via RPC), plan decision controls (Approve/Revise/Reject, wired to server), session output panel (fetched via RPC), session input form for waiting-for-input subtasks. PR management page with status badges (OPEN, APPROVED, CHANGES_REQUESTED, MERGED, CI_FAILED). Review assist panel with suggestions and inline comments. Tab system. Command palette (Cmd+K). Global keyboard shortcuts (G+T, G+I, C). Tauri menu bar tray with workspace work status icon/tooltip. Global shortcut Cmd/Ctrl+Shift+I for question handoff (opens latest waiting session or inbox). Event stream consumer with real Connect streaming RPC and query cache invalidation on events, including SESSION_FORK_UPDATED, WORKSPACE_WORK_STATUS_UPDATED, PR_UPDATED, REVIEW_ASSIST_UPDATED, INLINE_COMMENT_UPDATED. Dark mode toggle (opt-in). Notification inbox with server-side read state (MarkNotificationRead replaces local state). Create task dialog with prompt-only input, agent type selector, and plan mode toggle (wired to server). `SessionForkPanel` component. `SessionInputForm` component. 24 UI tests passing with mock transport.
- Repository group selector in create task dialog (useListRepositoryGroups hook + select dropdown). Credential bridge with Tauri commands (store_credential, list_credentials, delete_credential) and CredentialManager React component in settings page. Settings page with tabs: General (default agent setting), Appearance, Shortcuts, Credentials, About. Repository CRUD pages (`/repositories`, `/repository-groups`) with full create/update/delete flows. Hooks for all new RPCs (useCreateRepositoryGroup, useUpdateRepositoryGroup, useDeleteRepositoryGroup, useListRepositories, useCreateRepository, useUpdateRepository, useDeleteRepository, useGetWorkspaceSettings, useUpdateWorkspaceSettings).
