...
- Human RPCs require the Deck-audience user token plus the dedicated memory-only delibase-audience forwarded bearer metadata defined by the server contract. Only `DeleteFeatureData` in delibase-lifecycle mode instead requires the exact Deck-scoped delibase M2M identity; that identity is rejected by every other procedure. Generated clients must treat all credentials as sensitive and keep them out of messages, logs, errors, caches, persistence, and diagnostics.
